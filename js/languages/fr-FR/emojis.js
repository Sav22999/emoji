// descriptions took from Emojipedia.org
lang = 'fr';
//use i18n language code of the browser, more details: https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/API/i18n/LanguageCode
titles_language[lang] = {
    '🔍': 'Résultats',
    '🕒': 'Émoticônes les plus utilisés',
    '😀': 'Émoticône',
    '🙋': 'Personnes',
    '🦊': 'Animaux',
    '🅰️': 'Symboles',
    '🍎': 'Nourriture et boissons',
    '🏳️‍🌈': 'Drapeaux',
    '🏊': 'Sportif',
    '✈️': 'Voyage et lieux',
    '🖱️': 'Technologies et bureau',
    '👗': 'Vêtements et accessoires',
    '🖐️': 'Mains et parties du corps',
    '🛎️': 'Autre'
};
emojis_language[lang] = [];
/*search*/
emojis_language[lang][0] = {};
/*smileys*/
emojis_language[lang][2] = {
    '😀': [
        'Visage du sourire',
        ':)',
        'Visage heureux',
        'Émoticône'
    ],
    '😃': [
        'Visage souriant avec de grands yeux',
        ':D',
        'Visage heureux',
        'Émoticône',
        'Visage souriant avec bouche ouverte'
    ],
    '😄': [
        'Visage souriant avec les yeux souriants',
        'Visage heureux',
        'Émoticône',
        'Souillant avec des yeux étincelants',
        'Visage souriant avec la bouche ouverte et les yeux souriants'
    ],
    '😁': [
        'Beaming Face avec des yeux souriants',
        'Visage souriant avec les yeux souriants'
    ],
    '😆': [
        'Visage épineux',
        'xD',
        '><',
        'Grand sourire',
        'Sourire aux yeux fermés',
        'Rire',
        'Visage souriant avec des yeux serrés',
        'Visage souriant avec bouche ouverte et yeux serrés'
    ],
    '😅': [
        'Sangsue avec sueur',
        'Exercice',
        'Sueur heureuse',
        'Grinning Face avec des yeux étincelants et une goutte de sueur',
        'Visage souriant avec bouche ouverte et sueur froide'
    ],
    '🤣': [
        'ROFL',
        'Lancer sur le sol riant'
    ],
    '😂': [
        'Visage avec larmes de la joie',
        'Rire',
        'Cri riant',
        'Larmes riantes',
        'LOL'
    ],
    '🙂': [
        'Visage légèrement souriant',
        ':)',
        'Légèrement heureux',
        'C\'est bon'
    ],
    '🙃': [
        'Face à l\'envers vers le bas',
        'Sarcasm',
        'Silly',
        'Face vers le bas'
    ],
    '😉': [
        'Visage de clin d\'oeil',
        ';)',
        'Clin d\'oeil',
        'Fais un clin d\'oeil',
        'Visage clignotant'
    ],
    '😊': [
        'Visage souriant avec les yeux souriants',
        '^^',
        'Visage heureux',
        'Sourire',
        'Émoticône'
    ],
    '😇': [
        'Visage souriant avec Halo',
        'Ange',
        'Halo'
    ],
    '🥰': [
        'Visage souriant avec des cœurs',
        'En Face d\'Amour',
        'Visage souriant avec les yeux souriants et les trois cœurs'
    ],
    '😍': [
        'Visage souriant avec les yeux de cœur',
        '*.*',
        'Yeux de cœur',
        'Visage cardiaque',
        'Visage souriant avec des yeux en forme de cœur'
    ],
    '🤩': [
        'Sang-Étoile',
        'Excité',
        'Yeux étoilés',
        'Œil étoilé',
        'Wow Face',
        'Visage aux yeux étoilés',
        'Grinning Face avec les yeux des étoiles'
    ],
    '😘': [
        'Visage soufflant un baiser',
        ':*',
        ':-*',
        'Coup d\'un baiser',
        'Baiser soufflant',
        'Baiser',
        'Visage en train de lancer un baiser'
    ],
    '😗': [
        'Visage embrassant',
        ':*',
        ':-*',
        'Visage de canard',
        'Kissy Face',
        'Siffler'
    ],
    '☺️': [
        'Visage souriant',
        'Visage heureux',
        'Émoticône',
        'Sourire',
        'Visage souriant blanc'
    ],
    '😚': [
        'Baiser la face avec les yeux fermés',
        'Baiser la face',
        'Kissy Face'
    ],
    '😙': [
        'Baiser la face avec les yeux souriants',
        'Baiser la face',
        'Kissy',
        'Siffler',
        'Siffler'
    ],
    '😋': [
        'Nourriture savoureuse face',
        'Bonjour',
        'Affamé',
        'Lèvres lèvres souriantes',
        'Nourriture délicieuse face à face',
        'Yum Yum'
    ],
    '😛': [
        'Visage avec langue',
        ':P',
        'Joue',
        'Visage de langue',
        'Sortie de langue',
        'Visage avec langue bloquée'
    ],
    '😜': [
        'Visage du clin d\'oeil avec la langue',
        'P; P',
        'Fou',
        'Visage fou',
        'Viser la face avec une langue bloquée',
        'Visage avec une langue bloquée et un clin d\'oeil'
    ],
    '🤪': [
        'Zany Face',
        'Yeux fous',
        'Excité',
        'Sauvage',
        'Goofy Face',
        'Grinning Face avec un grand et un petit œil'
    ],
    '😝': [
        'Quadrant la face avec la langue',
        'xP',
        'Sortie de la langue',
        'Face aux langages bloqués et aux yeux serrés',
        'Visage avec langue bloquée et yeux serrés'
    ],
    '🤑': [
        'Mâche-bouche',
        ':$',
        'Yeux de Panneau Dollar',
        'Face d’argent',
        'Riche'
    ],
    '🤗': [
        'Visage d\'embrassage',
        'Câlin',
        'Accrochage',
        'Câlins',
        'Un visage heureux avec des mains accablantes'
    ],
    '🤭': [
        'Oups',
        'Visage à mains nues',
        'Visage souriant avec les yeux souriants et la bouche recouverte à la main'
    ],
    '🤫': [
        'Shh',
        'Écraser',
        'Ecrasement du visage',
        'Masquage',
        'Silencieux',
        'Silencieux',
        'Silencieux',
        'Visage avec les lèvres fermées couvertes par le doigt'
    ],
    '🤔': [
        'Qu\'est-ce que',
        '?',
        'Hmm',
        'Visage Pensant',
        'Pouce de menton',
        'Réfléchisseur',
        'Lancer de l\'ombre'
    ],
    '🤐': [
        'Code postal',
        'Zipper-bouche face',
        'Lèvres scellées',
        'Lèvres scellées',
        'Zipper',
        'Visage avec une bouche Zipper'
    ],
    '🤨': [
        'Visage avec sourcils levés',
        'Colbert',
        'Le Rocher',
        'Visage avec sourcils levés',
        'Visage avec un sourcil levé'
    ],
    '😐': [
        'Visage neutre',
        ':|',
        'Face avec bouche droite',
        'Face droite'
    ],
    '😑': [
        'Visage sans expression',
        '-_-',
        'Face avec bouche droite',
        'Visage droit'
    ],
    '😶': [
        'Visage sans bouche',
        'Visage vide',
        'Sans bouche',
        'Silencieux',
        'Silencieux'
    ],
    '😏': [
        'Visage souriant',
        'Rinçage',
        'Visage sexuel',
        'Smug Face',
        'Sourire suggestif'
    ],
    '😒': [
        'Visage non utilisé',
        'Mécontent',
        'Meh',
        'Œil latéral',
        'Sans impression'
    ],
    '🙄': [
        'Visage avec les yeux roulés',
        'Rouleau des yeux'
    ],
    '😬': [
        'Visage grimaçant',
        'Maladroit',
        'Eek',
        'Pied dans la montagne',
        'Nervous',
        'Snapchat Mutuel #1 Meilleur Ami'
    ],
    '🤥': [
        'Face de mensonge',
        'menteur',
        'Nez long',
        'Pinocchio'
    ],
    '😌': [
        'Visage retiré',
        'Contenus',
        'Plein de plaisir'
    ],
    '😔': [
        'Désolé',
        'Visage Pensif',
        'Pensif',
        'Tristesse',
        'format@@0 Sadface',
        'Chagrin',
        'Visage pensif triste'
    ],
    '😪': [
        'Visage endormi',
        'Larme latérale',
        'Bulle de chat Snot'
    ],
    '🤤': [
        'Salive',
        'Drooling Face',
        'Drool'
    ],
    '😴': [
        'Visage endormi',
        'ZZ',
        'Visage de sommeil',
        'Arrondissement',
        'Zzz Face'
    ],
    '🥱': ['Visage d\'anéantissement'],
    '😷': [
        'Visage avec Masque Médical',
        'Coronavirus',
        'COVID-19',
        'Masque',
        'Masque chirurgical'
    ],
    '🤒': [
        'Visage avec Thermomètre',
        'Ill',
        'Malade'
    ],
    '🤕': [
        'Visage à la tête',
        'Tête bandée',
        'Maladroit',
        'Blessé'
    ],
    '🤢': [
        'Visage nauséé',
        'Dégoût',
        'Visage vert',
        'Vomi'
    ],
    '🤮': [
        'Vomissement de visage',
        'Dire',
        'Bleah',
        'Bleurgh',
        'Yuk',
        'Yuck',
        'Eu',
        'Dire',
        'Lancer vers le haut',
        'Vomi',
        'Vomissement de la bouche ouverte'
    ],
    '🤧': [
        'Visage éternuant',
        'Gesundheit'
    ],
    '🥵': [
        'Visage chaud',
        'Visage surchauffé'
    ],
    '🥶': [
        'Visage froid',
        'Cool',
        'Froideur',
        'Visage Glacant'
    ],
    '🥴': [
        'Visage Woozy',
        'Visage ivre',
        'Visage avec les yeux inconnus et la bouche vague'
    ],
    '😵': [
        'Dizzy Face',
        'Visage des yeux de croix',
        'Visage des yeux en spirale'
    ],
    '🤯': [
        'Tête explosive',
        'Coup d\'esprit',
        'Face foudroyée avec tête explosive'
    ],
    '🤠': [
        'Chapeau de Cowboy',
        'Cowboy',
        'Visage de Cowboy',
        'Visage avec Chapeau de Cowboy'
    ],
    '🥳': [
        'Visage de groupe',
        'Visage de la fête',
        'Face à Corne de la fête et Chapeau de fête'
    ],
    '😎': [
        'Visage souriant avec lunettes de soleil',
        'Cool',
        'Meilleurs amis mutuels (Snapchat)',
        'Lunettes de soleil'
    ],
    '🤓': [
        'Visage Nerd',
        'Nerdy'
    ],
    '🧐': ['Visage avec monocle'],
    '😕': [
        'Visage confus',
        ':/',
        ':S',
        'Non pluss',
        'Problème'
    ],
    '😟': [
        'Visage inquiet',
        ':(',
        'Tristesse',
        'format@@0 Sadface'
    ],
    '🙁': [
        'Visage légèrement froncé',
        ':(',
        'Légèrement triste'
    ],
    '☹️': [
        'Face frontale',
        ':(',
        'Megafrown',
        'Face de frondaison blanche'
    ],
    '😮': [
        'Wow',
        ':O',
        'Oh mon!',
        'Visage avec bouche ouverte',
        'Bouche ouverte',
        'Visage Surprisé avec bouche ouverte'
    ],
    '😯': [
        'Visage purgé',
        ':O',
        'Surprise',
        'Visage Surprisé'
    ],
    '😲': [
        'Face Électrisée',
        ':O',
        'Wow',
        'Visage étourdi',
        'Visage ivre',
        'Visage gastrique'
    ],
    '😳': [
        'Visage vidé',
        ':$',
        'Visage flottant',
        'Embarrassé',
        'Honte',
        'Visage avec des yeux ouverts larges'
    ],
    '🥺': [
        'Visage de tête',
        ':(',
        'mendicité',
        'Yeux brillants',
        'Simp',
        'Visage avec les Yeux Pleading'
    ],
    '😦': [
        'Frowning Face avec bouche ouverte',
        'D:',
        'Anéantissement'
    ],
    '😨': [
        'Effrayé',
        'D:',
        'Visage effrayant',
        'Surprise'
    ],
    '😰': [
        'Visage angélique avec sueur',
        'Visage bleu',
        'Visage inquiet',
        'Visage nerveux',
        'Visage avec bouche ouverte et sueur froide'
    ],
    '😥': [
        'Sad mais Retrait de la face',
        ':\'(',
        'Sueur sourcil',
        'Visage dissolu mais soulagé'
    ],
    '😢': [
        'Visage pleurant',
        ':\'(',
        'En pleurant',
        'Déchirure'
    ],
    '😭': [
        'Visage fort pleurant',
        'Bawling',
        'En pleurant',
        'Larmes tristes',
        'Sobbing'
    ],
    '😱': [
        'Cri de peur',
        'Accueil seul',
        'Cri',
        'Visage hurlant'
    ],
    '😖': [
        'Visage ConFondé',
        'Bouche carquoisante',
        'Visage Scrunché'
    ],
    '😣': [
        'Visage persistant',
        'Visage sans défense',
        'Yeux croustillés'
    ],
    '😞': [
        'Visage désavoué',
        ':(',
        'Tristesse',
        'format@@0 Sadface'
    ],
    '😓': [
        'Descendre la face avec la sueur',
        'Travail difficile',
        'Visage de la sueur triste',
        'Visage avec sueur froide'
    ],
    '😩': [
        'Visage Weary',
        'Visage Détraqué',
        'Muriers'
    ],
    '😫': [
        'Visage fatigué',
        'Épuisé',
        'Fed Up',
        'Visage Détraqué'
    ],
    '😤': [
        'Visage avec la vapeur du nez',
        'Diffusion des griefs',
        'Frustré',
        'Visage fou',
        'Vaporisation',
        'Pousser avec la colère'
    ],
    '😡': [
        'Visage en mouvement',
        'Visage en colère',
        'Visage Grumpy',
        'Visage fou',
        'Visage rouge'
    ],
    '😠': [
        'Visage en colère',
        'En colère',
        'Visage Grumpy'
    ],
    '🤬': [
        'Taillade',
        '@#$%&!',
        'Visage avec symboles en bouche',
        'Malédiction',
        'Coussin',
        'Grawlix',
        'Visage avec symboles à la bouche',
        'Visage sérieux avec symboles recouvrant la bouche'
    ],
    '🥲': ['Visage souriant avec larme'],
    '🥸': ['Visage déguisé'],
    '😈': [
        'Visage souriant avec des cornes',
        'Diable',
        'Cornes du Diable',
        'Diable heureux',
        'Diable violet',
        'Diable rouge'
    ],
    '👿': [
        'Face en colère avec des cornes',
        'Diable',
        'Cornes du Diable',
        'Diable violet',
        'Gobelin violet',
        'Diable triste',
        'Imp'
    ],
    '🤡': [
        'Visage de clown',
        'Clown effrayant',
        'Clown du mal',
        'Clown effrayant'
    ],
    '👽': [
        'Alien',
        'ET'
    ],
    '👾': [
        'Monstre alien',
        'Envahisseur de l\'espace',
        'Monstre de jeu vidéo'
    ],
    '🤖': [
        'Robot',
        'Visage de robot'
    ],
    '👹': [
        'Ogre',
        'Masque',
        'Oni',
        'Monstre Rouge',
        'Ogres japonais'
    ],
    '👻': [
        'Fantôme',
        'Halloween'
    ],
    '🗣️': [
        'Tête parlante',
        'Plainte de Mansplaining',
        'Cri',
        'En criant',
        'Tête parlante dans la Silhouette'
    ],
    '👤': [
        'Poussière dans la Silhouette',
        'Ombre',
        'Silhouette',
        'Utilisateur',
        'Silhouette de la personne'
    ],
    '🫂': ['Des gens en train de se câliner'],
    '👥': [
        'Busts dans la Silhouette',
        'Ombres',
        'Silhouettes',
        'Utilisateurs',
        'Silhouette de deux personnes'
    ]
};
/*people*/
emojis_language[lang][3] = {
    '👶{{skin_tone}}': [
        'Bébé',
        'Enfant',
        'Enfant'
    ],
    '🧒{{skin_tone}}': [
        'Enfant',
        'Sexe enfant neutre'
    ],
    '👧{{skin_tone}}': ['Fille'],
    '🧑{{skin_tone}}': [
        'Personne',
        'Sexe Neutre Adulte',
        'Adulte'
    ],
    '👱{{skin_tone}}': [
        'Personne: Cheveux blonds',
        'Blonde',
        'Blonde',
        'Blond Man',
        'Personnes à cheveux Blond',
        'Homme avec cheveux Blonde'
    ],
    '👨{{skin_tone}}': [
        'Homme',
        'Homme',
        'Homme de Moustache'
    ],
    '🧔{{skin_tone}}': [
        'Homme : Barbe',
        'Barbe',
        'Homme barbé',
        'Personne Barbée'
    ],
    '👨{{skin_tone}}‍🦰': [
        'Homme : Cheveux rouges',
        'Gingembre',
        'Man Redhead',
        'Homme aux cheveux rouges'
    ],
    '👨‍{{skin_tone}}‍🦱': [
        'Homme : Cheveux bouclés',
        'Homme avec cheveux bouclés'
    ],
    '👨‍{{skin_tone}}‍🦳': [
        'Homme : Cheveux blancs',
        'Homme avec cheveux gris'
    ],
    '👨‍{{skin_tone}}‍🦲': [
        'Homme : Chauve',
        'Homme sans cheveux'
    ],
    '👩{{skin_tone}}': [
        'Femme',
        'Femme',
        'Dame',
        'Femme jaune'
    ],
    '👩‍{{skin_tone}}‍🦰': [
        'Femme: Cheveux rouges',
        'Ginger Femme',
        'Woman Redhead',
        'Femme aux cheveux rouges'
    ],
    '👩‍{{skin_tone}}‍🦱': [
        'Femme: Cheveux frisés',
        'Femme avec cheveux bouclés'
    ],
    '👩‍{{skin_tone}}‍🦳': [
        'Femme: Cheveux blancs',
        'Femme aux cheveux gris'
    ],
    '👩‍{{skin_tone}}‍🦲': [
        'Femme: Chauve',
        'Femme sans cheveux'
    ],
    '👱{{skin_tone}}‍♀️': [
        'Femme: Cheveux blonds',
        'Femme aux cheveux blonds'
    ],
    '👱{{skin_tone}}‍♂️': [
        'Homme : Cheveux blonds',
        'Homme avec cheveux Blonde'
    ],
    '🧓{{skin_tone}}‍': [
        'Personne âgée',
        'Sexe Neutre Adulte'
    ],
    '👴{{skin_tone}}‍': [
        'Vieux homme',
        'Homme aîné',
        'Grandpa',
        'Homme plus ancien'
    ],
    '👵{{skin_tone}}‍': [
        'Vielle Femme',
        'Femme âgée',
        'Grand-mère',
        'Nanna',
        'Vielle Dame',
        'Femme âgée'
    ],
    '👼{{skin_tone}}‍': [
        'Bébé Ange',
        'Ange',
        'Cherub',
        'Cupidon',
        'Rôle'
    ],
    '🎅{{skin_tone}}‍': [
        'Père Noël',
        'Saint Nicolas',
        'Sinterklaas',
        'Père Noël'
    ],
    '🤶{{skin_tone}}‍': [
        'Mme Claus',
        'Noël de la mère'
    ],
    '🤴{{skin_tone}}‍': ['Prince'],
    '👸{{skin_tone}}‍': [
        'Princesse',
        'Fille Blonde',
        'Fille avec couronne',
        'Fille avec Tiara'
    ],
    '👳‍{{skin_tone}}‍♂️': ['Homme Portant un Turban'],
    '👳‍{{skin_tone}}‍♀️': ['Turban portatif féminin'],
    '👲{{skin_tone}}‍': [
        'Personne avec un chapeau crânien',
        'Homme asiatique',
        'Homme avec casquette chinoise',
        'Homme avec Gua Pi Mao'
    ],
    '🧕{{skin_tone}}‍': [
        'Femme avec foulard',
        'Hijab',
        'Personne avec foulard'
    ],
    '🤵{{skin_tone}}‍': [
        'Personne à Tuxedo',
        'Groom',
        'Homme en costume',
        'Homme à Tuxedo'
    ],
    '👰{{skin_tone}}‍': [
        'Personne avec le Voile',
        'Épouse',
        'Épouse de Voile'
    ],
    '👰‍{{skin_tone}}‍♂️': ['Homme avec voile'],
    '👰‍{{skin_tone}}‍♀️': ['Femme avec voile'],
    '👩‍{{skin_tone}}‍🍼': ['Bébé nourri par la femme'],
    '👨‍{{skin_tone}}‍🍼': ['Bébé nourrissant de l\'homme'],
    '🥷': ['Ninja'],
    '🤰{{skin_tone}}‍': [
        'Femme enceinte',
        'Grossesse',
        'Dame enceinte'
    ],
    '🤱{{skin_tone}}‍': [
        'Nourriture maternelle',
        'allaitement maternel'
    ],
    '🦸‍{{skin_tone}}‍♂️': ['Superhéros homme'],
    '🦸{{skin_tone}}‍‍♀️': ['Superhéros Femme'],
    '🦹{{skin_tone}}‍‍♂️': ['Supervilain homme'],
    '🦹{{skin_tone}}‍‍♀️': ['Supervilain femme'],
    '🧙{{skin_tone}}‍': [
        'Mage',
        'Sorcerer',
        'Sorcière',
        'Sorcière',
        'Assistant'
    ],
    '🧙{{skin_tone}}‍‍♀️': [
        'Mage Femme',
        'Sorcière',
        'Sorcière'
    ],
    '🧚{{skin_tone}}‍‍♂️': ['Fée de l\'homme'],
    '🧚‍{{skin_tone}}‍♀️': ['Fée féminine'],
    '🧛‍{{skin_tone}}‍♂️': [
        'Vampire homme',
        'Dracula'
    ],
    '🧛{{skin_tone}}‍‍♀️': ['Vampire féminin'],
    '🧜‍{{skin_tone}}‍♂️': ['Merman'],
    '🧜‍{{skin_tone}}‍♀️': ['Sirène'],
    '🧝‍{{skin_tone}}‍♂️': ['Elfe de l\'homme'],
    '🧝‍{{skin_tone}}‍♀️': ['Elfe Femme'],
    '🧞‍♂️': ['Man Genie'],
    '🧞‍♀️': ['Woman Genie'],
    '🕵️‍{{skin_tone}}‍♂️': [
        'Détective de l\'homme',
        'Œil privé',
        'Sleuth',
        'Espionnage',
        'Man Sleuth',
        'Sleuth ou Espion',
        '007',
        'Inspecteur'
    ],
    '🕵️‍{{skin_tone}}‍♀️': ['Détective Femme'],
    '💂{{skin_tone}}‍': [
        'Garde',
        'Garde britannique',
        'Garde des pieds'
    ],
    '💂{{skin_tone}}‍‍♀️': ['Garde Femme'],
    '👮{{skin_tone}}‍': [
        'Officier de Police',
        'Policier',
        'Police',
        'Policier',
        'Femme de police'
    ],
    '👮{{skin_tone}}‍‍♀️': ['Policier Femme'],
    '👨‍{{skin_tone}}‍🚒': ['Pompiers d\'homme'],
    '👩{{skin_tone}}‍‍🚒': ['Femme pompière'],
    '👨{{skin_tone}}‍‍✈️': ['Pilote Homme'],
    '👩{{skin_tone}}‍‍✈️': ['Pilote Femme'],
    '👨‍{{skin_tone}}‍💻': [
        'Homme Technologue',
        'Homme Technologue'
    ],
    '👩‍{{skin_tone}}‍💻': ['Femme technologue'],
    '👨‍{{skin_tone}}‍🎤': [
        'Chanteur d\'homme',
        'Bowie',
        'Aladdin Sane'
    ],
    '👩‍{{skin_tone}}‍🎤': ['Chanteur Femme'],
    '👨‍{{skin_tone}}‍🎨': ['Artiste homme'],
    '👩‍{{skin_tone}}‍🎨': ['Artiste féminine'],
    '👷{{skin_tone}}‍‍♂️': ['Employé de la construction d\'homme'],
    '👷{{skin_tone}}‍‍♀️': ['Travailleur de la construction féminine'],
    '👨{{skin_tone}}‍‍🚀': [
        'Astronaute homme',
        'Cosmonaute de l\'homme'
    ],
    '👩{{skin_tone}}‍‍🚀': [
        'Astronaute féminine',
        'Cosmonaute des Femmes'
    ],
    '👨{{skin_tone}}‍‍🔬': ['Homme Scientifique'],
    '👩{{skin_tone}}‍‍🔬': ['Femme scientifique'],
    '👨‍{{skin_tone}}‍💼': [
        'Employé du bureau',
        'PDG',
        'Entrepreneur'
    ],
    '👩‍{{skin_tone}}‍💼': [
        'Travailleur de bureau féminin',
        'Entrepreneur',
        'PDG'
    ],
    '👨‍{{skin_tone}}‍🏭': ['Employé de l\'usine de l\'homme'],
    '👩{{skin_tone}}‍‍🏭': ['Travailleur d\'usine féminine'],
    '👨{{skin_tone}}‍‍🔧': ['Homme mécanicien'],
    '👩{{skin_tone}}‍‍🔧': ['Mécanicien Femme'],
    '👨{{skin_tone}}‍‍🍳': [
        'Cuisine d\'Homme',
        'Chef d\'homme',
        'Chef mâle'
    ],
    '👩{{skin_tone}}‍‍🍳': [
        'Cuisine féminine',
        'Chef Femme',
        'Chef Femme'
    ],
    '👨{{skin_tone}}‍‍🌾': ['Homme agriculteur'],
    '👩{{skin_tone}}‍‍🌾': ['Fermier Femme'],
    '👨{{skin_tone}}‍‍⚖️': ['Juge de l\'homme'],
    '👩{{skin_tone}}‍‍⚖️': ['Juge Femme'],
    '👨{{skin_tone}}‍‍🏫': ['Professeur d\'homme'],
    '👩{{skin_tone}}‍‍🏫': ['Professeur de femme'],
    '👨‍{{skin_tone}}‍🎓': ['Etudiant de l\'homme'],
    '👩‍{{skin_tone}}‍🎓': ['Étudiant Femme'],
    '👨{{skin_tone}}‍‍⚕️': [
        'Travailleur de la santé humaine',
        'Médecin masculin',
        'Infirmière masculine'
    ],
    '👩‍{{skin_tone}}‍⚕️': [
        'Travailleur en santé féminine',
        'Médecin Femme',
        'Infirmière féminine'
    ],
    '🤷‍{{skin_tone}}‍♂️': ['Éclaboussures de l\'homme'],
    '🤷‍{{skin_tone}}‍♀️': ['Éclaboussures féminines'],
    '🤦‍{{skin_tone}}‍♂️': ['Homme Facepalming'],
    '🤦‍{{skin_tone}}‍♀️': ['Femme Facepalming'],
    '🙇{{skin_tone}}‍‍♂️': ['Arc d\'homme'],
    '🙇{{skin_tone}}‍‍♀️': ['Arc féminin'],
    '🧏‍{{skin_tone}}‍♂️': ['Homme Sourd'],
    '🧏{{skin_tone}}‍‍♀️': ['Femme Sourde'],
    '🙋{{skin_tone}}‍‍♂️': [
        'Main de l\'Homme en levant',
        'L\'homme heureux se lève une main'
    ],
    '🙋‍{{skin_tone}}‍♀️': [
        'Main de l\'élevage des femmes',
        'Joyeuse Femme à une main'
    ],
    '💁‍{{skin_tone}}‍♂️': [
        'Main à pourboire de l\'homme',
        'Ordinateur de bureau d\'information'
    ],
    '💁‍{{skin_tone}}‍♀️': [
        'Main Tipping Femme',
        'Bureau d\'information Femme'
    ],
    '🙆‍{{skin_tone}}‍♂️': ['Man Gesturing OK'],
    '🙆‍{{skin_tone}}‍♀️': ['Gesture Femme OK'],
    '🙅‍{{skin_tone}}‍♂️': ['N° de geste de l\'homme'],
    '🙅‍{{skin_tone}}‍♀️': ['N° de geste féminin'],
    '🙎‍{{skin_tone}}‍♂️': ['Poussage de l\'homme'],
    '🙎{{skin_tone}}‍‍♀️': ['Pousse de femme'],
    '🙍{{skin_tone}}‍‍♂️': ['Man Frowning'],
    '🙍{{skin_tone}}‍‍♀️': ['Femme fronçant'],
    '🧟‍‍♂️': ['Zombie de l\'homme'],
    '🧟‍‍♀️': ['Zombie Femme'],
    '🚶‍{{skin_tone}}‍♂️': ['Marche de l\'homme'],
    '🚶‍{{skin_tone}}‍♀️': ['Marche féminine'],
    '🧍{{skin_tone}}‍‍♂️': ['Homme debout'],
    '🧍{{skin_tone}}‍‍♀️': ['Femme debout'],
    '🧎‍{{skin_tone}}‍♂️': ['Homme à genoux'],
    '🧎{{skin_tone}}‍‍♀️': ['Femme à genoux'],
    '👨‍{{skin_tone}}‍🦯': ['Homme avec canne blanche'],
    '👩‍{{skin_tone}}‍🦯': ['Femme avec canne blanche'],
    '👨‍{{skin_tone}}‍🦼': ['Homme en fauteuil roulant motorisé'],
    '👩‍{{skin_tone}}‍🦼': ['Femme en fauteuil roulant motorisé'],
    '👨‍{{skin_tone}}‍🦽': ['Homme en fauteuil roulant manuel'],
    '👩‍{{skin_tone}}‍🦽': ['Femme en fauteuil roulant manuel'],
    '👯‍♂️': ['Hommes aux Oreilles de Lapin'],
    '👯‍♀️': ['Femmes aux lanières'],
    '🕴️‍{{skin_tone}}‍': [
        'Personne en costume lévitant',
        'Homme aéroglisseur',
        'Garçon de Rude',
        'Walt Jabsco',
        'Homme en costume d\'affaires lévitant'
    ],
    '🧖‍‍{{skin_tone}}‍♂️': [
        'Homme dans la salle à vapeur',
        'Homme au sauna'
    ],
    '🧖‍‍{{skin_tone}}‍♀️': [
        'Femme dans la salle à vapeur',
        'Femme au sauna'
    ],
    '👫‍{{skin_tone}}‍': [
        'Main tendue féminine et homme',
        'Couple hétérosexuel',
        'Couple droit',
        'Homme et Femme tenant les mains'
    ],
    '👭‍{{skin_tone}}‍': [
        'Couple Lesbien',
        'Main tendue féminine',
        'Deux femmes tenant la main'
    ],
    '👬‍{{skin_tone}}‍': [
        'Main tendue masculine',
        'Couple gay',
        'Deux mains tendues masculines'
    ],
    '👨‍👩‍👦': [
        'Famille avec la Mère, le Père et le Fils',
        'Famille: Homme, Femme, Garçon'
    ],
    '👨‍👩‍👧': [
        'Famille avec la Mère, le Père et la Fille',
        'Famille: Homme, Femme, Fille'
    ],
    '👨‍👩‍👧‍👦': [
        'Famille avec Mère, Père, Fils et Fille',
        'Famille: Homme, Femme, Fille, Garçon'
    ],
    '👨‍👩‍👦‍👦': [
        'Famille avec la mère, le père et les deux fils',
        'Famille: Homme, Femme, Garçon'
    ],
    '👨‍👩‍👧‍👧': [
        'Famille avec la mère, le père et les deux filles',
        'Famille: Homme, Femme, Fille'
    ],
    '👨‍👨‍👦': [
        'Famille avec deux pères et fils',
        'Famille: Homme, Homme, Garçon'
    ],
    '👨‍👨‍👧': [
        'Famille avec deux pères et filles',
        'Famille: Homme, homme, fille'
    ],
    '👨‍👨‍👧‍👦': [
        'Famille avec deux pères, fils et fille',
        'Famille: Homme, Homme, Fille, Garçon'
    ],
    '👨‍👨‍👦‍👦': [
        'Famille avec deux pères et deux fils',
        'Famille: Homme, homme, garçon'
    ],
    '👨‍👨‍👧‍👧': [
        'Famille avec deux pères et deux filles',
        'Famille: Homme, Homme, Fille'
    ],
    '👩‍👩‍👦': [
        'Famille avec deux mères et fils',
        'Famille: Femme, Femme, Garçon'
    ],
    '👩‍👩‍👧': [
        'Famille avec deux mères et filles',
        'Famille: Femme, Femme, Fille'
    ],
    '👩‍👩‍👧‍👦': [
        'Famille avec deux mères, fils et fille',
        'Famille: Femme, Femme, Fille, Garçon'
    ],
    '👩‍👩‍👦‍👦': [
        'Famille avec deux mères et deux fils',
        'Famille: Femme, Femme, Garçon'
    ],
    '👩‍👩‍👧‍👧': [
        'Famille avec deux mères et deux filles',
        'Famille: Femme, Femme, Fille, Fille'
    ],
    '👨‍👦': [
        'Famille avec le Père et le Fils',
        'Famille: Homme, Garçon'
    ],
    '👨‍👦‍👦': [
        'Famille: Homme, garçon',
        'Famille avec le père et les deux fils'
    ],
    '👨‍👧': [
        'Famille avec père et fille',
        'Famille: Homme, Fille'
    ],
    '👨‍👧‍👦': [
        'Famille avec Père, Fils et Fille',
        'Famille: Homme, Fille, Garçon'
    ],
    '👨‍👧‍👧': [
        'Famille avec le père et les deux filles',
        'Famille: Homme, Fille'
    ],
    '👩‍👦': [
        'Famille avec Mère et Fils',
        'Famille: Femme, Garçon'
    ],
    '👩‍👦‍👦': [
        'Famille avec mère et deux fils',
        'Famille: Femme, Garçon, Garçon'
    ],
    '👩‍👧': [
        'Famille avec mère et fille',
        'Famille: Femme, Fille'
    ],
    '👩‍👧‍👦': [
        'Famille avec Mère, Fils et Fille',
        'Famille: Femme, Fille, Garçon'
    ],
    '👩‍👧‍👧': [
        'Famille avec mère et deux filles',
        'Famille: Femme, Fille, Fille'
    ],
    '💑': [
        'Couple avec le cœur',
        'Couple Amour',
        'Couple amoureux',
        'Homme et Femme avec cœur'
    ],
    '👨‍❤️‍👨': [
        'Deux hommes avec cœur',
        'Couple avec Coeur: Homme, Homme'
    ],
    '👩‍❤️‍👩': [
        'Deux femmes au cœur',
        'Couple avec cœur : Femme, Femme'
    ],
    '💆{{skin_tone}}‍‍♂️': ['Massage de la tête de l\'homme'],
    '💆‍{{skin_tone}}‍♀️': ['Massage de la tête des femmes'],
    '💇{{skin_tone}}‍‍♂️': ['L\'homme se fait tailler des cheveux'],
    '💇{{skin_tone}}‍‍♀️': ['Coupe de cheveux féminine'],
    '🛌{{skin_tone}}‍': [
        'Hébergement de couchage',
        'Personne au lit',
        'Sommeil de personne'
    ]
};
/*animals*/
emojis_language[lang][4] = {
    '😺': [
        'Chat mouillant',
        ':)',
        'Chat Joyeux',
        'Chat souriant',
        'format@@0 Happy Cat Face',
        'Cat visage souriant avec bouche ouverte'
    ],
    '😸': [
        'Chat souriant aux yeux souriants',
        ':D',
        'Chat mouillant',
        'Chat Joyeux',
        'Visage de chat mouillant'
    ],
    '😹': [
        'Chat avec larmes de joie',
        ':\')',
        'Cat Larmes Joyeuses',
        'Chat riant'
    ],
    '😻': [
        'Chat souriant avec les yeux de cœur',
        '*.*',
        'Chat des yeux de cœur',
        'Chat Amoureux',
        'Cat souriant avec des yeux en forme de cœur'
    ],
    '😼': [
        'Chat avec Wry Smile',
        'Chat souriant',
        'Visage de chat rampant',
        'Visage de chat avec sourire de Wry'
    ],
    '😽': [
        'Chat embrassant',
        ':*',
        'Face de chat embrassant',
        'Face de chat baiser avec les yeux fermés'
    ],
    '🙀': [
        'Chat Weary',
        ':O',
        'Chat effrayé',
        'Chat hurlant',
        'Cri du visage du chat dans la peur',
        'Visage de chat portatif'
    ],
    '😿': [
        'Chat pleurant',
        ':\'(',
        'Chat triste',
        'Visage de chat pleurant'
    ],
    '😾': [
        'Chat Poussant',
        'Grumpy Cat',
        'Visage de chat en mouvement'
    ],
    '🙈': [
        'Singe',
        'Voir-No-Maléfique singe',
        'Mizaru',
        'Yeux recouverts de singes'
    ],
    '🙉': [
        'Singe',
        'Singe Hear-No-Evil',
        'Kikazaru',
        'Oreilles de Couverture des Singes'
    ],
    '🙊': [
        'Singe',
        'Speak-No-Evil Monkey',
        'Iwazaru',
        'Bouche de couverture singe',
        'Pas de parole'
    ],
    '🐵': [
        'Singe',
        'Face singe',
        'Tête de singe'
    ],
    '🐒': [
        'Singe',
        'Singe Cheeky'
    ],
    '🦍': ['Gorilla'],
    '🐶': [
        'Visage de chien',
        'Chien',
        'Chiot'
    ],
    '🐕': [
        'Chien',
        'Doggo',
        'Akita Inu'
    ],
    '🦮': [
        'Visant le chien des yeux',
        'Chien guide'
    ],
    '🐕‍🦺': ['Chien de service'],
    '🐩': [
        'Poufle',
        'Chien',
        'Pouille miniature',
        'Poufle standard',
        'Poodle de jouet'
    ],
    '🐺': [
        'Loup',
        'Visage de loup'
    ],
    '🦊': [
        'Renard',
        'Visage du renard'
    ],
    '🦝': ['Raton laveur'],
    '🐱': [
        'Visage de chat',
        ':3',
        'Chaton',
        'Chaton'
    ],
    '🐈': [
        'Chat',
        'Chat domestique',
        'Félix',
        'Maissecat'
    ],
    '🐈‍⬛': ['Chat Noir'],
    '🦁': [
        'Lion',
        'Visage du Lion'
    ],
    '🐯': [
        'Visage du tigre',
        'Tigre Mignon'
    ],
    '🐅': [
        'Tigre',
        'Tigre du Bengale'
    ],
    '🐆': [
        'Leopard',
        'Léopard africain',
        'Jaguar'
    ],
    '🐴': [
        'Visage de cheval',
        'Tête de cheval'
    ],
    '🐎': [
        'Cheval',
        'Cheval galopant',
        'Cheval de course'
    ],
    '🦄': [
        'Licorne',
        'Visage de licorne'
    ],
    '🦓': [
        'Zebra',
        'Visage zèbre'
    ],
    '🦌': [
        'Cerf',
        'Buck',
        'Renne',
        'Stag'
    ],
    '🐮': [
        'Visage de vache',
        'Vache',
        'Vache heureuse'
    ],
    '🐂': [
        'Oeuf',
        'Taureau',
        'Tauffage',
        'Océan',
        'Diriger'
    ],
    '🐃': [
        'Buffalo d\'eau',
        'Buffalo',
        'Buffal d\'Eau Domestique'
    ],
    '🐄': [
        'Vache',
        'Vache laitière'
    ],
    '🐷': [
        'Visage de cochon',
        'Cochon',
        'Tête de cochon'
    ],
    '🐖': [
        'Cochon',
        'Porc',
        'Sow'
    ],
    '🐗': [
        'Sanglier',
        'Guerrier',
        'Sanglier sauvage',
        'Cochon sauvage'
    ],
    '🐽': ['Nez de cochon'],
    '🐏': [
        'Ram',
        'Mouton'
    ],
    '🐑': [
        'Ewe',
        'Lamb',
        'Mouton'
    ],
    '🐐': ['Chèvre'],
    '🐪': [
        'Chameau',
        'Chameau arabe',
        'Camelle Dromedaire',
        'Camouflage 1 Bump'
    ],
    '🐫': [
        'Camouflage à deux sauts',
        'Camel asiatique',
        'Camelle bactrienne'
    ],
    '🦙': [
        'Llama',
        'Alpaca'
    ],
    '🦒': [
        'Giraffe',
        'Giraffe Face'
    ],
    '🐘': ['Éléphant'],
    '🦏': [
        'Rhinoceros',
        'Rhino'
    ],
    '🦛': [
        'Hippopotamus',
        'Hippo'
    ],
    '🐭': [
        'Visage de la souris',
        'Souris'
    ],
    '🐁': [
        'Souris',
        'Dormir',
        'Tentant',
        'Souris'
    ],
    '🐀': [
        'Rat',
        'Tentant'
    ],
    '🐹': [
        'Hamster',
        'Visage de hamster'
    ],
    '🐰': [
        'Visage de Lapin',
        'lapin de Pâques'
    ],
    '🐇': [
        'Lapin',
        'Bunny',
        'Lapin de lapin'
    ],
    '🐿️': [
        'Chipmunk',
        'Écureuil'
    ],
    '🦔': ['Hedgehog'],
    '🦇': [
        'Chauve-souris',
        'Batman'
    ],
    '🐻': [
        'Ours',
        'Visage d\'ours',
        'Ours en peluche'
    ],
    '🐨': [
        'Koala',
        'Visage de Koala'
    ],
    '🐼': [
        'Panda',
        'Visage de panda'
    ],
    '🦘': [
        'Kangourou',
        'Famille Roo'
    ],
    '🦡': ['blaireau'],
    '🦃': [
        'La Turquie',
        'Dinde d\'Action de grâce',
        'Turquie sauvage'
    ],
    '🐔': [
        'Poulet',
        'Hen'
    ],
    '🐓': [
        'Cock',
        'Rooster',
        'Cockerel'
    ],
    '🐣': [
        'Poulet d\'éclosion',
        'Bébé Poulet',
        'Éclosion de poule'
    ],
    '🐤': [
        'Poulet de bébé',
        'Oiseau jaune'
    ],
    '🐥': [
        'Poulet de bébé frontal',
        'Poulet de bébé',
        'Poulet debout'
    ],
    '🐦': ['Oiseau'],
    '🐧': ['Penguin'],
    '🕊️': [
        'Colombe',
        'Colombe de la paix'
    ],
    '🦅': [
        'Aigle',
        'Aigle Chauve'
    ],
    '🦆': ['Canard'],
    '🦢': ['Cygne'],
    '🦉': ['Hibou'],
    '🦚': ['Paon'],
    '🦜': ['Perroquet'],
    '🐸': [
        'Grenouille',
        'Trappe',
        'Face de grenouille'
    ],
    '🐊': [
        'Crocodile',
        'Alligateur',
        'Croc'
    ],
    '🐢': [
        'Tortue',
        'Tortoise'
    ],
    '🦎': [
        'Lézard',
        'Gecko'
    ],
    '🐍': [
        'Serpent',
        'Serpent'
    ],
    '🐲': [
        'Visage du Dragon',
        'Tête de dragon'
    ],
    '🐉': ['Dragon'],
    '🦕': [
        'Sauropode',
        'Brachiosaurus',
        'Brontosaurus',
        'Dinosaur'
    ],
    '🦖': [
        'T-Rex',
        'Tyrannosaurus Rex'
    ],
    '🐳': [
        'Baleine époustouflante',
        'Baleine Mignonne'
    ],
    '🐋': ['Baleine'],
    '🐬': ['Dauphin'],
    '🐟': [
        'Poisson',
        'Poisson d\'eau douce'
    ],
    '🐠': [
        'Poisson tropical',
        'Poisson',
        'Poisson jaune-bleu'
    ],
    '🐡': [
        'Poisson-soufflet',
        'Fugu',
        'Poisson-globe'
    ],
    '🦈': [
        'Requin',
        'Grand requin blanc'
    ],
    '🐙': ['Pieuvre'],
    '🐚': [
        'Coquille spirale',
        'Enfer de mer',
        'Coquille'
    ],
    '🐌': [
        'Songes',
        'Limace',
        'Songle de jardin'
    ],
    '🦋': ['Papillon'],
    '🐛': [
        'Bug',
        'Chariot',
        'Insecte'
    ],
    '🐜': [
        'Ant',
        'Bug',
        'Insecte'
    ],
    '🐝': [
        'Abeille de miel',
        'Abeille',
        'Bumblebee'
    ],
    '🐞': [
        'Scarabée dame',
        'Ladybird',
        'Ladybug',
        'Bug'
    ],
    '🦗': [
        'Cricket',
        'Grasshopper'
    ],
    '🕷️': ['Araignée'],
    '🦂': ['Scorpion'],
    '🦟': ['moustique'],
    '🦀': ['Crabe'],
    '🦞': ['Homard'],
    '🦐': [
        'Crevettes',
        'Prawn'
    ],
    '🦑': ['Calamar'],
    '🦧': ['Orangutan'],
    '🦥': ['Paresseux'],
    '🦦': ['Otter'],
    '🦨': ['Skunk'],
    '🦬': ['Bison'],
    '🦣': ['Mammouth'],
    '🦫': ['Castor'],
    '🐻‍❄️': ['Ours polaire'],
    '🦤': ['Dodo'],
    '🦭': ['Sceau'],
    '🪲': ['Scarabée'],
    '🪳': ['Rugissement de coqs'],
    '🪰': ['Vol'],
    '🪱': ['Ver'],
    '🦩': ['Flamant']
};
/*symbols*/
emojis_language[lang][5] = {
    '💘': [
        'Coeur avec Flèche',
        'Flèche de Cupidon',
        'Lovestruck'
    ],
    '💝': [
        'Coeur avec ruban',
        'Boîte de chocolat',
        'Coffret Cadeau',
        'Cadeau de coeur'
    ],
    '💖': [
        'Sparkling Heart',
        'Coeur étincelant',
        'Coeur étincelant',
        'Coeur des étoiles'
    ],
    '💗': [
        'Cœur grandissant',
        'Cœur multiple',
        'Triple Coeur'
    ],
    '💓': [
        'Vaincre le cœur',
        'Alarme cardiaque',
        'Heartbeat',
        'Coeur de Wifi'
    ],
    '💞': [
        'Coeurs tournants',
        'Deux cœurs'
    ],
    '💕': [
        'Deux cœurs',
        'Petits cœurs',
        'Deux cœurs roses'
    ],
    '💟': ['Décoration Coeur'],
    '❣️': [
        'Exclamation cardiaque',
        'Cœur au-dessus du point',
        'Coeur Rouge comme Marque d\'Exclamation',
        'Ornement de Marque d\'Exclamation de Coeur Lourd'
    ],
    '💔': [
        'Cœur cassé',
        'Cœur de rupture',
        'Cœur brisé',
        'Cœur cassé'
    ],
    '❤️': [
        'Je t\'aime',
        'Coeur Rouge',
        'Coeur',
        'Coeur d\'amour',
        'Coeur Rouge'
    ],
    '🧡': [
        'Coeur Orange',
        'Je t\'aime'
    ],
    '💛': [
        'Cœur jaune',
        'Snapchat #1 BF',
        'Coeur d\'Or',
        'Je t\'aime'
    ],
    '💚': [
        'Coeur Vert',
        'Cœur jaloux',
        'Je t\'aime'
    ],
    '💙': [
        'Cœur Bleu',
        'Je t\'aime'
    ],
    '💜': [
        'Coeur Violet',
        'Je t\'aime'
    ],
    '🖤': [
        'Coeur Noir',
        'Coeur Sombre',
        'Je t\'aime'
    ],
    '🤍': [
        'Cœur blanc',
        'Je t\'aime'
    ],
    '🤎': [
        'Coeur marron',
        'Je t\'aime'
    ],
    '💯': [
        'Cent Points',
        '100',
        '%',
        'Garder 100',
        'Score parfait',
        'Symbole de cent points'
    ],
    '💬': [
        'Ballon vocal',
        'Bulle de chat',
        'Bulle vocale'
    ],
    '💭': [
        'Ballon de pensée',
        'Bulle de pensée',
        'Bulle de pensée'
    ],
    '💤': [
        'ZZ',
        'Temps de couchage',
        'Nageoire',
        'Sommeil',
        'Signe de sommeil',
        'Symbole de sommeil'
    ],
    '♠️': [
        'Costume de paille',
        'Spades',
        'Black Spade Suit'
    ],
    '♥️': [
        'Costume Coeur',
        'Carte avec cœur',
        'Coeurs',
        'Costume Coeur Noir'
    ],
    '♦️': [
        'Costume en Diamant',
        'Diamants',
        'Costume en Diamant Noir'
    ],
    '♣️': [
        'Costume de club',
        'Clubs',
        'Costume de Club Noir'
    ],
    '🏧': [
        'Panneau ATM',
        'DAB',
        'Telleer automatique'
    ],
    '🎵': [
        'Note musicale',
        'Paire de huitième Notes à faisceau',
        'Paire de quavers à faisceaux',
        'Note de musique'
    ],
    '🎶': [
        'Notes musicales',
        'Musique',
        'Notes de musique',
        'Chantage',
        'Notes musicales multiples'
    ],
    '🔔': [
        'Cloche',
        'Cloche de la Liberté',
        'Ringer',
        'Cloche de mariage'
    ],
    '🔕': [
        'Cloche avec Slash',
        'Notifications',
        'Sonnerie désactivée',
        'Cloche en sourdine',
        'Cloche avec trait d\'annulation'
    ],
    '🔈': [
        'Volume faible du haut-parleur',
        'Volume',
        'Haut-parleur'
    ],
    '🔉': [
        'Volume moyen du haut-parleur',
        'Réduire le volume',
        'Haut-parleur avec volume moyen',
        'Haut-parleur avec une seule vague sonore'
    ],
    '🔊': [
        'Haut volume du haut-parleur',
        'Augmenter le volume',
        'Haut-parleur avec trois vagues sonores'
    ],
    '🔇': [
        'Haut-parleur muet',
        'Couper le volume',
        'Haut-parleur avec trait d\'annulation'
    ],
    '⛔': ['Aucune entrée'],
    '🚫': [
        'Interdit',
        'Banni',
        'antislash cercle',
        'Non',
        'Cercle rouge croisé',
        'Restreint',
        'Pas de panneau d\'entrée'
    ],
    '🚳': [
        'Pas de vélos',
        'Pas de pancarte cyclable'
    ],
    '🚭': [
        'Pas de fumée',
        'Symbole de non-fumeur'
    ],
    '🚯': [
        'Pas de débordement',
        'Ne pas utiliser le symbole de la terreur'
    ],
    '🚱': [
        'Eau Non Potable',
        'Pas d\'eau',
        'Pas d\'eau potable',
        'Symbole de l\'eau non potable'
    ],
    '🚷': [
        'Pas de piétons',
        'Aucune personne',
        'Pas de marche'
    ],
    '🔞': [
        'Personne de moins de dix-huit ans',
        'NSFW',
        'Symbole de moins de dix-huit ans',
        '18'
    ],
    '📵': [
        'Aucun téléphone mobile',
        'Aucun téléphone cellulaire',
        'Aucun téléphone',
        'Aucun Smartphone'
    ],
    '☢️': [
        'Radioactive',
        'Symbole de rayonnement international',
        'Nucléaire',
        'Panneau radioactif'
    ],
    '☣️': [
        'Risque biologique',
        'Panneau "Biohazard"'
    ],
    '⬆️': [
        'Flèche vers le haut',
        'Flèche pointant vers le haut',
        'Flèche noire vers le haut'
    ],
    '↗️': [
        'Flèche en haut à droite',
        'Flèche diagonale vers la droite',
        'Flèche nord-est'
    ],
    '➡️': [
        'Flèche droite',
        'Flèche à droite',
        'Flèche noire vers la droite'
    ],
    '↘️': [
        'Flèche en bas à droite',
        'Flèche diagonale vers le bas à droite',
        'Flèche sud-est'
    ],
    '⬇️': [
        'Flèche bas',
        'Pointage de flèches vers le bas',
        'Flèche noire vers le bas'
    ],
    '↙️': [
        'Flèche en bas à gauche',
        'Flèche diagonale vers le bas gauche',
        'Flèche Sud-Ouest'
    ],
    '⬅️': [
        'Flèche gauche',
        'Flèche à gauche',
        'Flèche noire à gauche'
    ],
    '↖️': [
        'Flèche en haut à gauche',
        'Flèche diagonale vers la gauche',
        'Flèche Nord-Ouest'
    ],
    '↕️': [
        'Flèche vers le haut',
        'Flèches verticales',
        'Flèche vers le haut'
    ],
    '↔️': [
        'Flèche gauche-droite',
        'Flèches horizontales',
        'Flèches latérales',
        'Flèche gauche droite'
    ],
    '↩️': [
        'Flèche droite courbe à gauche',
        'Répondre par e-mail',
        'Flèche courbée gauche',
        'Flèche gauche avec crochet'
    ],
    '↪️': [
        'Flèche gauche courbée à droite',
        'Envoi d\'email',
        'Flèche courbée droite',
        'Flèche à droite avec crochet'
    ],
    '⤴️': [
        'Flèche droite vers le haut',
        'Flèche pointant vers la droite puis courbant vers le haut'
    ],
    '⤵️': [
        'Flèche droite Courbée vers le bas',
        'Flèche incurvée vers le bas',
        'Flèche pointant vers la droite puis courbant vers le bas'
    ],
    '🔃': [
        'Flèches verticales dans le sens horaire',
        'Sens horaire vers le bas et vers le haut ouvrir des flèches circulaires'
    ],
    '🔄': [
        'Bouton Flèches dans le sens inverse des aiguilles d\'une montre',
        'Rafraîchir',
        'Faire pivoter',
        'Commuter',
        'Flèches dans le sens anti-horaire',
        'Flèches circulaires ouvertes vers le bas et vers le haut'
    ],
    '🔙': [
        'Flèche arrière',
        'Précédent',
        'Retour avec flèche gauche au dessus'
    ],
    '🔚': [
        'Fin de la Flèche',
        'Fin',
        'Terminer avec la flèche gauche au dessus'
    ],
    '🔛': [
        'Allumé! Flèche',
        'Activé',
        'Activé avec le point d\'exclamation avec la flèche droite gauche ci-dessus'
    ],
    '🔜': [
        'Bientôt Flèche',
        'Bientôt',
        'Bientôt avec des flèches à droite au dessus'
    ],
    '🔝': [
        'Flèche du haut',
        'En haut',
        'Top avec la flèche vers le haut au-dessus'
    ],
    '🛐': [
        'Lieu de culte',
        'Bâtiment religieux'
    ],
    '⚛️': ['Symbole Atom'],
    '🕉️': [
        'Symbole Om',
        'Oui',
        'Aumkara',
        'Omkara',
        'Pranava'
    ],
    '✡️': [
        'Etoile de David',
        'Magen David'
    ],
    '☸️': [
        'Roue du Dharma',
        'Heaume'
    ],
    '☯️': ['Yin Yang'],
    '✝️': [
        'Croix latine',
        'Croix chrétienne'
    ],
    '☦️': ['Croix orthodoxe'],
    '☪️': ['Étoile et Croissant de Lune'],
    '☮️': [
        'Symbole de la paix',
        'Panneau de Paix'
    ],
    '🕎': [
        'Menorah',
        'Candelabrum',
        'Bougies',
        'Chanukiah',
        'Menorah',
        'Menorah avec neuf branches'
    ],
    '🔯': [
        'Étoile pointillée à six pointes',
        'Six étoiles pointues avec point central'
    ],
    '♈': [
        'Baies',
        'Zodiaque'
    ],
    '♉': [
        'Taurus',
        'Zodiaque'
    ],
    '♊': [
        'Gemini',
        'Zodiaque'
    ],
    '♋': [
        'Cancer',
        'Zodiaque'
    ],
    '♌': [
        'Leo',
        'Zodiaque'
    ],
    '♍': [
        'Vierge',
        'Zodiaque'
    ],
    '♎': [
        'Librairie',
        'Zodiaque'
    ],
    '♏': [
        'Scorpion',
        'Scorpion',
        'Zodiaque'
    ],
    '♐': [
        'Sagittarius',
        'Zodiaque'
    ],
    '♑': [
        'Capricorne',
        'Zodiaque'
    ],
    '♒': [
        'Verseau',
        'Zodiaque'
    ],
    '♓': [
        'Pisces',
        'Zodiaque'
    ],
    '⛎': [
        'Ophiuchus',
        'Zodiaque'
    ],
    '⏭️': [
        'Triangle double à pointe droite noir avec barre verticale',
        'Ignorer le symbole avant',
        'Bouton de la piste suivante'
    ],
    '⏮️': [
        'Triangle double à gauche noir avec barre verticale',
        'Revenir en arrière',
        'Bouton de la dernière piste',
        'Piste précédente'
    ],
    '⏯️': [
        'Triangle à pointe droite noir avec barre verticale double',
        'Bouton Lecture ou Pause',
        'Jouer à Symbole',
        'Mettre en pause le symbole'
    ],
    '🔀': [
        'Bouton Lecture aléatoire des traces',
        'Flèches torsadées vers la droite'
    ],
    '🔁': [
        'Bouton de répétition',
        'Symbole de boucle',
        'Retweet',
        'Ouverture de flèches circulaires dans le sens horaire à droite et à gauche'
    ],
    '🔂': [
        'Répéter un seul bouton',
        'Cercle de flèches avec numéro 1',
        'Boucler le symbole Une fois',
        'Symbole de répétition d\'une piste unique',
        'Flèches ouvertes circulaires dans le sens horaire à droite et à gauche avec une superposition encerclée'
    ],
    '▶️': [
        'Bouton de lecture',
        'Triangle droit',
        'Triangle pointant vers la droite',
        'Triangle noir à droite'
    ],
    '◀️': [
        'Bouton inversé',
        'Triangle gauche',
        'Triangle pointant à gauche',
        'Triangle noir à gauche'
    ],
    '🔼': [
        'Bouton vers le haut',
        'Triangle Haut',
        'Triangle vers le haut',
        'Petit triangle rouge pointant vers le haut'
    ],
    '🔽': [
        'Bouton vers le bas',
        'Triangle Bas',
        'Triangle de pointage inférieur',
        'Petit triangle rouge vers le bas'
    ],
    '⏩': [
        'Bouton Avance Rapide',
        'Avance rapide',
        'Symbole d\'Avance Rapide',
        'Triangle double pointant vers la droite noir'
    ],
    '⏪': [
        'Fast Reverse Button',
        'Rewind',
        'Revenir en arrière',
        'Triangle double à gauche noir'
    ],
    '⏫': [
        'Bouton rapide vers le haut',
        'Triangle double vers le haut',
        'Triangle double à pointe noire'
    ],
    '⏬': [
        'Bouton rapide vers le bas',
        'Triangle double vers le bas',
        'Triangle double du point noir vers le bas'
    ],
    '⏹️': [
        'Bouton d\'arrêt',
        'Arrêter',
        'Arrêt du symbole',
        'Carré noir pour Stop'
    ],
    '⏏️': [
        'Bouton d\'éjection',
        'Eject Symbol'
    ],
    '🎦': [
        'Cinéma',
        'Écran de cinéma',
        'Films',
        'Cinéma',
        'Symbole du cinéma',
        'Enregistrement en cours'
    ],
    '📶': [
        'Barres d\'antenne',
        'Barres de réception',
        'Force du signal',
        'Stairs',
        'Symbole de force du signal',
        'Antenne avec barres'
    ],
    '✖️': [
        'Multiplier',
        'Symbole de la multiplication',
        'Multiplication lourde X'
    ],
    '➕': [
        'Plus',
        'Symbole Plus',
        'Panneau lourd Plus'
    ],
    '➖': [
        'Minus',
        'Minus Symbol',
        'Panneau lourd Minus'
    ],
    '➗': [
        'Diviser',
        'Symbole de division',
        'Panneau de division lourde'
    ],
    '♾️': [
        'Infini',
        'Panneau en papier permanent'
    ],
    '‼️': [
        'Marque de double Exclamation',
        'Marque de double Exclamation Rouge'
    ],
    '⁉️': [
        'Point de question d\'exclamation',
        'Marque d\'exclamation rouge et point d\'interrogation'
    ],
    '❓': [
        'Marque de questions',
        'Marque de question rouge',
        'Ornement de la note de question noire'
    ],
    '❔': [
        'Point d\'interrogation blanc',
        'Point d\'interrogation blanc',
        'Ornement de la marque blanche de question'
    ],
    '❗': [
        'Marque d\'Exclamation',
        'Marque d\'Exclamation Rouge',
        'Symbole de marque d\'exclamation lourde'
    ],
    '❕': [
        'Marque d\'Exclamation Blanche',
        'Ornement de Marque d\'Exclamation Blanche'
    ],
    '⭕': [
        'Cercle rouge creux',
        'Cercle',
        'Corriger',
        'Cercle rouge',
        'Marque du cercle',
        'Grand cercle lourd'
    ],
    '☑️': [
        'Cocher la case avec la case de contrôle',
        'V',
        'Case à cocher',
        'Cocher dans la case à cocher',
        'Boîte aux urnes avec contrôle'
    ],
    '✔️': [
        'Cocher la note',
        'V',
        'Contrôler',
        'Cocher',
        'Checkmark',
        'Marque de contrôle lourde'
    ],
    '❌': [
        'Marque de Croix',
        'Croix',
        'X'
    ],
    '✅': [
        'Bouton de la note',
        'V',
        'Marque de chèque vert',
        'Coche Verte',
        'Cocher la note',
        'Marque de contrôle blanche lourde'
    ],
    '❎': [
        'Bouton croix',
        'Croix',
        'X',
        'Marque X',
        'Marque de croix carrée négative'
    ],
    '#️⃣': [
        'Panneau numérique Keycap',
        '#',
        'Clé de hachage',
        'Hashtag',
        'Octothorpe',
        'Clé livrée',
        'Panneau numérique'
    ],
    '*️⃣': [
        'Keycap Asterisk',
        'Casquette de clés Étoile'
    ],
    '0️⃣': [
        'Nombre 0',
        'Keycap Digit Zero'
    ],
    '1️⃣': [
        'Numéro 1',
        'Keycap Digit One'
    ],
    '2️⃣': [
        'Numéro 2',
        'Keycap Digit Two'
    ],
    '3️⃣': [
        'Numéro 3',
        'Keycap Digit Three'
    ],
    '4️⃣': [
        'Numéro 4',
        'Chiffre Clavier 4'
    ],
    '5️⃣': [
        'Numéro 5',
        'Chiffre Cinq Keycap'
    ],
    '6️⃣': [
        'Numéro 6',
        'Chiffre Clavier Six'
    ],
    '7️⃣': [
        'Numéro 7',
        'Keycap Digit Seven'
    ],
    '8️⃣': [
        'N° 8',
        'Numéroter Huitième Numérotation'
    ],
    '9️⃣': [
        'N° 09',
        'Keycap Digit Nine'
    ],
    '🔟': [
        'Numéro 10',
        'Keycap: 10',
        '10',
        'Keycap Digit Ten'
    ],
    '🔠': [
        'Input Latin Uppercase',
        'ABCD',
        'Majuscule',
        'Symbole d\'entrée pour lettres majuscules',
        'Symbole d\'entrée pour les lettres de la majuscule latine'
    ],
    '🔡': [
        'Input Latin Lowercase',
        'abcd',
        'minuscule',
        'Symbole d\'entrée pour lettres minuscules',
        'Symbole d\'entrée pour les petites lettres latines'
    ],
    '🔢': [
        'Input Numbers',
        '1234',
        'Numéros',
        'Numeric Input',
        'Symbole d\'entrée pour les nombres'
    ],
    '🔣': [
        'Symbole d\'entrée pour les Symboles',
        'Symboles',
        'Entrée de symbole'
    ],
    '🔤': [
        'Input Latin Letters',
        'abc',
        'Alphabet',
        'Lettres',
        'Symbole d\'entrée pour les lettres',
        'Symbole d\'entrée pour les lettres latines'
    ],
    '🅰️': [
        'Un bouton (Blood Type)',
        'Type de sang A',
        'Lettre A',
        'Lettre A de la capitale latine carrée négative'
    ],
    '🆎': [
        'Bouton AB (Blood Type)',
        'Lettres AB',
        'Type de sang AB',
        'Carré négatif AB'
    ],
    '🅱️': [
        'Bouton B (Blood Type)',
        'Lettre B',
        'Type de sang B',
        'Lettre capitale latine carrée négative B'
    ],
    '🆑': [
        'Bouton CL',
        'Nettoyer',
        'Bouton de nettoyage',
        'Panneau CL',
        'format@@0 Squared CL',
        'Lettres CL'
    ],
    '🆒': [
        'Bouton de refroidissement',
        'Carré froid',
        'Signe cool',
        'Cool carré'
    ],
    '🆓': [
        'Bouton gratuit',
        'Gratuit',
        'Panneau Libre',
        'Carré libre'
    ],
    'ℹ️': [
        'Information',
        'Infos',
        'minuscule I',
        'Informations touristiques',
        'Source des informations'
    ],
    '🆔': [
        'Bouton ID',
        'ID',
        'Identification',
        'Panneau d\'identification',
        'ID carré'
    ],
    '🆕': [
        'Nouveau bouton',
        'Nouveau',
        'Nouveau panneau',
        'Nouveau carré'
    ],
    '🅾️': [
        'Bouton O (Blood Type)',
        'Type de sang O',
        '0',
        'Lettre capitale latine carrée négative',
        'Lettre O'
    ],
    '🆗': [
        'Bouton OK',
        'OK',
        'Ok',
        'Carré',
        'Panneau OK',
        'Carré OK'
    ],
    '🅿️': [
        'Bouton P',
        'Panneau de stationnement',
        'Lettre capitale latine carrée négative',
        'Lettre P'
    ],
    '🆘': [
        'Bouton SOS',
        'Signal de détresse',
        'Urgence',
        'SOS',
        'Panneau SOS',
        'SOS carré',
        'Aide'
    ],
    '🆙': [
        'Haut! Bouton',
        'Niveau supérieur',
        'Monter',
        'Inscription haut',
        'Carré vers le haut avec un point d\'exclamation'
    ],
    '🆚': [
        'Bouton contre',
        'Carré contre'
    ],
    '🆖': [
        'Bouton NG',
        'NG carré',
        'Pas de bon signe',
        'Pas de bien',
        'NG',
        'Blooper'
    ],
    '🔴': [
        'Cercle rouge',
        'Grand cercle rouge'
    ],
    '🟠': [
        'Cercle orange',
        'Grand cercle orange'
    ],
    '🟡': [
        'Cercle jaune',
        'Grand cercle jaune'
    ],
    '🟢': [
        'Cercle vert',
        'Grand cercle vert'
    ],
    '🔵': [
        'Cercle Bleu',
        'Grand cercle bleu'
    ],
    '🟣': [
        'Cercle violet',
        'Grand cercle violet'
    ],
    '🟤': [
        'Cercle marron',
        'Grand cercle marron'
    ],
    '⚫': [
        'Cercle Noir',
        'Cercle noir moyen'
    ],
    '⚪': [
        'Cercle blanc',
        'Cercle blanc moyen'
    ],
    '🟥': [
        'Carré rouge',
        'Grand carré rouge'
    ],
    '🟧': [
        'Carré orange',
        'Grand Carré Orange'
    ],
    '🟨': [
        'Carré jaune',
        'Grand carré jaune'
    ],
    '🟩': [
        'Carré vert',
        'Grand Carré Vert'
    ],
    '🟦': [
        'Carré Bleu',
        'Grand Carré Bleu'
    ],
    '🟪': [
        'Carré violet',
        'Grand carré pourpre'
    ],
    '🟫': [
        'Carré marron',
        'Grand carré marron'
    ],
    '⬛': ['Grand carré noir'],
    '⬜': ['Grand carré blanc'],
    '◼️': ['Carré moyen noir'],
    '◻️': ['Carré moyen blanc'],
    '◾': ['Petit carré noir et moyen'],
    '◽': ['Petit carré blanc et moyen'],
    '▪️': ['Petit Carré Noir'],
    '▫️': ['Petit carré blanc'],
    '💠': [
        'Forme en diamant avec un point à l\'intérieur',
        'La mignonne',
        'Fleur de Diamant',
        'Kawaii'
    ],
    '🔶': ['Grand Diamant Orange'],
    '🔷': ['Grand Diamant Bleu'],
    '🔸': ['Petit Diamant Orange'],
    '🔹': ['Petit Diamant Bleu'],
    '🔺': [
        'Triangle rouge pointé vers le haut',
        'Triangle rouge vers le haut'
    ],
    '🔻': [
        'Triangle rouge pointé vers le bas',
        'Triangle rouge vers le bas'
    ],
    '🔳': ['Bouton du carré blanc'],
    '🔲': ['Bouton du carré noir'],
    '©️': [
        'Droit d\'auteur',
        'Panneau de droit d\'auteur'
    ],
    '®️': [
        'Enregistré',
        'Panneau Enregistré'
    ],
    '™️': [
        'Marque commerciale',
        'MT',
        'Marque commerciale',
        'Panneau de Marque Commerciale'
    ],
    '🛅': [
        'Bagages restants',
        'Sac avec clé',
        'Suitcase verrouillée'
    ],
    '🛄': ['Revendication de bagages'],
    '🛃': ['Douanes'],
    '🛂': [
        'Contrôle du passeport',
        'Contrôle des bordures'
    ],
    '🚰': [
        'Symbole de l\'eau potable',
        'Soif',
        'Assoiffé',
        'Robinet d\'eau',
        'Faucet d\'eau'
    ],
    '🇦': [
        'Lettre A',
        'Lettre A de symbole de l\'indicateur régional'
    ],
    '🇧': [
        'Lettre B',
        'Lettre de symbole de l\'indicateur régional B'
    ],
    '🇨': [
        'Lettre C',
        'Lettre C de symbole de l\'indicateur régional'
    ],
    '🇩': [
        'Lettre D',
        'Lettre D de symbole de l\'indicateur régional'
    ],
    '🇪': [
        'Lettre E',
        'Lettre du symbole de l\'indicateur régional E'
    ],
    '🇫': [
        'Lettre F',
        'Lettre F de symbole de l\'indicateur régional'
    ],
    '🇬': [
        'Lettre G',
        'Lettre G du symbole de l\'indicateur régional'
    ],
    '🇭': [
        'Lettre H',
        'Lettre du symbole de l\'indicateur régional H'
    ],
    '🇮': [
        'Lettre I',
        'Lettre du symbole de l\'indicateur régional I'
    ],
    '🇯': [
        'Lettre J',
        'Lettre de symbole de l\'indicateur régional J'
    ],
    '🇰': [
        'Lettre K',
        'Lettre K de symbole de l\'indicateur régional'
    ],
    '🇱': [
        'Lettre L',
        'Lettre du symbole de l\'indicateur régional L'
    ],
    '🇲': [
        'Lettre M',
        'Lettre M du symbole de l\'indicateur régional'
    ],
    '🇳': [
        'Lettre N',
        'Lettre du symbole de l\'indicateur régional N'
    ],
    '🇴': [
        'Lettre O',
        'Lettre O du symbole de l\'indicateur régional'
    ],
    '🇵': [
        'Lettre P',
        'Lettre P de symbole de l\'indicateur régional'
    ],
    '🇶': [
        'Lettre Q',
        'Lettre Q de symbole de l\'indicateur régional'
    ],
    '🇷': [
        'Lettre R',
        'Lettre R du symbole de l\'indicateur régional'
    ],
    '🇸': [
        'Lettre S',
        'Lettre S de symbole de l\'indicateur régional'
    ],
    '🇹': [
        'Lettre T',
        'Lettre T de symbole de l\'indicateur régional'
    ],
    '🇺': [
        'Lettre U',
        'Lettre U de symbole de l\'indicateur régional'
    ],
    '🇻': [
        'Lettre V',
        'Lettre du symbole de l\'indicateur régional V'
    ],
    '🇼': [
        'Lettre W',
        'Lettre W du symbole de l\'indicateur régional'
    ],
    '🇽': [
        'Lettre X',
        'Lettre du symbole de l\'indicateur régional X'
    ],
    '🇾': [
        'Lettre Y',
        'Lettre du symbole de l\'indicateur régional Y'
    ],
    '🇿': [
        'Lettre Z',
        'Lettre du symbole de l\'indicateur régional Z'
    ],
    '💌': [
        'Lettre d\'amour',
        'Enveloppe Coeur',
        'Flirtmessage'
    ],
    '🚮': [
        'Mettre le Litter dans son Symbole de Place',
        'Signe de la corbeille',
        'Mettre le Litter dans la Corbeille',
        'Personne avec la corbeille'
    ],
    '♿': [
        'Symbole du fauteuil roulant',
        'Salle de bains accessible'
    ],
    '🚹': [
        'Symbole des hommes',
        'Symbole des hommes',
        'Toilet pour hommes',
        'Restroom masculin',
        'WC masculin'
    ],
    '🚺': [
        'Symbole de la femme',
        'Symbole de la femme',
        'Toilette pour femmes',
        'Restroom féminin',
        'Cave de Famille'
    ],
    '🚻': [
        'Restructuration',
        'Panneau de la salle de bains',
        'Panneau de toilette',
        'Panneau WC'
    ],
    '🚼': [
        'Symbole du bébé',
        'Poste de changement de Bébé',
        'Symbole du changement de bébé',
        'Infirmière'
    ],
    '🚾': [
        'Armoire à eau',
        'Toiles',
        'WC de toilette'
    ],
    '⚠️': [
        'Panneau d\'avertissement',
        'Symbole d\'alerte'
    ],
    '🚸': [
        'Enfants en traversant',
        'Croisement des enfants',
        'Croisement de l\'école'
    ],
    '🔅': [
        'Symbole de faible luminosité',
        'Bouton d\'atténuation',
        'Diminuer la luminosité'
    ],
    '🔆': [
        'Symbole de haute luminosité',
        'Augmenter la luminosité',
        'Bouton lumineux'
    ],
    '📳': [
        'Mode vibreur',
        'Coeur du téléphone',
        'Mode silencieux'
    ],
    '📴': ['Téléphone mobile éteint'],
    '〰️': [
        'Vague',
        'Tiret Vageux'
    ],
    '💱': ['Échange de devises'],
    '💲': [
        'Panneau en dollar lourd',
        'Dollar',
        'Panneau Dollar'
    ],
    '♻️': [
        'Symbole de recyclage noir universel',
        'Symbole de recyclage',
        'Logo de recyclage'
    ],
    '🔱': [
        'Emblème de Trident',
        'Fourche à fourche',
        'Trident'
    ],
    '📛': [
        'Nom du badge',
        'Tofu en feu',
        'Etiquette de Nom',
        'Etiquette de Feu'
    ],
    '🔰': [
        'Symbole japonais pour le débutant',
        'Marque Shoshinsha',
        'Bouclier vert jaune'
    ],
    '➰': [
        'Boucle bouclée',
        'Boucle de curling',
        'Boucle'
    ],
    '➿': [
        'Boucle double',
        'Boucle Double Curling',
        'Courriel vocal'
    ],
    '〽️': [
        'Marque alternative de la pièce',
        'M',
        'McDonald\'s'
    ],
    '✳️': ['Huit Asterisk parlé.'],
    '✴️': [
        'Huit étoiles noires pointues',
        'Étoile orange',
        'Huit étoiles pointées'
    ],
    '❇️': ['Étincelle'],
    'Ⓜ️': [
        'Lettre M du capital latin encerclé',
        'M cerclé',
        'Métro',
        'M cerclé'
    ],
    '🈁': [
        'Squared Katakana Koko',
        'Signe de mot japonais signifiant ici',
        'Ici',
        'Destination'
    ],
    '🈂️': [
        'Squared Katakana Sa',
        'Signe Japonaise signifiant Service ou Service Frais'
    ],
    '🈷️': [
        'format@@0 Squared CJK Unified Ideograph-6708',
        '74 radicaux',
        '月',
        'Montant mensuel du signe japonais'
    ],
    '🈶': [
        'format@@0 Squared CJK Unified Ideograph-6709',
        'Signe Japonais signifiant Non Gratuit',
        'Propre',
        '有'
    ],
    '🈯': [
        'format@@0 Squared CJK Unified Ideograph-6307',
        'Signe Japonais Sens Réservé',
        'Poignée du doigt',
        '指'
    ],
    '🉐': [
        'Avantage idéographe encerclé',
        'Bargain de sens du signe japonais',
        'Acquérir',
        '得'
    ],
    '🈹': [
        'format@@0 Squared CJK Unified Ideograph-5272',
        'Remise sur le Signe Japonais',
        'Vente de Bargain',
        '割'
    ],
    '🈚': [
        'format@@0 Squared CJK Unified Ideograph-7121',
        'Signe Japonais Gratuit',
        'Manquant',
        '無'
    ],
    '🈲': [
        'format@@0 Squared CJK Unified Ideograph-7981',
        'Signe japonais interdit',
        'Forbid',
        '禁'
    ],
    '🉑': [
        'Accepter l\'idéographe encerclé',
        'Signe Japonais Acceptable',
        'Accepter'
    ],
    '🈸': [
        'format@@0 Squared CJK Unified Ideograph-7533',
        'Application de sens du signe japonais',
        'Formulaire de demande',
        'Singe',
        'Demander',
        '申'
    ],
    '🈴': [
        'format@@0 Squared CJK Unified Ideograph-5408',
        'Signe japonais signifiant passage (Grade)',
        'Accord',
        '合'
    ],
    '🈳': [
        'format@@0 Squared CJK Unified Ideograph-7a7a',
        'Signe japonais signifiant vacance',
        'Vide et disponible',
        '空'
    ],
    '㊗️': [
        'Félicitations idéographiques encerclées',
        'Félicitations au Signe Japonais'
    ],
    '㊙️': [
        'Secret de l\'idéographe encerclé',
        'Secret de la signification du signe japonais',
        'Secrète'
    ],
    '🈺': [
        'Idéographe unifié CJK carré 55b6',
        'Signe Japonais Sensant Ouvert pour les Entreprises',
        'Travail',
        '営'
    ],
    '🈵': [
        'format@@0 Squared CJK Unified Ideograph-6e80',
        'Signe japonais signifiant plein; Pas de vacance',
        'Plein',
        '満'
    ],
    '🔘': ['Bouton radio'],
    '🛑': [
        'Panneau d\'arrêt',
        'Panneau octogonal'
    ],
    '⚕️': [
        'Symbole médical',
        'Esculapius',
        'Asklepios',
        'Sceptre d\'Asclepius',
        'Bâton d\'Aesculapius'
    ],
    '🛗': [
        'Élévateur',
        'Ascenseur'
    ],
    '⚧️': ['Symbole transgenres'],
    '♂️': [
        'Panneau masculin',
        'Symbole de l\'homme',
        'Symbole de Mars'
    ],
    '♀️': [
        'Panneau féminin',
        'Symbole de la femme',
        'Venus Symbol'
    ]
};
/*food and drinks*/
emojis_language[lang][6] = {
    '🍇': [
        'Raisins',
        'Raisin'
    ],
    '🍈': [
        'Pastèque',
        'Cantaloupe',
        'Miel',
        'Muskmelon'
    ],
    '🍉': ['Pastèque'],
    '🍊': [
        'Tangerine',
        'Mandarin',
        'Orange'
    ],
    '🍋': [
        'Citron',
        'Lemonade'
    ],
    '🍌': [
        'Banane',
        'Plantain'
    ],
    '🍍': ['Ananas'],
    '🥭': ['Mango'],
    '🍎': [
        'Pomme rouge',
        'Pomme délicieuse rouge'
    ],
    '🍏': [
        'Pomme verte',
        'Pomme délicieuse dorée',
        'Pomme de Grand-Smith'
    ],
    '🍐': ['Poire'],
    '🍑': [
        'Pêche',
        'Bas',
        'format@@0 Butt'
    ],
    '🍒': [
        'Cerises',
        'Cerise',
        'Fleur sauvage'
    ],
    '🍓': ['Fraise'],
    '🥝': [
        'Fruit de Kiwi',
        'Baie de Goose Chinoise',
        'Kiwi',
        'Kiwifruit'
    ],
    '🍅': ['Tomate'],
    '🥥': [
        'Noix de coco',
        'Cocoanut'
    ],
    '🥑': ['Avocado'],
    '🍆': [
        'Plante d\'Agneau',
        'Aubergine',
        'Phallique',
        'Légumes Violets'
    ],
    '🥔': [
        'Pomme de terre',
        'Patate cuite',
        'Idaho Patate'
    ],
    '🥕': ['Carotte'],
    '🌽': [
        'Oreille de maïs',
        'Maïs',
        'Le maïs sur le crâne',
        'Maïs',
        'Oreille du Maïs'
    ],
    '🌶️': [
        'Poivron chaud',
        'Poivron de Chili',
        'Épicé'
    ],
    '🥒': [
        'Concombre',
        'Gherkin',
        'Corniche'
    ],
    '🥬': [
        'Vert feuillu',
        'Choy Bok',
        'Chou chinois',
        'Cos Lettuce',
        'Laitue romaine',
        'Verts feuillus'
    ],
    '🥦': ['Brocoli'],
    '🍄': [
        'Champignon',
        'Champignon',
        'Toadstool'
    ],
    '🥜': [
        'Peanuts',
        'Noix'
    ],
    '🌰': [
        'Châtaignier',
        'maïs',
        'Écrou'
    ],
    '🍞': [
        'Pain',
        'Pain de pain'
    ],
    '🥐': ['Croissant'],
    '🥖': [
        'Pain de Baguette',
        'Pain français',
        'Baguette'
    ],
    '🥨': ['Pretzel'],
    '🥯': ['Bagel'],
    '🥞': [
        'Crêpes',
        'Créatures',
        'Gâteau Raccourci'
    ],
    '🧀': [
        'Bord de fromage',
        'Fromage'
    ],
    '🍖': [
        'Viande en os',
        'Barbecue',
        'Barbecue',
        'Viande de Manga'
    ],
    '🍗': [
        'Poultry Leg',
        'Baton de tambour',
        'Jambe de Turquie'
    ],
    '🥩': [
        'Découpe de Viande',
        'Viande',
        'Steak'
    ],
    '🥓': [
        'Bacon',
        'Rashers'
    ],
    '🍔': [
        'Hamburger',
        'Burger',
        'Cheeseburger'
    ],
    '🍟': [
        'Frites françaises',
        'Jetons',
        'Frites',
        'Frites de McDonald\'s'
    ],
    '🍕': [
        'Pizza',
        'Pizza de Pepperoni',
        'Tranche de pizza'
    ],
    '🌭': [
        'Chien chaud',
        'Hotdog',
        'Saucisse'
    ],
    '🥪': ['Sandwich'],
    '🌮': ['Taco'],
    '🌯': [
        'Burrito',
        'Envelopper'
    ],
    '🥙': [
        'Pain farci',
        'Doner Kebab',
        'Gyro',
        'Shawarma',
        'Sandwich Pita'
    ],
    '🍳': [
        'Cuisine',
        'Petit déjeuner',
        'Oeuf frit',
        'Poêle à frire'
    ],
    '🥘': [
        'Poêle peu profond de nourriture',
        'Paella',
        'Poêle de nourriture'
    ],
    '🍲': [
        'Pot de nourriture',
        'Bol de nourriture',
        'Soupe',
        'Ragoût'
    ],
    '🥣': [
        'Bol à cuillère',
        'Bol de céréales'
    ],
    '🥗': [
        'Salade verte',
        'Salade'
    ],
    '🍿': [
        'Pop-corn',
        'Poussière de maïs'
    ],
    '🧂': [
        'Sel',
        'format@@0 Salt Shaker'
    ],
    '🥫': [
        'Nourriture en conserve',
        'Boîte de nourriture',
        'Boîte d\'Étain',
        'Nourriture étanche'
    ],
    '🍱': [
        'Boîte Bento',
        'Boîte à lunch'
    ],
    '🍘': [
        'Craqueur de riz',
        'Craqueur'
    ],
    '🍙': [
        'Boule de riz',
        'Onigiri'
    ],
    '🍚': [
        'Riz cuit',
        'Riz bouilli',
        'Riz',
        'Bol de riz',
        'Riz à la vapeur'
    ],
    '🍛': [
        'Riz de Curry',
        'Curry',
        'Nourriture indienne',
        'Curry et riz'
    ],
    '🍜': [
        'Bol à vapeur',
        'Nouilles',
        'Nouilles avec baguettes',
        'Ramen'
    ],
    '🍝': [
        'Spaghetti',
        'Pâtes'
    ],
    '🍠': [
        'Pomme de terre sucrée rôtie',
        'Pomme de terre sucrée',
        'Yam'
    ],
    '🍢': [
        'Oden',
        'Kebab',
        'Broche'
    ],
    '🍣': [
        'sushi',
        'Sashimi',
        'Fruit de la mer'
    ],
    '🍤': [
        'Crevettes frites',
        'Crête frite',
        'Tempura des crevettes'
    ],
    '🍥': [
        'Gâteau de poisson avec tourbillon',
        'Gâteau aux poissons',
        'Pink Swirl'
    ],
    '🥮': [
        'Gâteau de Lune',
        'Gâteau de Lune'
    ],
    '🍡': [
        'Dango',
        'Bâton de dessert',
        'Boules Vertes Roses'
    ],
    '🥟': [
        'Beignets',
        'Empanada',
        'Pierogi'
    ],
    '🥠': ['Cookie de Fortune'],
    '🥡': [
        'Boîte de décollage',
        'Boîte-alimentaire chinoise',
        'Oyster Pail'
    ],
    '🍦': [
        'Crème glacée douce',
        'M. Whippy',
        'Serve doux'
    ],
    '🍧': [
        'Glace rasée',
        'Cône de neige'
    ],
    '🍨': [
        'Crème glacée',
        'Bol de glace',
        'Dessert'
    ],
    '🍩': [
        'Noisette',
        'Donut'
    ],
    '🍪': [
        'Cookie',
        'Biscuit',
        'Cookie de Puce de Chocolat'
    ],
    '🎂': [
        'Gâteau d\'anniversaire',
        'Date d\'anniversaire',
        'Gâteau',
        'Gâteau aux bougies'
    ],
    '🍰': [
        'Gâteau court',
        'Gâteau',
        'Pièce de gâteau',
        'Shortcake de fraise'
    ],
    '🧁': [
        'Gâteau à la coupe',
        'Gâteau aux fées'
    ],
    '🥧': ['Tarte à tarte'],
    '🍫': [
        'Barre de chocolat',
        'Barre de bonbons',
        'Chocolat'
    ],
    '🍬': [
        'Bonbons',
        'Tout le monde',
        'Doux'
    ],
    '🍭': [
        'Pop-Lollipop',
        'Sucker'
    ],
    '🍮': [
        'Pousse',
        'Creme Caramel',
        'Dessert',
        'Frappe',
        'Pudding'
    ],
    '🍯': [
        'Pot de miel',
        'Miel',
        'Pot'
    ],
    '🍼': [
        'Bouteille de bébé',
        'Alimentation en bouteille'
    ],
    '🥛': [
        'Verre de Lait',
        'Lait'
    ],
    '☕': [
        'Boissons chaudes',
        'Café',
        'Espresso',
        'Chocolat chaud',
        'Thé'
    ],
    '🍵': [
        'Thé sans poignée',
        'Thé vert',
        'Matcha',
        'Thé vert Matcha'
    ],
    '🍶': [
        'Sake',
        'Bouteille',
        'Vin de riz',
        'Bouteille et coupe de saké'
    ],
    '🍾': [
        'Bouteille avec Cork Popping',
        'Célébration',
        'Champagne',
        'Vin étincelant'
    ],
    '🍷': [
        'Verre à vin',
        'Alcool',
        'Vin Rouge',
        'Vin'
    ],
    '🍸': [
        'Verre à cocktails',
        'Cocktail',
        'Martini'
    ],
    '🍹': [
        'Boisson tropicale',
        'Coup de fruits',
        'Boisson de Tiki'
    ],
    '🍺': [
        'Tasse à bière',
        'Bière',
        'Stein de bière'
    ],
    '🍻': [
        'Tasses à bière à bières à patinage',
        'Bières',
        'Cordialement'
    ],
    '🥂': [
        'Lunettes à patinage',
        'Célébration',
        'Lunettes de champagne',
        'Cordialement'
    ],
    '🥃': [
        'Verre de Tumbler',
        'Bourbon',
        'Liqueur',
        'Rum',
        'Whiskey',
        'Whisky'
    ],
    '🥤': [
        'Coupe avec paille',
        'Milkshake',
        'Lissage',
        'Pop de soude',
        'Boisson douce'
    ],
    '🥢': ['Baguettes'],
    '🍽️': [
        'Fork et Couteau avec Plaque',
        'Dîner'
    ],
    '🍴': [
        'Fork et Couteau',
        'Taillade',
        'Couteau et fourche',
        'Argent'
    ],
    '🥄': ['Cuillère'],
    '🧄': ['Ail'],
    '🧅': ['Oignon'],
    '🧇': ['Waffle'],
    '🧆': ['Falafel'],
    '🧈': ['Beurre'],
    '🦪': ['Huître'],
    '🧃': [
        'Boîte-boissons',
        'Boîte de jus'
    ],
    '🧉': [
        'Boire au matelas',
        'Chimarra',
        'Cimarrón',
        'Yerba Mate'
    ],
    '🫒': ['Olive'],
    '🫐': ['Myrtilles'],
    '🫑': ['Poivron de la cloche'],
    '🫓': ['Pain'],
    '🫔': [
        'Tamalé',
        'tamoul'
    ],
    '🫕': ['Fondue'],
    '🧋': [
        'Thé bulle',
        'Boba'
    ],
    '🧊': ['Cube de glace']
};
/*flags*/
emojis_language[lang][7] = {
    '🏁': [
        'Drapeau Chequered',
        'Indicateur Dessiné',
        'Grille Fille',
        'Drapeau de course'
    ],
    '🚩': [
        'Drapeau triangulaire',
        'Drapeau sur le mât',
        'Drapeau rouge',
        'Drapeau triangulaire sur message'
    ],
    '🎌': [
        'Drapeaux croisés',
        'Deux drapeaux'
    ],
    '🏴': [
        'Drapeau noir',
        'Drapeau noir agité'
    ],
    '🏳️': [
        'Drapeau blanc',
        'Drapeau blanc agitant'
    ],
    '🏳️‍🌈': [
        'Drapeau arc-en-ciel',
        'Pride Flag'
    ],
    '🏳️‍⚧️': [
        'Indicateur de transgenres',
        'Bleu, rose et drapeau blanc',
        'Drapeau Trans'
    ],
    '🏴‍☠️': [
        'Drapeau Pirate',
        'Jolly Roger'
    ],
    '🇺🇳': [
        'Drapeau des Nations Unies',
        'ONU',
        'Drapeau : Nations Unies'
    ],
    '🇪🇺': [
        'Drapeau de l\'Union européenne',
        'EU',
        'Drapeau : Union européenne',
        'EU Flag'
    ],
    '🇦🇨': [
        'Drapeau de l\'île de l\'Ascension',
        'AC'
    ],
    '🇦🇩': [
        'Drapeau d\'Andorre',
        'AD',
        'Andorran Flag'
    ],
    '🇦🇪': [
        'Drapeau des Émirats Arabes Unis',
        'AE',
        'Emirati Flag',
        'Drapeau Émirats Arabes Unis'
    ],
    '🇦🇫': [
        'Drapeau de l\'Afghanistan',
        'AF',
        'Afghan Flag'
    ],
    '🇦🇬': [
        'Drapeau d\'Antigua-et-Barbuda',
        'AG'
    ],
    '🇦🇮': [
        'Drapeau d\'Anguilla',
        'AI',
        'Drapeau Anguillan'
    ],
    '🇦🇱': [
        'Drapeau de l\'Albanie',
        'Am',
        'Drapeau albanais'
    ],
    '🇦🇲': [
        'Drapeau d\'Arménie',
        'Am',
        '🇦🇲'
    ],
    '🇦🇴': [
        'Drapeau d\'Angola',
        'AO',
        'Drapeau angolais'
    ],
    '🇦🇶': [
        'Drapeau de l\'Antarctique',
        'AQ',
        'Drapeau Antarctique'
    ],
    '🇦🇷': [
        'Drapeau de l\'Argentine',
        'AR',
        'Drapeau argentin'
    ],
    '🇦🇸': [
        'Drapeau des Samoa Américaines',
        'COMME',
        'American Samoan Flag'
    ],
    '🇦🇹': [
        'Drapeau de l\'Autriche',
        'A',
        'Austrian Flag'
    ],
    '🇦🇺': [
        'Drapeau de l\'Australie',
        'UA',
        'Drapeau australien',
        'Drapeau australien'
    ],
    '🇦🇼': [
        'Drapeau d\'Aruba',
        'AW',
        'Drapeau Aruban'
    ],
    '🇦🇽': [
        'Drapeau des îles A<unk> land',
        'AX',
        'Drapeau A<unk> land'
    ],
    '🇦🇿': [
        'Drapeau d\'Azerbaïdjan',
        'AZ',
        'Drapeau azerbaïdjanais'
    ],
    '🇧🇦': [
        'Drapeau de la Bosnie-et-Herzégovine',
        'BA'
    ],
    '🇧🇧': [
        'Drapeau de la Barbade',
        'BO',
        'Drapeau Barbadien',
        'Drapeau Bajan'
    ],
    '🇧🇩': [
        'Drapeau du Bangladesh',
        'BD',
        'Bangladeshi Flag'
    ],
    '🇧🇪': [
        'Drapeau de la Belgique',
        'Être',
        'Drapeau belge'
    ],
    '🇧🇫': [
        'Drapeau du Burkina Faso',
        'BF',
        'Drapeau Burkinabé'
    ],
    '🇧🇬': [
        'Drapeau de la Bulgarie',
        'Glycémie',
        'Drapeau bulgare'
    ],
    '🇧🇭': [
        'Drapeau de Bahreïn',
        'BH',
        'Drapeau bahrani',
        'Drapeau bahreïnien'
    ],
    '🇧🇮': [
        'Drapeau du Burundi',
        'BI',
        'Burundian Flag'
    ],
    '🇧🇯': [
        'Drapeau du Bénin',
        'BP',
        'Drapeau béninais'
    ],
    '🇧🇱': [
        'Drapeau de Saint Barthélemy',
        'BL',
        'Drapeau : Saint-Barthélemy'
    ],
    '🇧🇲': [
        'Drapeau des Bermudes',
        'BM',
        'Bermudan Flag'
    ],
    '🇧🇳': [
        'Drapeau de Brunei',
        'NB',
        'Drapeau Brunéien'
    ],
    '🇧🇴': [
        'Drapeau de la Bolivie',
        'BO',
        'Bolivian Flag'
    ],
    '🇧🇶': [
        'Drapeau des Caraïbes Pays-Bas',
        'BP',
        'Bonaire'
    ],
    '🇧🇷': [
        'Drapeau du Brésil',
        'BR',
        'Drapeau brésilien'
    ],
    '🇧🇸': [
        'Drapeau des Bahamas',
        'Drapeau bahamien',
        'BS'
    ],
    '🇧🇹': [
        'Drapeau du Bhoutan',
        'BT',
        'Drapeau bhoutanais'
    ],
    '🇧🇻': [
        'Drapeau de l\'île de Bouvet',
        'BV',
        'Drapeau : Île Bouvet'
    ],
    '🇧🇼': [
        'Drapeau du Botswana',
        'BW',
        'Batswana Flag'
    ],
    '🇧🇾': [
        'Drapeau de la Biélorussie',
        'PAR',
        'Drapeau biélorusse'
    ],
    '🇧🇿': [
        'Drapeau de Belize',
        'BP',
        'Drapeau Belizean'
    ],
    '🇨🇦': [
        'Drapeau du Canada',
        'Drapeau canadien',
        'CA'
    ],
    '🇨🇨': [
        'Drapeau des îles Cocos (Keeling)',
        'CC'
    ],
    '🇨🇩': [
        'Flag of Congo - Kinshasa',
        'CD'
    ],
    '🇨🇫': [
        'Drapeau de la République centrafricaine',
        'FC',
        'Drapeau centrafricain'
    ],
    '🇨🇬': [
        'Drapeau du Congo - Brazzaville',
        'CG'
    ],
    '🇨🇭': [
        'Drapeau suisse',
        'CH',
        'Croix Rouge',
        'Drapeau suisse'
    ],
    '🇨🇮': [
        'Drapeau de la Côte d\'Ivoire',
        'CI',
        'Flag: Côte d’Ivoire'
    ],
    '🇨🇰': [
        'Drapeau des îles Cook',
        'CK',
        'Drapeau de l\'Île Cook'
    ],
    '🇨🇱': [
        'Drapeau du Chili',
        'CL',
        'Drapeau chilien'
    ],
    '🇨🇲': [
        'Drapeau du Cameroun',
        'CM',
        'Drapeau camerounais'
    ],
    '🇨🇳': [
        'Drapeau de la Chine',
        'CN',
        'Drapeau Chinois'
    ],
    '🇨🇴': [
        'Drapeau de la Colombie',
        'CM',
        'Drapeau colombien'
    ],
    '🇨🇵': [
        'Drapeau de l\'île Clipperton',
        'PP',
        'Drapeau : Île Clipperton'
    ],
    '🇨🇷': [
        'Drapeau du Costa Rica',
        'CR',
        'Drapeau costaricien'
    ],
    '🇨🇺': [
        'Drapeau de Cuba',
        'CU',
        'Drapeau cubain'
    ],
    '🇨🇻': [
        'Drapeau du Cap-Vert',
        'Cv',
        'Drapeau du Cap-Verdien'
    ],
    '🇨🇼': [
        'Drapeau de Curaçao',
        'CW',
        'Curaçaoan Flag'
    ],
    '🇨🇽': [
        'Drapeau de l\'île de Noël',
        'CX',
        'Drapeau de l\'île de Noël'
    ],
    '🇨🇾': [
        'Drapeau de Chypre',
        'CY',
        'Drapeau chypriote'
    ],
    '🇨🇿': [
        'Drapeau de Tchéquie',
        'CZ',
        'Drapeau tchèque'
    ],
    '🇩🇪': [
        'Drapeau de l\'Allemagne',
        'DL',
        'Drapeau Deutsch',
        'Drapeau allemand'
    ],
    '🇩🇬': [
        'Drapeau de Diego Garcia',
        'DG',
        'Drapeau : Diego Garcia'
    ],
    '🇩🇯': [
        'Drapeau de Djibouti',
        'DJ',
        'Djibouti Flag',
        'Djiboutian Flag'
    ],
    '🇩🇰': [
        'Drapeau du Danemark',
        'DK',
        'Drapeau danois'
    ],
    '🇩🇲': [
        'Drapeau de la Dominique',
        'DM'
    ],
    '🇩🇴': [
        'Drapeau de la République Dominicaine',
        'FAIRE',
        'Drapeau du dieu',
        'Drapeau Dominicain'
    ],
    '🇩🇿': [
        'Drapeau de l\'Algérie',
        'DZ',
        'Algerian Flag'
    ],
    '🇪🇦': [
        'Drapeau de Ceuta & Melilla',
        'Drapeau de Melilla',
        'Drapeau de Ceuta',
        'EA',
        'Flag: Ceuta & Melilla'
    ],
    '🇪🇨': [
        'Drapeau de l\'Équateur',
        'EC',
        'Ecuadorian Flag'
    ],
    '🇪🇪': [
        'Drapeau de l\'Estonie',
        'EE',
        'Drapeau estonien'
    ],
    '🇪🇬': [
        'Drapeau de l\'Égypte',
        'EG',
        'Drapeau égyptien'
    ],
    '🇪🇭': [
        'Drapeau du Sahara occidental',
        'EH',
        'Drapeau du Sahara occidental'
    ],
    '🇪🇷': [
        'Drapeau d\'Érythrée',
        'JE',
        'Eritrean Flag'
    ],
    '🇪🇹': [
        'Drapeau de l\'Éthiopie',
        'ET',
        'Ethiopian Flag'
    ],
    '🇪🇸': [
        'Drapeau de l\'Espagne',
        'S',
        'Drapeau espagnol'
    ],
    '🇫🇮': [
        'Drapeau de la Finlande',
        'IF',
        'Drapeau finlandais'
    ],
    '🇫🇯': [
        'Drapeau des Fidji',
        'FJ',
        'Fijian Flag'
    ],
    '🇫🇰': [
        'Drapeau des îles Malouines',
        'FK',
        'Drapeau Falklander'
    ],
    '🇫🇲': [
        'Drapeau de Micronésie',
        'MM',
        'Drapeau Micronésien'
    ],
    '🇫🇴': [
        'Drapeau des îles Féroé',
        'Février',
        'Faroe Islander Flag'
    ],
    '🇫🇷': [
        'Drapeau de la France',
        'FR',
        'Drapeau français'
    ],
    '🇬🇦': [
        'Drapeau du Gabon',
        'GR',
        'Drapeau gabonais'
    ],
    '🇬🇧': [
        'Drapeau de la Grande-Bretagne',
        'GR',
        'Drapeau britannique',
        'Royaume-Uni',
        'Jack de l\'Union',
        'RU'
    ],
    //"🏴󠁧󠁢󠁥󠁮󠁧󠁿": ["Flag of England", "St George's Cross", "English Flag"],
    //"🏴󠁧󠁢󠁷󠁬󠁳󠁿": ["Flag of Wales", "Welsh Flag", "Red Dragon", "Baner Cymru", "Y Ddraig Goch"],
    //"🏴󠁧󠁢󠁳󠁣󠁴󠁿": ["Flag of Scotland", "Scottish Flag", "St Andrew's Cross", "Saltire"],
    //"🏴󠁧󠁢󠁮󠁩󠁲󠁿": ["Flag of Northern Ireland"],
    '🇬🇩': [
        'Drapeau de la Grenade',
        'GD',
        'Drapeau Grenadien'
    ],
    '🇬🇪': [
        'Drapeau de la Géorgie',
        'GE',
        'Drapeau géorgien'
    ],
    '🇬🇫': [
        'Drapeau de la Guyane française',
        'GF',
        'Drapeau Guinéen français'
    ],
    '🇬🇬': [
        'Drapeau de Guernesey',
        'GG',
        'Guernsey Flag'
    ],
    '🇬🇭': [
        'Drapeau du Ghana',
        'GH',
        'Drapeau ghanéen'
    ],
    '🇬🇮': [
        'Drapeau de Gibraltar',
        'GI',
        'Drapeau Gibraltaire'
    ],
    '🇬🇱': [
        'Drapeau du Groenland',
        'GL',
        'Greenlandic Flag'
    ],
    '🇬🇲': [
        'Drapeau de la Gambie',
        'MJ',
        'Gambian Flag'
    ],
    '🇬🇳': [
        'Drapeau de la Guinée',
        'GN',
        'Guinean Flag'
    ],
    '🇬🇵': [
        'Drapeau de la Guadeloupe',
        'GR',
        'Drapeau guadeloupéen'
    ],
    '🇬🇶': [
        'Drapeau de Guinée équatoriale',
        'GQ',
        'Equatorial Guinean Flag',
        'Drapeau équato-guinéen'
    ],
    '🇬🇷': [
        'Drapeau de la Grèce',
        'GR',
        'Drapeau grec'
    ],
    '🇬🇸': [
        'Drapeau de la Géorgie du Sud et des îles Sandwich du Sud',
        'SG'
    ],
    '🇬🇹': [
        'Drapeau du Guatemala',
        'GT',
        'Guatemalan Flag'
    ],
    '🇬🇺': [
        'Drapeau de Guam',
        'GU',
        'Drapeau guaméen',
        'Drapeau Chamorro'
    ],
    '🇬🇼': [
        'Drapeau de la Guinée-Bissau',
        'GW',
        'Guinea-Bissau Flag'
    ],
    '🇬🇾': [
        'Drapeau de la Guyane',
        'GY',
        'Drapeau guyanais'
    ],
    '🇭🇰': [
        'Drapeau de Hong Kong SAR Chine',
        'HK',
        'Drapeau Hong Kong'
    ],
    '🇭🇲': [
        'Drapeau des îles Heard & McDonald',
        'HM',
        'Drapeau : Îles Heard & McDonald'
    ],
    '🇭🇳': [
        'Drapeau du Honduras',
        'HN',
        'Drapeau Hondurien'
    ],
    '🇭🇷': [
        'Drapeau de la Croatie',
        'RH',
        'Drapeau croate'
    ],
    '🇭🇹': [
        'Drapeau d\'Haïti',
        'HT',
        'Haitian Flag'
    ],
    '🇭🇺': [
        'Drapeau de la Hongrie',
        'PH',
        'Drapeau hongrois'
    ],
    '🇮🇨': [
        'Drapeau des îles Canaries',
        'G/I'
    ],
    '🇮🇩': [
        'Drapeau de l\'Indonésie',
        'ID',
        'Drapeau indonésien'
    ],
    '🇮🇪': [
        'Drapeau de l\'Irlande',
        'IG',
        'Drapeau irlandais'
    ],
    '🇮🇱': [
        'Drapeau d\'Israël',
        'IL',
        'Drapeau israélien'
    ],
    '🇮🇲': [
        'Drapeau de l\'île de Man',
        'IM',
        'Manx Flag'
    ],
    '🇮🇳': [
        'Drapeau de l\'Inde',
        'EN',
        'Drapeau Indien'
    ],
    '🇮🇴': [
        'Drapeau du territoire britannique de l\'océan Indien',
        'IO',
        'Drapeau britannique de l\'océan Indien'
    ],
    '🇮🇶': [
        'Drapeau de l\'Irak',
        'IQ',
        'Iraqi Flag'
    ],
    '🇮🇷': [
        'Drapeau de l’Iran',
        'IR',
        'Drapeau iranien'
    ],
    '🇮🇸': [
        'Drapeau de l\'Islande',
        'EST',
        'Drapeau islandais'
    ],
    '🇮🇹': [
        'Drapeau de l\'Italie',
        'TI',
        'Drapeau italien'
    ],
    '🇯🇪': [
        'Drapeau de Jersey',
        'JE',
        'Drapeau de Jersey'
    ],
    '🇯🇲': [
        'Drapeau de la Jamaïque',
        'JM',
        'Drapeau jamaïcain'
    ],
    '🇯🇴': [
        'Drapeau de la Jordanie',
        'JO',
        'Drapeau jordanien'
    ],
    '🇯🇵': [
        'Drapeau du Japon',
        'JP',
        'Drapeau japonais'
    ],
    '🇰🇪': [
        'Drapeau du Kenya',
        'KE',
        'Drapeau kényan'
    ],
    '🇰🇬': [
        'Drapeau du Kirghizstan',
        'KG',
        'Kyrgyzstani Flag'
    ],
    '🇰🇭': [
        'Drapeau du Cambodge',
        'KH',
        'Drapeau cambodgien'
    ],
    '🇰🇮': [
        'Drapeau de Kiribati',
        'KI',
        'Drapeau I-Kiribati'
    ],
    '🇰🇲': [
        'Drapeau des Comores',
        'KM',
        'Drapeau comorien'
    ],
    '🇰🇳': [
        'Drapeau de Saint-Christophe-et-Nevis',
        'KN'
    ],
    '🇰🇵': [
        'Drapeau de la Corée du Nord',
        'KP',
        'Drapeau nord-coréen'
    ],
    '🇰🇷': [
        'Drapeau de la Corée du Sud',
        'KR',
        'Drapeau sud-coréen'
    ],
    '🇰🇼': [
        'Drapeau du Koweït',
        'KW',
        'Kuwaiti Flag'
    ],
    '🇰🇾': [
        'Drapeau des îles Caïman',
        'KY',
        'Drapeau caymanien'
    ],
    '🇰🇿': [
        'Drapeau du Kazakhstan',
        'ZG',
        'Kazakhstani Flag',
        'Kazakh Flag'
    ],
    '🇱🇦': [
        'Drapeau du Laos',
        'LA',
        'Drapeau laotien',
        'Drapeau laotien'
    ],
    '🇱🇧': [
        'Drapeau du Liban',
        'LG',
        'Drapeau libanais'
    ],
    '🇱🇨': [
        'Drapeau de Sainte-Lucie',
        'LC'
    ],
    '🇱🇮': [
        'Drapeau du Liechtenstein',
        'LI',
        'Drapeau du Liechtensteiner'
    ],
    '🇱🇰': [
        'Drapeau du Sri Lanka',
        'LG',
        'Drapeau sri-lankais'
    ],
    '🇱🇷': [
        'Drapeau du Liberia',
        'LR',
        'Drapeau libérien'
    ],
    '🇱🇸': [
        'Flag of Lesotho',
        'LS',
        'Basotho Flag'
    ],
    '🇱🇹': [
        'Drapeau de la Lituanie',
        'LT',
        'Drapeau lituanien'
    ],
    '🇱🇺': [
        'Drapeau luxembourgeois',
        'LU',
        'Drapeau luxembourgeois'
    ],
    '🇱🇻': [
        'Drapeau de Lettonie',
        'LV',
        'Drapeau Letton'
    ],
    '🇱🇾': [
        'Drapeau de la Libye',
        'LS',
        'Drapeau libyen'
    ],
    '🇲🇦': [
        'Drapeau du Maroc',
        'MM',
        'Drapeau marocain'
    ],
    '🇲🇨': [
        'Drapeau de Monaco',
        'MR',
        'Drapeau monégasque'
    ],
    '🇲🇩': [
        'Drapeau de la Moldavie',
        'DM',
        'Drapeau moldave'
    ],
    '🇲🇪': [
        'Drapeau du Monténégro',
        'Moi',
        'Drapeau Monténégrin'
    ],
    '🇲🇫': [
        'Drapeau de Saint Martin',
        'MF',
        'Flag: St. Martin'
    ],
    '🇲🇬': [
        'Drapeau de Madagascar',
        'MG',
        'Madagascan Flag'
    ],
    '🇲🇭': [
        'Drapeau des îles Marshall',
        'MH',
        'Drapeau Marshallais'
    ],
    '🇲🇰': [
        'Drapeau de la Macédoine',
        'MK',
        'Drapeau macédonien'
    ],
    '🇲🇱': [
        'Drapeau du Mali',
        'ML',
        'Drapeau malien'
    ],
    '🇲🇲': [
        'Drapeau du Myanmar (Birmanie)',
        'MM',
        'Drapeau du Myanmar',
        'Drapeau pour la Birmanie',
        'Drapeau birman'
    ],
    '🇲🇳': [
        'Drapeau de Mongolie',
        'NM',
        'Drapeau mongol'
    ],
    '🇲🇴': [
        'Drapeau de Macao Sar Chine',
        'MO',
        'Drapeau macanais'
    ],
    '🇲🇵': [
        'Drapeau des îles Mariannes du Nord',
        'MM',
        'Drapeau Micronésien'
    ],
    '🇲🇶': [
        'Drapeau de la Martinique',
        'MQ',
        'Drapeau serpent de Martinique',
        'Drapeau Martiniquais'
    ],
    '🇲🇷': [
        'Drapeau de la Mauritanie',
        'MR',
        'Drapeau mauritanien'
    ],
    '🇲🇸': [
        'Flag of Montserrat',
        'SM',
        'Montserratian Flag'
    ],
    '🇲🇹': [
        'Drapeau de Malte',
        'MT',
        'Maltese Flag'
    ],
    '🇲🇺': [
        'Flag of Mauritius',
        'MU',
        'Drapeau Mauricien'
    ],
    '🇲🇻': [
        'Drapeau des Maldives',
        'MT',
        'Drapeau des Maldiviens'
    ],
    '🇲🇼': [
        'Drapeau du Malawi',
        'MW',
        'Malawian Flag'
    ],
    '🇲🇽': [
        'Drapeau du Mexique',
        'MX',
        'Drapeau mexicain'
    ],
    '🇲🇾': [
        'Drapeau de Malaisie',
        'MON',
        'Malaysian Flag'
    ],
    '🇲🇿': [
        'Drapeau du Mozambique',
        'ZM',
        'Drapeau mozambicain'
    ],
    '🇳🇦': [
        'Drapeau de la Namibie',
        'NA',
        'Namibian Flag'
    ],
    '🇳🇨': [
        'Drapeau de la Nouvelle-Calédonie',
        'NC',
        'Nouveau drapeau calédonien'
    ],
    '🇳🇪': [
        'Drapeau du Niger',
        'NS',
        'Drapeau Nigérien'
    ],
    '🇳🇫': [
        'Drapeau de l\'île Norfolk',
        'NF',
        'Drapeau de l\'île Norfolk'
    ],
    '🇳🇬': [
        'Drapeau du Nigeria',
        'NG',
        'Drapeau nigérian'
    ],
    '🇳🇮': [
        'Flag of Nicaragua',
        'NI',
        'Nicaraguan Flag'
    ],
    '🇳🇱': [
        'Drapeau des Pays-Bas',
        'NL',
        'Drapeau néerlandais'
    ],
    '🇳🇴': [
        'Drapeau de la Norvège',
        'NON',
        'Drapeau norvégien'
    ],
    '🇳🇵': [
        'Drapeau du Népal',
        'NP',
        'Drapeau népalais'
    ],
    '🇳🇷': [
        'Drapeau de Nauru',
        'NR',
        'Nauruan Flag'
    ],
    '🇳🇺': [
        'Drapeau de Niue',
        'NU',
        'Niuean Flag'
    ],
    '🇳🇿': [
        'Drapeau de la Nouvelle-Zélande',
        'NZ',
        'Drapeau Kiwi'
    ],
    '🇴🇲': [
        'Drapeau d\'Oman',
        'OM',
        'Drapeau omanais'
    ],
    '🇵🇦': [
        'Drapeau du Panama',
        'PP',
        'Drapeau panaméen'
    ],
    '🇵🇪': [
        'EF',
        'Drapeau du Pérou',
        'Drapeau péruvien'
    ],
    '🇵🇫': [
        'Drapeau de la Polynésie Française',
        'PF',
        'Drapeau Polynésien français'
    ],
    '🇵🇬': [
        'Drapeau de la Papouasie - Nouvelle-Guinée',
        'PG',
        'Indicateur PNG',
        'Papua New Guinean Flag'
    ],
    '🇵🇭': [
        'Drapeau des Philippines',
        'PH'
    ],
    '🇵🇰': [
        'Drapeau du Pakistan',
        'PK',
        'Drapeau pakistanais'
    ],
    '🇵🇱': [
        'Drapeau de la Pologne',
        'PL',
        'Drapeau polonais'
    ],
    '🇵🇲': [
        'Drapeau de Saint Pierre & Miquelon',
        'MM'
    ],
    '🇵🇳': [
        'Drapeau de Pitcairn',
        'NP'
    ],
    '🇵🇷': [
        'Drapeau de Porto Rico',
        'PR',
        'Drapeau de Porto Rican'
    ],
    '🇵🇸': [
        'Drapeau des Territoires palestiniens',
        'PP',
        'Palestine'
    ],
    '🇵🇹': [
        'Drapeau du Portugal',
        'PT',
        'Drapeau portugais'
    ],
    '🇵🇼': [
        'Drapeau de Palau',
        'PT',
        'Drapeau Palauan'
    ],
    '🇵🇾': [
        'Drapeau du Paraguay',
        'PY',
        'Paraguayan Flag'
    ],
    '🇶🇦': [
        'Drapeau du Qatar',
        'QA',
        'Drapeau Qatari'
    ],
    '🇷🇪': [
        'Drapeau de la Réunion',
        'RETOUR',
        'Drapeau Réunionnais'
    ],
    '🇷🇴': [
        'Drapeau de la Roumanie',
        'RO',
        'Drapeau roumain'
    ],
    '🇷🇸': [
        'Drapeau de Serbie',
        'SR',
        'Drapeau serbe'
    ],
    '🇷🇺': [
        'Drapeau de la Russie',
        'RU',
        'Drapeau Russe'
    ],
    '🇷🇼': [
        'Flag of Rwanda',
        'RW',
        'Rwandan Flag'
    ],
    '🇸🇦': [
        'Drapeau de l\'Arabie Saoudite',
        'SA',
        'Drapeau Arabe Saoudite'
    ],
    '🇸🇧': [
        'Drapeau des îles Salomon',
        'SB',
        'Drapeau de Salomon'
    ],
    '🇸🇨': [
        'Drapeau des Seychelles',
        'SM',
        'Seychellois Flag'
    ],
    '🇸🇩': [
        'Drapeau du Soudan',
        'Carte SD',
        'Sudanese Flag'
    ],
    '🇸🇪': [
        'Drapeau de la Suède',
        'SE',
        'Drapeau suédois'
    ],
    '🇸🇬': [
        'Drapeau de Singapour',
        'SG',
        'Drapeau singapourien'
    ],
    '🇸🇭': [
        'Drapeau de Sainte-Hélène',
        'SH'
    ],
    '🇸🇮': [
        'Drapeau de la Slovénie',
        'SI',
        'Drapeau slovène'
    ],
    '🇸🇯': [
        'Drapeau de Svalbard & Jan Mayen',
        'SJ'
    ],
    '🇸🇰': [
        'Drapeau de la Slovaquie',
        'SK',
        'Drapeau slovaque',
        'Slovakian Flag'
    ],
    '🇸🇱': [
        'Drapeau de la Sierra Leone',
        'SL',
        'Sierra Leonean Flag'
    ],
    '🇸🇲': [
        'Drapeau de Saint-Marin',
        'SM',
        'Drapeau sammarin'
    ],
    '🇸🇳': [
        'Drapeau du Sénégal',
        'SN',
        'Drapeau sengalais'
    ],
    '🇸🇴': [
        'Drapeau de la Somalie',
        'SO',
        'Somalian Flag'
    ],
    '🇸🇷': [
        'Drapeau du Suriname',
        'SR',
        'Drapeau Surinamer'
    ],
    '🇸🇸': [
        'Drapeau du Sud-Soudan',
        'SS',
        'Drapeau sud-soudanais'
    ],
    '🇸🇹': [
        'Drapeau de Sa<unk> o Tomé & Príncipe',
        'JE'
    ],
    '🇸🇻': [
        'Drapeau du Salvador',
        'SV',
        'Drapeau du Salvador'
    ],
    '🇸🇽': [
        'Drapeau de Saint-Martin',
        'SX'
    ],
    '🇸🇾': [
        'Drapeau de la Syrie',
        'SY',
        'Drapeau syrien'
    ],
    '🇸🇿': [
        'Drapeau du Swaziland',
        'SZ',
        'Drapeau Eswatini'
    ],
    '🇹🇦': [
        'Flag of Tristan Da Cunha',
        'TI'
    ],
    '🇹🇨': [
        'Drapeau des îles Turques et Caïques',
        'CT'
    ],
    '🇹🇩': [
        'Drapeau du Tchad',
        'TD',
        'Drapeau tchadien'
    ],
    '🇹🇫': [
        'Drapeau des territoires français du Sud',
        'TF',
        'Drapeau français des Terres du Sud'
    ],
    '🇹🇬': [
        'Drapeau du Togo',
        'TG',
        'Togolese Flag'
    ],
    '🇹🇭': [
        'Drapeau de la Thaïlande',
        'TH',
        'Drapeau Thaï'
    ],
    '🇹🇯': [
        'Drapeau du Tadjikistan',
        'TJ',
        'Drapeau du Tadjik'
    ],
    '🇹🇰': [
        'Drapeau de Tokelau',
        'TK',
        'Drapeau Tokelauan'
    ],
    '🇹🇱': [
        'Drapeau du Timor-Leste',
        'TL',
        'Drapeau du Timor-Leste',
        'Drapeau du Timor oriental'
    ],
    '🇹🇲': [
        'Drapeau du Turkménistan',
        'MT',
        'Turkmen Flag'
    ],
    '🇹🇳': [
        'Drapeau de la Tunisie',
        'NT',
        'Drapeau tunisien'
    ],
    '🇹🇴': [
        'Drapeau de Tonga',
        'À',
        'Tongan Flag'
    ],
    '🇹🇷': [
        'Drapeau de la Turquie',
        'TR',
        'Drapeau turc'
    ],
    '🇹🇹': [
        'Drapeau de Trinité-et-Tobago',
        'TT'
    ],
    '🇹🇻': [
        'Drapeau de Tuvalu',
        'Télévision',
        'Drapeau tuvaluan'
    ],
    '🇹🇼': [
        'Drapeau de Taïwan',
        'TW',
        'Drapeau taïwanais'
    ],
    '🇹🇿': [
        'Drapeau de la Tanzanie',
        'TZ',
        'Tanzanian Flag'
    ],
    '🇺🇦': [
        'Drapeau de l’Ukraine',
        'UA',
        'Drapeau ukrainien'
    ],
    '🇺🇬': [
        'Drapeau de l\'Ouganda',
        'UG',
        'Drapeau ougandais'
    ],
    '🇺🇲': [
        'Drapeau : Îles éloignées des États-Unis',
        'UM'
    ],
    '🇺🇸': [
        'Drapeau des États-Unis',
        'US',
        'Drapeau américain',
        'Drapeau américain'
    ],
    '🇺🇾': [
        'Flag of Uruguay',
        'UY',
        'Uruguayan Flag'
    ],
    '🇺🇿': [
        'Flag of Uzbekistan',
        'UZ',
        'Uzbekistani Flag',
        'Uzbek Flag'
    ],
    '🇻🇦': [
        'Drapeau du Saint-Siège',
        'VA',
        'Drapeau Vanticanien'
    ],
    '🇻🇨': [
        'Drapeau de Saint-Vincent-et-Grenadines',
        'VC'
    ],
    '🇻🇪': [
        'Drapeau du Venezuela',
        'Ve',
        'Drapeau vénézuélien'
    ],
    '🇻🇬': [
        'Drapeau des îles Vierges britanniques',
        'VG',
        'Drapeau de l\'île Vierge Britannique'
    ],
    '🇻🇮': [
        'Drapeau des îles Vierges américaines',
        'Vi',
        'Drapeau de l\'île Vierge des États-Unis'
    ],
    '🇻🇳': [
        'Drapeau du Viêt Nam',
        'VN',
        'Drapeau vietnamien'
    ],
    '🇻🇺': [
        'Flag of Vanuatu',
        'VU',
        'Ni-Vanuatu Flag'
    ],
    '🇼🇫': [
        'Drapeau de Wallis & Futuna',
        'WF'
    ],
    '🇼🇸': [
        'Drapeau de Samoa',
        'WS',
        'Samoan Flag'
    ],
    '🇽🇰': [
        'Drapeau du Kosovo',
        'XK',
        'Kosovar Flag'
    ],
    '🇾🇪': [
        'Drapeau du Yémen',
        'YE',
        'Drapeau yéménite'
    ],
    '🇾🇹': [
        'Drapeau de Mayotte',
        'YT',
        'Drapeau Mayotte'
    ],
    '🇿🇦': [
        'Drapeau de l\'Afrique du Sud',
        'ZA',
        'Drapeau sud-africain'
    ],
    '🇿🇲': [
        'Drapeau de la Zambie',
        'ZM',
        'Zambian Flag'
    ],
    '🇿🇼': [
        'Drapeau du Zimbabwe',
        'ZW',
        'Drapeau du Zimbabwe',
        'Drapeau Zim'
    ]
};
/*sports*/
emojis_language[lang][8] = {
    '🧗‍{{skin_tone}}‍‍♂️': [
        'Escalade de l\'homme',
        'Escalade masculine',
        'Escalade de l\'homme'
    ],
    '🧗‍{{skin_tone}}‍‍♀️': [
        'Escalade féminine',
        'Escalade féminine',
        'Escalade féminine'
    ],
    '🏇‍{{skin_tone}}‍': [
        'Course de chevaux',
        'Course de chevaux',
        'Jockey'
    ],
    '⛷‍{{skin_tone}}‍️': ['Skier'],
    '🏂‍{{skin_tone}}‍': ['Snowboarder'],
    '🏌️‍‍{{skin_tone}}‍♂️': ['Man Golfer'],
    '🏌️‍‍{{skin_tone}}‍♀️': ['Golfeur Femme'],
    '🏄‍‍{{skin_tone}}‍♂️': ['Surfeur homme'],
    '🏄‍‍{{skin_tone}}‍♀️': ['Surfeur féminin'],
    '🚣‍{{skin_tone}}‍‍♂️': ['Bateau à Voile Homme'],
    '🚣‍{{skin_tone}}‍‍♀️': ['Bateau en voûte féminine'],
    '🏊‍{{skin_tone}}‍‍♂️': [
        'Nager Homme',
        'Nageur homme'
    ],
    '🏊‍{{skin_tone}}‍‍♀️': [
        'Nager Femme',
        'Nageuse féminine'
    ],
    '⛹‍{{skin_tone}}‍️‍♂️': [
        'Balle rebondissante',
        'Joueur de basket-ball homme'
    ],
    '⛹️‍‍{{skin_tone}}‍♀️': [
        'Balle rebondissante Femme',
        'Joueur de basket-ball féminin'
    ],
    '🏋️‍‍{{skin_tone}}‍♂️': [
        'Lever les poids des hommes',
        'Haut-poids de l\'homme'
    ],
    '🏋️‍‍{{skin_tone}}‍♀️': [
        'Poids des femmes levantes',
        'Poids de la femme'
    ],
    '🚴‍‍{{skin_tone}}‍♂️': [
        'Moto Homme',
        'Cycliste homme',
        'Cycliste mâle'
    ],
    '🚴‍‍{{skin_tone}}‍♀️': [
        'Vélo féminin',
        'Cycliste féminin',
        'Cycliste féminin'
    ],
    '🚵‍‍{{skin_tone}}‍♂️': [
        'Vélo de montagne de l\'homme',
        'Motards en montagne'
    ],
    '🚵‍‍{{skin_tone}}‍♀️': [
        'Vélo de montagne féminin',
        'Motards de montagne féminins'
    ],
    '🤸‍‍{{skin_tone}}‍♂️': [
        'Chariot d\'homme',
        'Roue de charpente de l\'homme'
    ],
    '🤸‍‍{{skin_tone}}‍♀️': [
        'Chariot Femme',
        'Roue de chariot Femme'
    ],
    '🤼': [
        'Les gens luttent',
        'Lutte',
        'Lutteurs'
    ],
    '🤽‍‍{{skin_tone}}‍♂️': ['Polo de l\'Homme Jouant à l\'eau'],
    '🤽‍‍{{skin_tone}}‍♀️': ['Polo d\'eau de jeu féminin'],
    '🤾‍‍{{skin_tone}}‍♂️': ['Handball de jeu d\'homme'],
    '🤾‍‍{{skin_tone}}‍♀️': ['Femme à main jouant'],
    '🤹‍‍{{skin_tone}}‍♂️': [
        'Jonglerie de l\'homme',
        'Jongleur d\'homme'
    ],
    '🤹‍‍{{skin_tone}}‍♀️': [
        'Jonglerie féminine',
        'Jongleur féminin'
    ],
    '🧘‍‍{{skin_tone}}‍♂️': ['Homme dans la position Lotus'],
    '🧘‍‍{{skin_tone}}‍♀️': ['Position de la femme dans le lotus'],
    '🏃‍‍{{skin_tone}}‍♂️': ['Course d\'Hommes'],
    '🏃‍‍{{skin_tone}}‍♀️': ['Course de Femmes'],
    '🕺‍{{skin_tone}}‍': ['Danse de l\'homme'],
    '💃‍{{skin_tone}}‍': ['Danse féminine'],
    '🤺': [
        'Clôture de personne',
        'Eclat'
    ],
    '🏆': [
        'Trophée',
        'Championnat',
        'Gagnant'
    ],
    '🏅': ['Médaille de sport'],
    '🥇': [
        'Médaille de la 1ère Place',
        'Médaille de la première place',
        'Médaille d\'Or'
    ],
    '🥈': [
        'Médaille de la 2ème Place',
        'Médaille de la seconde place',
        'Médaille d\'Argent'
    ],
    '🥉': [
        'Médaille de la 3ème place',
        'Médaille de la troisième place',
        'Médaille de Bronze'
    ],
    '⚽': [
        'Football',
        'Football'
    ],
    '⚾': [
        'Baseball',
        'Boule Softball',
        'Blancs'
    ],
    '🥎': [
        'Boule Softball',
        'Jaune'
    ],
    '🏀': [
        'Basket-ball',
        'Orange',
        'Basket-ball et Hoop'
    ],
    '🏐': ['Volley-ball'],
    '🏈': [
        'Football américain',
        'Football',
        'Gridiron',
        'Superbol'
    ],
    '🏉': [
        'Rugby Football',
        'Football',
        'La ligue',
        'Rugby',
        'Union'
    ],
    '🎾': [
        'Tennis',
        'Balle',
        'Racket',
        'Racquet'
    ],
    '🥏': [
        'Disque volant',
        'Frisbee'
    ],
    '🎳': [
        'Bowling',
        'Épingles',
        'Skittles',
        'Dix Pins de Bowling'
    ],
    '🏏': [
        'Cricket',
        'Chauve-souris',
        'Balle'
    ],
    '🏑': [
        'Hockey de champ',
        'Balle',
        'Bâton'
    ],
    '🏒': [
        'Hockey sur glace',
        'Balle',
        'Bâton'
    ],
    '🥍': [
        'Lacrosse',
        'Balle',
        'Bâton'
    ],
    '🏓': [
        'Ping Pong',
        'Tennis de table',
        'Balle',
        'Paddle'
    ],
    '🏸': [
        'Badminton',
        'Shuttlecock',
        'Racquet'
    ],
    '🥊': ['Gants de boxe'],
    '🥋': [
        'Uniforme d\'arts martiaux',
        'Judo',
        'Karaté',
        'Kung Fu'
    ],
    '⛳': [
        'Signaler dans un trou',
        'Drapeau de golf'
    ],
    '⛸️': [
        'Patinage sur Glace',
        'Patinage'
    ],
    '🎣': [
        'Pêche',
        'Pôle',
        'Poisson',
        'Canne à pêche'
    ],
    '🎽': [
        'Chemise de Course',
        'Singlet',
        'Ceinture'
    ],
    '🎿': [
        'Skis',
        'En cours de ski',
        'Démarrage'
    ],
    '🛷': ['Lèche'],
    '🥌': ['Pierre de curling'],
    '🎯': [
        'Coup direct',
        'Tir à l\'arc',
        'Bullseye',
        'Flèches'
    ],
    '🎱': [
        'Pool 8 Balle',
        'Boule de Cue',
        'Snooker',
        'Billiards'
    ],
    '🎮': [
        'Contrôleur de jeu vidéo',
        'Playstation',
        'Xbox',
        'Manette de jeu',
        'Wii U'
    ],
    '♟️': ['Pion d\'échecs noirs'],
    '🏹': [
        'Arc et Flèche',
        'Arche'
    ],
    '🥅': ['Filet d\'objectif'],
    '🀄': [
        'Dragon Rouge de Mahjong',
        'Mahjong',
        'Mahjong Tile',
        '中',
        'Dragon Rouge de la tuile de Mahjong'
    ],
    '🎴': [
        'Cartes de jeu de fleurs',
        'Deck de cartes',
        'Hanafuda',
        'Hwatu',
        'Cartes de jeu'
    ]
};
/*travel and places*/
emojis_language[lang][9] = {
    '🚂': [
        'Locomotive à vapeur',
        'Train à vapeur',
        'Locomotive ferroviaire'
    ],
    '🚃': [
        'Voiture ferroviaire',
        'Voiture de fer',
        'Voiture ferroviaire',
        'Véhicule de fer',
        'Wagon ferroviaire'
    ],
    '🚄': ['Train à grande vitesse'],
    '🚅': [
        'Train à balles',
        'Shinkansen',
        'Train à grande vitesse avec nez de balle'
    ],
    '🚆': ['Former'],
    '🚇': [
        'Souterrain',
        'Tube',
        'Métro',
        'Métro'
    ],
    '🚈': [
        'Rail léger',
        'Former'
    ],
    '🚊': ['Tram'],
    '🚝': [
        'Monorail',
        'Former'
    ],
    '🚞': [
        'Chemin de fer de montagne',
        'Funicular',
        'Train et Montagne'
    ],
    '🚋': ['Tram Car'],
    '🚌': [
        'Bus',
        'Coach'
    ],
    '🚍': [
        'Bus en cours',
        'Devant du bus'
    ],
    '🚎': [
        'Trolleybus',
        'Bus électrique'
    ],
    '🚐': [
        'Minibus',
        'Minivan',
        'Transporteur de personnes'
    ],
    '🚑': ['Ambulance'],
    '🚒': [
        'Moteur de feu',
        'Service d\'incendie',
        'Camion de Feu'
    ],
    '🚓': [
        'Voiture de police',
        'Voiture Policier',
        'Voiture de police'
    ],
    '🚔': [
        'Voiture de police en cours',
        'Devant de la voiture de police',
        'Voiture Policier'
    ],
    '🚕': [
        'Taxi',
        'Taxi de New York',
        'Côté Taxi'
    ],
    '🚖': [
        'Taxi',
        'Taxi à venir',
        'Devant du Taxi'
    ],
    '🚗': [
        'Automobile',
        'Voiture rouge',
        'Côté voiture'
    ],
    '🚘': [
        'Automobile à venir',
        'Voiture rouge',
        'Devant de la voiture'
    ],
    '🚚': [
        'Camion de livraison',
        'Camion'
    ],
    '🚛': [
        'Lorry articulé',
        'Camion Vert'
    ],
    '🚜': [
        'tracteur',
        'Ferme'
    ],
    '🛻': ['Camion de ramassage'],
    '🏎️': [
        'Voiture de course',
        'F1',
        'Formule 1',
        'Voiture de course'
    ],
    '🏍️': [
        'Moto',
        'Moto'
    ],
    '🛵': [
        'Trottinette motrice',
        'Vespa',
        'Vélo moteur',
        'Vélo moteur'
    ],
    '🛺': [
        'Rickshaw automatique',
        'Tuk Tuk'
    ],
    '🚲': [
        'Vélo',
        'Vélo'
    ],
    '🛴': ['Expulser la trottinette'],
    '⛵': [
        'Voilier',
        'Dinghy',
        'Yacht'
    ],
    '🚤': [
        'Speedboat',
        'Bateau à moteur',
        'Bateau à moteur'
    ],
    '🛳️': ['Vaisseau Passager'],
    '⛴️': ['Baies'],
    '🛥️': ['Bateau à moteur'],
    '🚢': ['Navire de croisière'],
    '✈️': [
        'Avion',
        'Aeroplane'
    ],
    '🛩️': [
        'Petit avion',
        'Petit Aéroplan'
    ],
    '🚁': ['Hélicoptère'],
    '🚟': ['Voie ferrée de suspension'],
    '🚠': ['Téléphérique de montagne'],
    '🚡': [
        'Tramway aérien',
        'Câble Car',
        'Gondola',
        'Voie ferrée'
    ],
    '🛰️': ['Satellite'],
    '🚀': [
        'Fusée',
        'Navette Espace'
    ],
    '🛸': [
        'Saucer Volant',
        'OVNI'
    ],
    '🚉': [
        'Gare ferroviaire',
        'Plateau de Train'
    ],
    '🚏': ['Arrêt de bus'],
    '🛣️': [
        'Autoroute',
        'Route',
        'Autoroute',
        'Préfecture'
    ],
    '🛤️': ['Voie ferrée'],
    '🛫': [
        'Départ avion',
        'Décollage'
    ],
    '🛬': [
        'Arrivée en avion',
        'Avion Arrivé',
        'Atterrissage'
    ],
    '🗾': ['Carte du Japon'],
    '⛰️': ['Montagne'],
    '🏔️': ['Montagne enneigée'],
    '🌋': ['Volcan'],
    '🗻': [
        'Mont Fuji',
        'Fuji-san'
    ],
    '🏕️': [
        'Camping',
        'Camping'
    ],
    '🏖️': ['Plage avec parapluie'],
    '🏜️': ['Desert'],
    '🏝️': ['Île du Désert'],
    '🏞️': ['Parc national'],
    '🏟️': [
        'Stade',
        'Grand stand',
        'Stade de Sport'
    ],
    '🏛️': [
        'Bâtiment classique',
        'Grecque',
        'Rome'
    ],
    '🏗️': [
        'Construction de Bâtiment',
        'Grue'
    ],
    '🏘️': [
        'Bâtiments',
        'Groupe de Maisons'
    ],
    '🏚️': [
        'Déréguler le bâtiment de la maison',
        'Maison abandonnée',
        'Vieille Maison',
        'Maison hantée'
    ],
    '🏠': [
        'Accueil',
        'Bâtiment de maison'
    ],
    '🏡': [
        'Maison avec jardin',
        'Maison et arbre'
    ],
    '🏢': [
        'Immeuble de bureau',
        'Bâtiment de la Ville',
        'Bâtiment haut de gamme'
    ],
    '🏣': [
        'Poste japonaise',
        'Marque postale japonaise'
    ],
    '🏤': [
        'Bureau de poste',
        'Poste européenne'
    ],
    '🏥': [
        'Hôpital',
        'JE',
        'A&E',
        'Salle d\'urgence',
        'Accident et urgence',
        'Croix Rouge',
        'Médicale'
    ],
    '🏦': [
        'Bancaire',
        'BK',
        'Bakkureru',
        'Branche bancaire',
        'Bakkureru'
    ],
    '🏨': [
        'Hôtel',
        'Hébergement',
        'Construction H'
    ],
    '🏩': [
        'Hôtel d\'amour',
        'Hôtel Coeur d\'Amour'
    ],
    '🏪': [
        'Magasin de commodité',
        'Boutique 24h',
        '7-Onze',
        'Magasin d\'angle',
        'Kwik-E-Mart'
    ],
    '🏫': [
        'École',
        'Tour de l\'horloge',
        'Moyen élémentaire supérieur'
    ],
    '🏬': [
        'Magasin du Département',
        'Centre commercial',
        'Magasins'
    ],
    '🏭': [
        'Usine',
        'Industriel',
        'Industrie',
        'Pollution',
        'Brouillard'
    ],
    '🏯': [
        'Château japonais',
        'Forteresse'
    ],
    '🏰': [
        'Château Européen',
        'Turrets'
    ],
    '💒': [
        'Coeur d\'église',
        'Mariage de l\'église',
        'Mariage'
    ],
    '🗼': [
        'Tour de Tokyo',
        'Tour Eiffel',
        'Tour rouge'
    ],
    '🗽': [
        'Statue de la Liberté',
        'New York'
    ],
    '⛪': [
        'Bâtiment de l\'église',
        'Croix'
    ],
    '🕌': [
        'Mosquée',
        'Toit en dôme',
        'Minaret'
    ],
    '🛕': ['Temple hindou'],
    '🕍': [
        'Synagogue',
        'Juif',
        'Synagog',
        'Temple'
    ],
    '⛩️': [
        'Sanctuaire du Shinto',
        'Kami-no-michi'
    ],
    '🕋': [
        'Kaaba',
        'Mecca'
    ],
    '🏙️': ['Cityscape'],
    '⛱️': [
        'Parapluie au sol',
        'Parapluie de plage'
    ],
    '🌄': [
        'Lever du soleil sur les montagnes',
        'Matin',
        'Lever du soleil'
    ],
    '🌅': [
        'Lever du soleil',
        'Coucher du soleil'
    ],
    '🌆': [
        'Paysage urbain au crépuscule',
        'Ciel Orange',
        'Ville crépusculaire'
    ],
    '🌇': [
        'Bâtiments du coucher du soleil',
        'Coucher du soleil de la ville'
    ],
    '🌃': [
        'Nuit avec étoiles',
        'Ville la nuit',
        'Nuit Étoilée'
    ],
    '🗿': [
        'Moai',
        'Statue de l\'île de Pâques',
        'Gravure de Roche Humaine',
        'Statue Moyai'
    ],
    '🌠': [
        'Étoile de tir',
        'Quand vous souhaitez dans une étoile',
        'Météoroïde'
    ],
    '🌌': [
        'Voie lactée',
        'Galaxie',
        'Ciel de la nuit',
        'Espace libre',
        'Étoiles',
        'Univers'
    ],
    '🎑': [
        'Cérémonie d\'observation de la lune',
        'Herbe, Beignets et Lune',
        'Récolter la Lune',
        'Fête de la Lune',
        'Tsukimi'
    ],
    '🛖': ['Cabane'],
    '🎢': [
        'Coastre Roll-up',
        'Rollercoaster',
        'Parc d\'attractions'
    ]
};
/*technologies (smartphones, laptops, etc.) and office*/
emojis_language[lang][10] = {
    '⌚': [
        'Regarder',
        'Feuille de temps',
        'Montre Pomme'
    ],
    '⏰': ['Horloge alarme'],
    '⏱️': ['Chronomètre'],
    '⏲️': ['Horloge de minuterie'],
    '🕰️': ['Horloge de Mantelpiece'],
    '🧭': ['Boussole'],
    '🕹️': ['Joystick'],
    '🎙️': ['Microphone Studio'],
    '🎚️': ['Curseur de niveau'],
    '🎛️': ['Contrôler les Knobs'],
    '📻': [
        'Radio',
        'Radio numérique',
        'Sans-fil'
    ],
    '📱': [
        'Téléphone mobile',
        'Téléphone intelligent',
        'iPhone',
        'Téléphone cellulaire'
    ],
    '📲': [
        'Téléphone mobile avec Flèche droite à gauche',
        'Appel téléphonique',
        'Pointer vers le téléphone'
    ],
    '☎️': [
        'Téléphone noir',
        'Téléphone Rotatif',
        'Rouge'
    ],
    '📞': [
        'Récepteur de téléphone',
        'Handset',
        'Téléphone',
        'Noir'
    ],
    '📟': [
        'Téléavertisseur',
        'Beeper',
        'Bleeper'
    ],
    '📠': [
        'Télécopieur',
        'Facsimile',
        'Fax'
    ],
    '🔋': [
        'Batterie',
        'Batterie AA',
        'Batterie du téléphone'
    ],
    '🔌': [
        'Prise électrique',
        'AC Adaptor',
        'Câble d\'alimentation',
        'Prise d\'alimentation'
    ],
    '💻': [
        'Ordinateur portable',
        'Carnet',
        'Ordinateur personnel',
        'Pc',
        'MacBook',
        'Surface'
    ],
    '🖥️': [
        'Ordinateur de bureau',
        'iMac',
        'Pc'
    ],
    '🖨️': ['Imprimante'],
    '⌨️': ['Keyboard'],
    '🖱️': [
        'Souris de l\'ordinateur',
        'Souris à trois boutons'
    ],
    '🖲️': ['Balle de Trackball'],
    '💽': [
        'Disque de l\'ordinateur',
        'Minidisc',
        'CD',
        'Dvd',
        'Blu-ray',
        'BD'
    ],
    '💾': [
        'Disquette',
        'Disque de 3,5′′',
        'Disque'
    ],
    '💿': [
        'Disque optique',
        'CD',
        'CD-ROM',
        'Disque compact'
    ],
    '📀': [
        'Dvd',
        'DVD-ROM',
        'Vidéo DVD'
    ],
    '🎥': [
        'Caméra de film',
        'Caméra de film',
        'Hollywood',
        'Films'
    ],
    '🎞️': ['Cadres de film'],
    '📽️': ['Projecteur de film'],
    '📺': [
        'Télévision',
        'Télévision'
    ],
    '📷': ['Appareil photo numérique'],
    '📸': ['Caméra avec Flash'],
    '📹': [
        'Caméra Vidéo',
        'Caméra'
    ],
    '📼': [
        'Vidéocassette',
        'VHS',
        'VCR',
        'Bande vidéo',
        'Vidéocassette'
    ],
    '💳': [
        'Carte de Crédit',
        'Carte de débit',
        'AMEX',
        'Diners Club',
        'American Express',
        'Mastercard',
        'Carte VISA'
    ],
    '📧': [
        'Courriel',
        'Courriel'
    ],
    '📨': [
        'Enveloppe Entrante',
        'Enveloppe avec des lignes',
        'Enveloppe rapide'
    ],
    '📩': [
        'Enveloppe avec Flèche vers le bas ci-dessus',
        'Enveloppe de la flèche vers le bas',
        'Insérer dans l\'Enveloppe'
    ],
    '📤': ['Boîte de réception'],
    '📥': ['Boîte de réception'],
    '📮': ['Boîte aux lettres'],
    '📁': [
        'Dossier de fichier',
        'Répertoire',
        'Répertoire',
        'Dossier de fichiers fermés'
    ],
    '📂': [
        'Ouvrir le dossier de fichier',
        'Répertoire'
    ],
    '🗂️': ['Diviseurs d\'indices de cartes'],
    '📅': [
        'Calendrier',
        '17 Juillet',
        'Journée mondiale de l\'émoji'
    ],
    '🗓️': ['Pad du calendrier en spirale'],
    '📆': [
        'Calendrier de déchirage',
        'Calendrier du jour',
        'Calendrier de bureau'
    ],
    '📈': [
        'Augmentation du graphique',
        'Graphique positif',
        'Graphique pointant vers le haut',
        'Graphique avec tendance vers le haut'
    ],
    '📉': [
        'Diminution de la carte',
        'Graphique négatif',
        'Graphique de pointe vers le bas',
        'Graphique avec tendance vers le bas'
    ],
    '📊': [
        'Graphique à barres',
        'Graphique à barres'
    ],
    '📌': [
        'Pushpin',
        'Thumb Tack'
    ],
    '📋': ['Presse-papiers'],
    '📍': [
        'Pushpin rond',
        'Épingle abandonnée',
        'Épingle de la carte',
        'Épingler',
        'Épingle rouge'
    ],
    '📎': [
        'Paperclip',
        'Clippy'
    ],
    '📏': ['Règle Droite'],
    '📐': [
        'Règle triangulaire',
        'Règle du Triangle'
    ],
    '✂️': [
        'Scissors',
        'Couper'
    ],
    '✏️': [
        'Crayon',
        'Crayon de plomb'
    ],
    '✒️': [
        'Nib noir',
        'Stylo Nib',
        'Fontaine de stylo'
    ],
    '🖋️': [
        'Fontaine de stylo',
        'Fontaine Gauche Bas'
    ],
    '🖊️': [
        'Stylo',
        'Stylo à bille gauche inférieur',
        'Stylo à bille'
    ],
    '🖌️': [
        'Pinceau',
        'Brosse',
        'Pinceau inférieur gauche'
    ],
    '🖍️': [
        'Crayon',
        'Ecrayon inférieur gauche'
    ],
    '📝': [
        'Mémo',
        'Memorandum',
        'Note',
        'Crayon et Papier'
    ],
    '🗑️': [
        'Panier en papier usé',
        'Boîte à ordures',
        'Corbeille',
        'Poubelle'
    ],
    '📡': ['Antenne satellite'],
    '🔭': [
        'Télescope',
        'Stargazing'
    ],
    '🔬': [
        'Microscope',
        'Magnify',
        'La science'
    ],
    '📭': ['Ouvrir la boîte aux lettres avec un drapeau réduit'],
    '📬': [
        'Ouvrir la boîte aux lettres avec un drapeau élevé',
        'Ouvrir la boîte aux lettres'
    ],
    '📪': ['Boîte aux lettres fermée avec drapeau réduit'],
    '📫': ['Boîte aux lettres fermée avec drapeau élevé'],
    '📰': ['Journaux'],
    '🗞️': [
        'Journal de décollage',
        'Livraison des journaux'
    ],
    '📄': [
        'Page face vers le haut',
        'Page imprimée'
    ],
    '📜': [
        'Défilement',
        'Diplôme',
        'Parchment'
    ],
    '📃': [
        'Page avec Curl',
        'Curled Page',
        'Curly Page'
    ],
    '📓': [
        'Carnet',
        'Livre noir et blanc'
    ],
    '📔': ['Bloc-notes avec couverture décorative'],
    '📕': [
        'Livre fermé',
        'Livre Rouge',
        'Manuel rouge'
    ],
    '📖': [
        'Livre Ouvert',
        'Réserver',
        'Révélation'
    ],
    '📗': [
        'Livre vert',
        'Manuel vert'
    ],
    '📘': [
        'Livre Bleu',
        'Manuel bleu'
    ],
    '📙': [
        'Livre orange',
        'Manuel orange'
    ],
    '📚': [
        'Livres',
        'Tas de Livres',
        'Pile de livres'
    ],
    '🗳️': [
        'Boîte aux urnes avec feuille',
        'Boîte aux votes',
        'Vote'
    ],
    '🗄️': [
        'Cabinet de dossier',
        'Cabinet de dépôt'
    ],
    '🗃️': ['Boîte de fichiers de carte'],
    '🖇️': ['Clips de papier liés']
};
/*clothes and accessories*/
emojis_language[lang][11] = {
    '👣': [
        'Empreintes',
        'Pieds',
        'Étapes'
    ],
    '👓': [
        'Lunettes',
        'Eveyglasses'
    ],
    '🕶️': ['Lunettes de soleil'],
    '🥽': ['Lunettes'],
    '🥼': ['Manteau de laboratoire'],
    '👔': [
        'Necktie',
        'Chemise de commerce',
        'Chemise et cravate'
    ],
    '👕': [
        'T-shirt',
        'Chemise en tee',
        'Polo chemise'
    ],
    '👖': [
        'Jeans',
        'Denim',
        'Pantalon',
        'Pantalon'
    ],
    '🧣': ['Scarf'],
    '🧤': ['Gants'],
    '🧥': ['Manteau'],
    '🧦': ['Socks'],
    '👗': [
        'Robe',
        'Jupe',
        'Toile'
    ],
    '👘': [
        'Kimono',
        'Toge de toilette',
        'Robe japonaise'
    ],
    '👙': [
        'Bikini',
        'Baigneurs',
        'Costume de bain'
    ],
    '🩱': ['Costume de bain en une pièce'],
    '👚': [
        'Vêtements pour femmes',
        'Blouse',
        'Chemise rose',
        'Blouse'
    ],
    '👛': [
        'Porte-monnaie',
        'Portefeuille'
    ],
    '👜': ['Sac à main'],
    '👝': [
        'Sac d\'embrayage',
        'Embrayage',
        'Petit sac',
        'Pouch'
    ],
    '🎒': [
        'Sac à dos',
        'Sac',
        'Sac d\'école',
        'Sacoche de l\'école',
        'Rucksack'
    ],
    '👞': [
        'Mans Shoe',
        'Chaussure',
        'Show marron',
        'Série de costumes'
    ],
    '👟': [
        'Chaussure athlétique',
        'Chaussure de Course',
        'Sneaker',
        'Coureur',
        'Entraîneur',
        'Chaussure de tennis'
    ],
    '🥾': ['Botte de randonnée'],
    '🥿': ['Chaussure plate'],
    '👠': [
        'Chaussure à haut talon',
        'Stiletto',
        'talons hauts'
    ],
    '👡': [
        'Sandale féminine',
        'Sandale à talon',
        'Saldals'
    ],
    '👢': [
        'Bottes pour femmes',
        'Bottes de Cowgirl',
        'Bottes hautes du genou',
        'Bottes à talon'
    ],
    '👑': [
        'Couronne',
        'Roi',
        'Reine',
        'Prince',
        'Royal',
        'Princesse'
    ],
    '👒': [
        'Chapeau',
        'Chapeau Femme',
        'Chapeau avec arc',
        'Chapeau de dames'
    ],
    '🎩': [
        'Chapeau Haut',
        'Veste formelle',
        'Groom'
    ],
    '🎓': [
        'Casquette de Graduation',
        'Ecole',
        'Diplômé',
        'Tableau de mortiers',
        'Université',
        'Casquette académique carrée'
    ],
    '🧢': [
        'Casquette Facturée',
        'Casquette de baseball'
    ],
    '⛑️': [
        'Casque avec Croix Blanche',
        'Casque de sauvetage de l\'ouvrier'
    ],
    '💼': [
        'Porte-documents',
        'Suitcase'
    ],
    '🎖️': [
        'Médaille Militaire',
        'Médaille',
        'Médaillon',
        'Décoration Militaire'
    ],
    '📿': [
        'Perles de prière',
        'Perles de Dhikr',
        'Perles Rosaires'
    ],
    '🎗️': ['Rappel Ribbo'],
    '🤿': ['Masque de plongée'],
    '🦺': ['Gilet de sécurité'],
    '🥻': [
        'Sari',
        'Saree',
        'Shari'
    ],
    '🩲': ['Briques'],
    '🩳': ['Shorts'],
    '🛼': [
        'Patinage à Roulettes',
        'Patinage en ligne',
        'Derby à rouleau'
    ],
    '🩴': [
        'Retourner le flop',
        'Sandale Thong'
    ],
    '🪖': ['Casque militaire'],
    '🩰': [
        'Chaussures à Ballet',
        'Chaussure de Pointe'
    ]
};
/*hands and parts of body*/
emojis_language[lang][12] = {
    '👋{{skin_tone}}': [
        'Panneau à mains agaçantes',
        'Au revoir',
        'Vague à la main',
        'Bonjour',
        'Hi'
    ],
    '🖐️{{skin_tone}}': [
        'Livrez avec les doigts en rafale',
        'Cinq mains',
        'Coupé à la main',
        'Main levée avec doigts en rafale'
    ],
    '🤚‍{{skin_tone}}‍': [
        'Décollage de la main',
        'Retour arrière'
    ],
    '✋‍{{skin_tone}}‍': [
        'Main levée',
        'Haut Cinq',
        'Arrêter'
    ],
    '🖖‍{{skin_tone}}‍': [
        'Vulcan Salute',
        'Spock',
        'Trek des étoiles',
        'Main levée avec une partie entre le milieu et les doigts de l\'anneau'
    ],
    '👌‍{{skin_tone}}‍': [
        'Panneau OK',
        'Ok',
        'Parfait',
        'Oui'
    ],
    '✌️‍{{skin_tone}}‍': [
        'Main de Victoire',
        'Devis de l\'air',
        'Panneau de Paix',
        'Panneau V'
    ],
    '🤞‍{{skin_tone}}‍': [
        'Doigts croisés',
        'Doigts croisés',
        'Bonne chance',
        'Livrez avec l\'index et les doigts du milieu croisés'
    ],
    '🤟‍{{skin_tone}}‍': [
        'Geste d\'amour-vous',
        'Signe de la main Je t\'aime'
    ],
    '🤘‍{{skin_tone}}‍': [
        'Signe des cornes',
        'Doigts du Diable',
        'Métal lourd',
        'Roche activée'
    ],
    '🤙‍{{skin_tone}}‍': [
        'Appelez-moi Signe à la main',
        'Main de téléphone',
        'Shaka'
    ],
    '🤏‍{{skin_tone}}‍': ['Pincement à la main'],
    '👈‍{{skin_tone}}‍': [
        'Indice de l\'index arrière à gauche',
        'Index du dos du pointeur gauche blanc'
    ],
    '👉‍{{skin_tone}}‍': [
        'Indice de pointage du dos à droite',
        'Index du dos de la pointe blanche à droite'
    ],
    '👆‍{{skin_tone}}‍': [
        'Indice de pointage vers le haut de l\'index',
        'Index du dos pointant le White Up',
        'Doigt du milieu'
    ],
    '👇‍{{skin_tone}}‍': [
        'Indice de pointage arrière vers le bas',
        'Index du dos du point blanc vers le bas',
        'Pointant vers le bas'
    ],
    '☝‍{{skin_tone}}‍️': [
        'Indice de pointage vers le haut',
        'Index de pointage de White Up',
        'Secrète'
    ],
    '🖕‍{{skin_tone}}‍': [
        'Doigt du milieu',
        'Retourner l\'oiseau',
        'Doigt Rude',
        'Main inversée avec le poignet moyen étendu'
    ],
    '👍‍{{skin_tone}}‍': [
        'Signe de pouce vers le haut',
        'J\'aime',
        'Oui'
    ],
    '👎‍{{skin_tone}}‍': [
        'Panneau des vignettes vers le bas',
        'Mauvais',
        'Dislike',
        'Non'
    ],
    '✊‍{{skin_tone}}‍': [
        'Poing élevé',
        'Pompe de poing'
    ],
    '👊‍{{skin_tone}}‍': [
        'Poing à venir',
        'Bro Fist',
        'Brofist',
        'Bump du poing',
        'Frappe',
        'Poing fermé',
        'Panneau Poing Poing'
    ],
    '🤛‍{{skin_tone}}‍': ['Bump du poing gauche'],
    '🤜‍{{skin_tone}}‍': ['Bump du poing droit'],
    '🤌‍{{skin_tone}}‍': [
        'Doigts pinçés',
        'Ma Che Vuoi',
        'Doigts'
    ],
    '👏‍{{skin_tone}}‍': [
        'Embrasement du panneau des mains',
        'Applaudissements',
        'Clap',
        'Clapage',
        'Clapet de golf',
        'Cycle des applaudissements'
    ],
    '🙌‍{{skin_tone}}‍': [
        'Lever les deux mains en célébration',
        'Lever les mains',
        'Armes dans l\'air',
        'Banzai',
        'Miracle du Festival',
        'Hallelujah',
        'Mains louées',
        'Deux mains'
    ],
    '👐‍{{skin_tone}}‍': [
        'Ouvrir le panneau des mains',
        'Câlin',
        'Mains de Jazz'
    ],
    '🤲‍{{skin_tone}}‍': [
        'Palmiers ensemble',
        'Dua'
    ],
    '🤝‍{{skin_tone}}‍': [
        'Handshake',
        'Secouement des mains'
    ],
    '🙏‍{{skin_tone}}‍': [
        'Mains pliées',
        'Namaste',
        'S\'il vous plaît',
        'Prière',
        'Merci',
        'Personne avec les mains pliées'
    ],
    '✍‍{{skin_tone}}‍️': ['Main d\'écriture'],
    '💪‍{{skin_tone}}‍': [
        'Biceps Flexed',
        'Muscles des bras flexibles',
        'Musle',
        'Forte'
    ],
    '🦵‍{{skin_tone}}‍': ['Jambe'],
    '🦶‍{{skin_tone}}‍': ['Pied'],
    '👂‍{{skin_tone}}‍': [
        'Oreille',
        'Oreilles',
        'Audience',
        'Écoute'
    ],
    '👃‍{{skin_tone}}‍': [
        'Nez',
        'Imbriquer',
        'Sniffage',
        'Stinky'
    ],
    '🧠': ['Cerveau'],
    '🦷': ['Dent'],
    '👀': [
        'Yeux',
        'Boules d\'yeux',
        'Yeux bouleversants',
        'Yeux sages'
    ],
    '👁️': [
        'Oeil',
        'Œil unique'
    ],
    '👅': ['Sortie de la langue'],
    '👄': [
        'Bouche',
        'Lèvres embrassantes',
        'Lèvres'
    ],
    '🫀': ['Cœur anatomique'],
    '🫁': ['Poumons'],
    '🦾': ['Bras mécanique'],
    '🦿': ['Jambe mécanique']
};
/*other*/
emojis_language[lang][13] = {
    '💩': [
        'Tas de Poo',
        'Terre de chien',
        'Crotte souriante'
    ],
    '💋': [
        'Marque Baiser',
        'Lèvres embrassantes'
    ],
    '💅': [
        'Pointe polonaise',
        'Doigts',
        'Manicure',
        'Nonchalant'
    ],
    '🤳': [
        'Selfie',
        'Caméra du téléphone',
        'Main de Selfie'
    ],
    '🦴': ['Os'],
    '🧳': [
        'Bagages',
        'Suitcase'
    ],
    '🌂': [
        'Parapluie fermé',
        'Parapluie réduit',
        'Parapluie rose'
    ],
    '☂️': ['Parapluie'],
    '☔': [
        'Parapluie avec gouttes de pluie',
        'Pluie',
        'Rainy'
    ],
    '🧵': ['Slot de fil de discussion'],
    '🧶': ['Balle de Yarn'],
    '💄': [
        'Lipstick',
        'Glose des lèvres',
        'Maquillage'
    ],
    '💍': [
        'Sonnerie',
        'Bague en Diamant',
        'Bague d\'engagement'
    ],
    '💥': [
        'Symbole de collision',
        'Bang',
        'Éclater',
        'Impact',
        'Étincelle rouge',
        'Explosion'
    ],
    '💫': [
        'Symbole étourdissant',
        'Cercle et étoile'
    ],
    '💦': [
        'Symbole de la sueur éclaboussante',
        'Pleins d\'appâts',
        'Éclaboussure d\'eau',
        'Gouttes d\'eau'
    ],
    '💨': [
        'Absent',
        'Rapide',
        'Vapeur',
        'Vaporisation',
        'Vent',
        'Symbole du tableau de bord',
        'Rafale de vent'
    ],
    '🌬️': [
        'Visage du vent',
        'Vent soufflant',
        'Nature Mère',
        'Visage soufflant du vent'
    ],
    '🐾': [
        'Impressions de pattes',
        'Impression de pattes de chat',
        'Impression de pattes de chien',
        'Impression de pattes de chats',
        'Impression de pattes de chiot'
    ],
    '🕸️': [
        'Toile d\'araignée',
        'Toile oblique',
        'Web',
        'Toile d\'araignée'
    ],
    '🦠': [
        'Microbe',
        'Cellule',
        'Coronavirus',
        'COVID-19',
        'Germe',
        'Microorganisme',
        'Virus'
    ],
    '💐': ['Bouquet de fleurs'],
    '🌍': [
        'Globe montrant Europe-Afrique',
        'Terre',
        'Globe',
        'Planète',
        'Monde',
        'Globe montrant l\'Europe et l\'Afrique'
    ],
    '🌎': [
        'Globe Show Americas',
        'Terre',
        'Globe',
        'Planète',
        'Monde'
    ],
    '🌏': [
        'Globe montrant l\'Asie-Australie',
        'Terre',
        'Globe',
        'Planète',
        'Monde',
        'Globe montrant l\'Asie et l\'Australie'
    ],
    '🌐': [
        'Globe avec Meridians',
        'Internet',
        'World Wide Web',
        'WWW'
    ],
    '🌑': [
        'Nouveau symbole de lune',
        'Lune Sombre',
        'Lune des ombres',
        'Éclipse solaire'
    ],
    '🌒': ['Symbole de lune du Croissant de Lune'],
    '🌓': ['Symbole du premier quart de lune'],
    '🌔': ['Symbole de la lune Gibbeuse cireuse'],
    '🌕': ['Symbole de la pleine lune'],
    '🌖': ['Symbole de la Lune Gibbous Waning'],
    '🌗': ['Symbole du dernier quart de lune'],
    '🌘': ['Symbole du Croissant de Lune Waning'],
    '🌙': ['Lune du Croissant de Lune'],
    '🌚': [
        'Nouvelle lune avec visage',
        'Lune effrayante',
        'Face de la Lune Sombre',
        'Lune de Molester'
    ],
    '🌝': [
        'Pleine Lune avec visage',
        'Moonface',
        'Émoticône de la Lune',
        'Lune souriante'
    ],
    '🌛': ['Premier quart de lune avec visage'],
    '🌜': ['Dernier quart de lune avec visage'],
    '🌞': [
        'Soleil avec visage',
        'Émoticône du Soleil',
        'Sunface',
        'Soleil souriant'
    ],
    '☀️': [
        'Dim',
        'Éclat du soleil'
    ],
    '⭐': [
        'Étoiles',
        'Étoile blanche moyenne'
    ],
    '🌟': [
        'Étoile rayonnante',
        'Étoile brillante'
    ],
    '☁️': [
        'Nuage',
        'Nuageux',
        'Overcast'
    ],
    '⛅': ['Soleil Derrière le nuage'],
    '⛈️': [
        'Nuage avec foudre et pluie',
        'Nuage de tonnerre et pluie'
    ],
    '🌤️': ['Soleil blanc avec petit nuage'],
    '🌥️': [
        'Soleil Derrière le grand nuage',
        'Soleil blanc Derrière le nuage'
    ],
    '🌦️': ['Le soleil blanc derrière le nuage avec la pluie'],
    '🌧️': ['Nuage avec pluie'],
    '🌨️': ['Nuage avec neige'],
    '🌩️': ['Nuage avec la foudre'],
    '🌪️': [
        'Tornade',
        'Nuage avec Tornade'
    ],
    '🌫️': ['Fog'],
    '🌈': [
        'Arc en Ciel',
        'Fierté Gay',
        'Arc en Ciel Primaire'
    ],
    '❄️': [
        'Flocon de neige',
        'Neige',
        'Neige'
    ],
    '🌀': [
        'Cyclone',
        'Ouragan',
        'Spirale',
        'Tourbillon',
        'Tornade'
    ],
    '⚡': [
        'Panneau à haute tension',
        'Éclair',
        'Éclair de tonnerre',
        'Tension élevée'
    ],
    '☃️': [
        'Bonhomme de neige',
        'Bonhomme de neige avec flocons'
    ],
    '⛄': [
        'Bonhomme de neige sans neige',
        'Bonhomme de neige givré',
        'Olaf',
        'Bonhomme de neige'
    ],
    '☄️': ['Comète'],
    '🔥': [
        'Feu',
        'Flamme',
        'Chaud',
        'Lit',
        'Série instantanée'
    ],
    '💧': [
        'Droplet',
        'Eau',
        'Goutte d\'eau'
    ],
    '✨': [
        'Étincelles',
        'Paillette',
        'Shiny',
        'Nouveau'
    ],
    '🌊': [
        'Vague d\'eau',
        'Plage',
        'Vague de l\'océan',
        'La mer'
    ],
    '🎄': [
        'Arbre de Noël',
        'Noël',
        'Arbre de Noël'
    ],
    '🎃': [
        'Citrouille-Lanterne',
        'Halloween',
        'Citrouille'
    ],
    '🌻': [
        'Tournesol',
        'Fleur jaune'
    ],
    '🌼': [
        'Blossom',
        'Fleur florissante',
        'Daisy',
        'Fleur jaune'
    ],
    '🌷': ['Tulip'],
    '🌹': [
        'Fleur de rose',
        'Rose Rouge'
    ],
    '☘️': [
        'Pierre de chamane',
        'Trèfle',
        'Fièvre du trésor'
    ],
    '🍀': [
        'Trèfle à quatre feuilles',
        'Trèfle',
        'L\'Irlande',
        'Chanceux',
        'Fortune'
    ],
    '🌸': [
        'Cherry Blossom',
        'Fleur rose',
        'Sakura'
    ],
    '💮': [
        'Fleur blanche',
        'Cherry Blossom',
        'Papier Doly',
        'Tampon bien fait'
    ],
    '🌺': ['Hibiscus'],
    '🏵️': ['Rosette'],
    '🌱': [
        'Graine',
        'Printemps',
        'Sprout',
        'Propulsion'
    ],
    '🌲': [
        'Arbre vert éternel',
        'Arbre du sapin',
        'Pin'
    ],
    '🌳': [
        'Arbre décimal',
        'Arbre arrondi'
    ],
    '🌴': [
        'Palmier',
        'Arbre de noix de coco'
    ],
    '🎋': [
        'Arbre de Tanabata',
        'Tanabata',
        'Arbre de vœux'
    ],
    '🎍': [
        'Décoration de pin',
        'Bambou',
        'Kadomatsu',
        'Décoration du Nouvel An'
    ],
    '🌵': [
        'Cactus',
        'Desert'
    ],
    '🌾': [
        'Feuille de riz',
        'Rogner',
        'Fermeture',
        'Blé',
        'Oreille de riz'
    ],
    '🌿': [
        'Herbe',
        'Rogner',
        'Plante'
    ],
    '🍁': [
        'Feuille d\'érable',
        'Le Canada',
        'Canadien',
        'Érable'
    ],
    '🍂': [
        'Feuille tombée',
        'Feuille d\'automne',
        'Feuilles d\'automne',
        'Feuilles marron',
        'Feuilles d\'automne',
        'Feuilles tombées'
    ],
    '🍃': [
        'Feuille flottant dans le vent',
        'Feuilles Vertes',
        'Printemps'
    ],
    '🪴': ['Plante en pot'],
    '🔑': [
        'Clés',
        'Clé d\'Or'
    ],
    '🗝️': ['Ancienne clé'],
    '🔨': [
        'Marteau',
        'Marteau de griffe',
        'Handyman',
        'Outil'
    ],
    '⛏️': [
        'Choisir',
        'Pioche'
    ],
    '🪓': ['Hache'],
    '🪚': ['Scie de menuiserie'],
    '🪛': ['Tournevis'],
    '🪝': ['Crochet'],
    '🪠': ['Plunger'],
    '🪣': ['Seau'],
    '🔩': [
        'Écrou et rouleau',
        'Bolt',
        'Vis'
    ],
    '🗜️': [
        'Ramper',
        'Vice-président de la table',
        'WinZip',
        'Compression'
    ],
    '🔒': [
        'Verrouillé',
        'Verrouillage fermé',
        'Verrouillage'
    ],
    '🔓': [
        'Déverrouillé',
        'Ouvrir le cadenas',
        'Verrouillage Ouvert'
    ],
    '🔏': [
        'Verrouillé avec stylo',
        'Verrouiller et stylo',
        'Verrouiller avec le stylet Fontaine',
        'Verrouiller avec stylo encre'
    ],
    '🔐': [
        'Verrouillé avec la clé',
        'Verrouillage fermé avec clé'
    ],
    '🗡️': [
        'Dagger',
        'Arme de couteau'
    ],
    '⚔️': ['Épées croisées'],
    '🔫': [
        'Pistol',
        'Fusil',
        'Revolver',
        'Canon d\'écureuil',
        'Pistolet à eau',
        'Pistolet d\'eau'
    ],
    '🧰': ['Boîte à outils'],
    '🧲': ['Aimant'],
    '⛓️': ['Chaînes'],
    '💉': [
        'Syringe',
        'Donation de sang',
        'Test sanguin',
        'Aiguille',
        'Vaccination'
    ],
    '🧬': [
        'ADN',
        'ADN double hélix'
    ],
    '🧪': ['Tube de test'],
    '🛏️': [
        'Lit',
        'Chambre à coucher'
    ],
    '🛋️': [
        'Canapé et Lampe',
        'Salon',
        'Réglages',
        'Canapé'
    ],
    '🪑': ['Chaise'],
    '🚪': [
        'Porte',
        'Chemin de la porte',
        'Porte avant'
    ],
    '🪟': ['Fenêtre'],
    '🪜': ['Échelle'],
    '🪞': ['Miroir'],
    '🚽': [
        'Toilette',
        'Salle de bains',
        'Loo',
        'Restructuration',
        'Toiles'
    ],
    '🚿': [
        'Douche',
        'Tête de douche'
    ],
    '🛁': [
        'Baignoire',
        'Bain de bulles'
    ],
    '🧴': ['Bouteille de lotion'],
    '🧷': ['Épingle de sécurité'],
    '🧺': ['Panier'],
    '🧻': [
        'Rouleau de Papier',
        'Papier de toilette'
    ],
    '🧼': ['Barre de savon'],
    '🧽': ['Éponge'],
    '🧯': ['Extincteur de feu'],
    '🚬': [
        'Cigarette',
        'Symbole de la fumée'
    ],
    '⚰️': [
        'Cercueil',
        'Casket',
        'Funérailles'
    ],
    '⚱️': [
        'Urne funéraire',
        'Vase'
    ],
    '🛡️': ['Bouclier'],
    '⚙️': [
        'Équipement',
        'Options',
        'Options'
    ],
    '⚖️': [
        'Échelle de solde',
        'Écailles de la justice'
    ],
    '🔗': [
        'Lier',
        'Chaîne',
        'Lien hypertexte',
        'Chaîne liée'
    ],
    '💀': [
        'Crâne',
        'Mort',
        'Crâne gris',
        'Squelette'
    ],
    '☠️': ['Crâne et Os de croix'],
    '🦻': ['Oreille avec aide auditive'],
    '🎡': [
        'Roue de Ferris',
        'Grande roue',
        'Fairgroundl',
        'Roue d’observation'
    ],
    '⛽': [
        'Pompe de carburant',
        'Pétrole',
        'Essence',
        'Gaz'
    ],
    '🚨': [
        'Voitures de police légères',
        'Lumière d\'urgence',
        'Lumière clignotante',
        'Siren'
    ],
    '🚥': ['Feu de circulation horizontal'],
    '🚦': ['Feu de circulation vertical'],
    '⚓': [
        'Schéma d\'ancrage d\'amirauté',
        'pêcheur'
    ],
    '🚧': [
        'Panneau de construction',
        'Panneau rayé noir et jaune'
    ],
    '🪂': ['Parachute'],
    '💺': [
        'Siège',
        'Avion de train d\'autobus'
    ],
    '🪐': ['Saturne'],
    '🎆': [
        'Feux d\'artifice',
        'Explosion'
    ],
    '🎇': [
        'Senko Hanabi',
        'Feu d\'artifice'
    ],
    '💷': [
        'Pound Banknote',
        'Note de 20 £',
        'Note de Vingt Quid',
        'Billets de banque avec panneau livre sterling'
    ],
    '💶': [
        'Euro Banknote',
        'Note de 100 €',
        'Billet de banque avec panneau Euro'
    ],
    '💵': [
        'Billets en dollars',
        'Note $1',
        'Billets de banque avec panneau Dollar',
        'Facture en dollars',
        'Dollar américain'
    ],
    '💴': [
        'Yen Banknote',
        'Note de 1000 yens',
        'Note de Yen',
        'Billet de banque avec panneau Yen'
    ],
    '⛺': ['Tente de Camping'],
    '⛲': [
        'Fontaine',
        'Fonctionnalité de l\'eau',
        'Fontaine d\'eau',
        'Stationner'
    ],
    '💰': [
        'Porte-monnaie',
        'Sacs d\'argent',
        'Riche',
        '$'
    ],
    '🏷️': ['Étiquette'],
    '🔖': [
        'Marque-page',
        'Tags de prix',
        'Étiquette'
    ],
    '📑': ['Onglets de signet'],
    '📯': [
        'Corne postale',
        'Bugle',
        'Corne française'
    ],
    '🪁': ['Cerf-volant'],
    '🪀': ['Yo-Yo'],
    '🎊': [
        'Confetti Ball',
        'Confetti'
    ],
    '🎉': [
        'Popper de fête',
        'Célébration',
        'Chapeau de fête'
    ],
    '🎈': [
        'Ballon',
        'Soirée',
        'Ballon rouge'
    ],
    '🌡️': [
        'Thermomètre',
        'Météo chaude',
        'Température'
    ],
    '🛢️': ['Tambour de pétrole'],
    '💣': ['Bombe'],
    '🔪': [
        'Couteau de cuisine',
        'Couteau',
        'Couteau de boucher',
        'Hocho'
    ],
    '🏺': [
        'Amphora',
        'Jar',
        'Vase'
    ],
    '🗺️': ['Carte du Monde'],
    '🎼': [
        'Score musical',
        'Partitions musicales',
        'Treble Clef'
    ],
    '🎬': [
        'Planche de clappe',
        'Couper',
        'Action',
        'Directeur',
        'Ardoise de film'
    ],
    '🎻': [
        'Violon',
        'Quatuor à cordes',
        'Violon le plus petit du monde'
    ],
    '🎺': [
        'Trumpet',
        'Corne',
        'Jazz.'
    ],
    '🎸': [
        'Guitare',
        'Guitare acoustique',
        'Guitare basses',
        'Guitare électrique'
    ],
    '🎷': [
        'Saxophone',
        'Jazz.',
        'Sax'
    ],
    '🥁': [
        'Tambour',
        'Tambour à bâtonnets'
    ],
    '🎹': [
        'Piano',
        'Clavier musical'
    ],
    '🪕': ['Famille Banjo'],
    '🪗': ['Accordéon'],
    '🪘': ['Tambour long'],
    '🎨': [
        'Palette d\'artistes',
        'Art',
        'Peinture'
    ],
    '🧩': [
        'Pièce de puzzle',
        'Pièce de puzzle de scie puzzle'
    ],
    '🎲': [
        'Mort de jeu',
        'Dés'
    ],
    '🎭': [
        'Arts de la scène',
        'Théâtre',
        'Théatre',
        'Masques de drame',
        'Masques de théâtre grec',
        'Masques tragiques et comiques',
        'Venise'
    ],
    '🔍': [
        'Agrandissement du verre incliné à gauche',
        'Magnifier',
        'Icône de recherche',
        'Verre Magnifiant à gauche',
        'Chercher'
    ],
    '🔎': [
        'Loupe de verre inclinée à droite',
        'Magnifier',
        'Icône de recherche',
        'Verre Magnifiant à pointe droite',
        'Chercher'
    ],
    '⌛': [
        'Verre horaire terminé',
        'Verre horaire'
    ],
    '⏳': [
        'Verre horaire non fait',
        'Verre d\'hiver avec sable fluo'
    ],
    '🕐': [
        'Une horloge O\'Clock',
        'Horloge Face One'
    ],
    '🕜': ['Face d\'horloge - Trente'],
    '🕑': [
        'Deux horloges',
        'Clock Face Deux O\' Horloge'
    ],
    '🕝': ['Face de l\'horloge deux fois trente'],
    '🕒': [
        'Trois horloges',
        'Horloge Trois O-Horloge'
    ],
    '🕞': ['Visage de l\'horloge trois-trente'],
    '🕓': [
        'Quatre horloges',
        'Horloge Face Quatre O’Horloge'
    ],
    '🕟': ['Visage de l\'horloge Quatre-trente'],
    '🕔': [
        'Cinq horloges',
        'Horloge Face Cinq O’Horloge'
    ],
    '🕠': ['Face d\'horloge cinq-trente'],
    '🕕': [
        'Six horloges',
        'Horloge Face Six O’Horloge'
    ],
    '🕡': ['Face d\'horloge - Six-Trente'],
    '🕖': [
        'Sept horloges',
        'Clock Face Sept O\' Horloge'
    ],
    '🕢': ['Face d\'horloge Seven-Trente'],
    '🕗': [
        'Huit O’Horloge',
        'Horloge face à huit heures'
    ],
    '🕣': ['Visage de l\'horloge des Huit - Trente'],
    '🕘': [
        'Neuf horloges',
        'Horloge Neuf face à l\'horloge'
    ],
    '🕤': ['Visage de l\'horloge à neuf et trente'],
    '🕙': [
        'Dix horloges',
        'Face de l\'horloge Dix fois Horloge'
    ],
    '🕥': ['Face de l\'horloge Dix Trente'],
    '🕚': [
        'Onze horloge O\'\'',
        'Horloge Face Onze'
    ],
    '🕦': ['Face d\'horloge Onze - Trente'],
    '🕛': [
        'Douze O\'Horloge',
        'Face de l\'horloge Douze O\'Clock'
    ],
    '🕧': ['Face de l\'horloge - Douze - Trente'],
    '♨️': [
        'Sources thermales',
        'Onsen',
        'Vapeur'
    ],
    '💈': [
        'Pôle de Barbier',
        'Barbier',
        'Rayures du Barbier',
        'Coiffeur'
    ],
    '🎪': [
        'Tente de cirque',
        'Gros haut',
        'Circus'
    ],
    '💢': [
        'Symbole de colère',
        'Panneau de Colère',
        'Pop de Veine'
    ],
    '🗯️': [
        'Bulle de Colère Droite',
        'Zig Zag Bubble',
        'Bulle de parole en colère'
    ],
    '💊': [
        'Pill',
        'Capsule',
        'Tablette',
        'Médicaments'
    ],
    '🎎': [
        'Poupées',
        'Hinamatsuri',
        'Poupées Impériales',
        'Poupées Japonaises'
    ],
    '🎏': [
        'Streamer carpe',
        'Drapeau de poisson',
        'Koinobori',
        'Chaussettes de Vent'
    ],
    '🎐': [
        'Jeton de vent',
        'Furin',
        'Méduse',
        'Cloche du vent'
    ],
    '🎀': [
        'Arc en ruban',
        'Arc rose',
        'Arc'
    ],
    '🎁': [
        'Cadeau emballé',
        'Cadeau d\'anniversaire',
        'Cadeau de Noël',
        'Cadeau',
        'Coffret Cadeau',
        'Présent',
        'Cadeau emballé'
    ],
    '🎫': [
        'Billet',
        'Billet de ticket',
        'Ticket de Tour du Monde'
    ],
    '🃏': [
        'Joker',
        'Carte Joker',
        'Carte de jeu Joker Noir'
    ],
    '💡': [
        'Ampoule',
        'Idée',
        'Ampoule électrique'
    ],
    '🔦': [
        'Lampe de poche',
        'Torche',
        'Torche électrique'
    ],
    '🏮': [
        'Lanterne de Papier Rouge',
        'Lanterne asiatique',
        'Lanterne japonaise',
        'Lanterne rouge',
        'Lanterne d\'Izakaya'
    ],
    '📦': [
        'Paquet',
        'Boîte',
        'Colis'
    ],
    '📇': [
        'Index de la carte',
        'Rolodex',
        'Carte système',
        'Index de la carte'
    ],
    '🕳️': ['Trou'],
    '👁️‍🗨️': [
        'Je suis un témoin',
        'Bulle de chat Oeil'
    ],
    '⚜️': [
        'Fleur-de-lis',
        'Eclaireurs',
        'Saints de la Nouvelle-Orléans'
    ],
    '⚗️': ['Alembic'],
    '🕯️': ['Bougie'],
    '🖼️': [
        'Image encadrée',
        'Peinture',
        'Cadre d\'image',
        'Cadre avec image'
    ],
    '🛍️': ['Sacs de vente'],
    '🎟️': ['Billets d\'entrée'],
    '🛎️': ['Bellhop Bell'],
    '🧾': ['Reçu'],
    '🧮': ['Abacus'],
    '🧫': ['Plat de Petri'],
    '🧹': [
        'Balai',
        'Brosse',
        'Balayage'
    ],
    '🎰': [
        'Machine à Sous',
        'Casino',
        'Machine à fruits',
        'Jeu',
        'Machine de Poker'
    ],
    '🦽': ['Fauteuil roulant manuel'],
    '🦼': ['Fauteuil roulant motorisé'],
    '🪔': ['Lampe Diya'],
    '🦯': ['White Cane'],
    '🩸': ['Abandon de sang'],
    '🩹': ['Bandage adhésif'],
    '🩺': ['stéthoscope'],
    '🪶': ['Plume'],
    '🪤': ['Piège à souris'],
    '🫖': ['Teapot'],
    '🪨': ['Roche'],
    '🪵': ['Bois'],
    '🪄': ['Baguette magique'],
    '🪅': ['Épinglé'],
    '🪆': [
        'Poupées imbriquées',
        'Matryoshka'
    ],
    '🪡': ['Aiguille de couture'],
    '🪢': ['Nœud'],
    '🪙': ['Pièce'],
    '🪃': ['Boomerang'],
    '🪥': ['Brosse à dents'],
    '🪦': ['Pierre de Tête'],
    '🪧': ['Placard'],
    '🪒': ['Razor']
};