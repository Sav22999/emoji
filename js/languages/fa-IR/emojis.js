// descriptions took from Emojipedia.org
lang = 'fa';
//use i18n language code of the browser, more details: https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/API/i18n/LanguageCode
titles_language[lang] = {
    '🔍': 'نتایج',
    '🕒': 'شکلک های پر استفاده',
    '😀': 'لبخند',
    '🙋': 'مردم',
    '🦊': 'حیوانات',
    '🅰️': 'نمادها',
    '🍎': 'غذا و نوشیدنی‌ها',
    '🏳️‍🌈': 'پرچم ها',
    '🏊': 'ورزش ها',
    '✈️': 'سفر و اماکن',
    '🖱️': 'فن آوری ها و اداره',
    '👗': 'پوشاک و لوازم جانبی',
    '🖐️': 'دست ها و اعضای بدن',
    '🛎️': 'ديگر'
};
emojis_language[lang] = [];
/*search*/
emojis_language[lang][0] = {};
/*smileys*/
emojis_language[lang][2] = {
    '😀': [
        'صورت پوزخند',
        ':)',
        'صورت شاد',
        'صورت خندان'
    ],
    '😃': [
        'صورت پوزخند با چشمانی بزرگ',
        ':D',
        'صورت شاد',
        'صورت خندان',
        'صورت خندان با دهان باز'
    ],
    '😄': [
        'صورت پوزخند با چشمانی خندان',
        'صورت شاد',
        'صورت خندان',
        'صورت پوزخند با چشمانی نیمه بسته',
        'صورت خندان با دهان باز و چشمانی خندان'
    ],
    '😁': [
        'صورت با چشمانی خندان',
        'صورت پوزخند با چشمانی خندان'
    ],
    '😆': [
        'صورت پوزخند با چشمانی نیمه بسته',
        'xD',
        '><',
        'لبخند بزرگ',
        'لبخند چشم بسته',
        'خندیدن',
        'صورت پوزخند با چشمانی محکم بسته شده',
        'صورت خندان با دهان باز و چشمانی محکم بسته شده'
    ],
    '😅': [
        'صورت پوزخند عرق کرده',
        'تمرین',
        'لبخند عرق کرده',
        'صورت پوزخند با چشمانی نیمه بسته شده و عرق کرده',
        'صورت خندان با دهان باز و عرق سرد'
    ],
    '🤣': [
        'ROFL',
        'از شدت خنده روی زمین پیچ و تاب خوردن'
    ],
    '😂': [
        'صورت با اشک شوق',
        'خندیدن',
        'گریه از شدت خنده',
        'اشک شوق',
        'LOL'
    ],
    '🙂': [
        'صورت کمی خندان',
        ':)',
        'کمی خوشحال',
        'این خوبه'
    ],
    '🙃': [
        'صورت برعکس',
        'طعنه',
        'احمقانه',
        'صورت برعکس'
    ],
    '😉': [
        'صورت چشمک زن',
        ';)',
        'چشمک',
        'صورت چشمک',
        'صورت چشمک زن'
    ],
    '😊': [
        'صورت خندان با چشمانی خندان',
        '^^',
        'صورت شاد',
        'لبخند',
        'صورت لبخند زن'
    ],
    '😇': [
        'صورت خندان با هاله',
        'فرشته',
        'هاله'
    ],
    '🥰': [
        'صورت خندان با قلب ها',
        'صورت عاشق',
        'صورت خندان با چشمانی خندان و سه قلب'
    ],
    '😍': [
        'صورت خندان با چشمان قلبی',
        '*.*',
        'چشمان قلبی',
        'صورت قلبی',
        'صورت خندان با چشمان قلبی'
    ],
    '🤩': [
        'ستاره زده',
        'هیجان زده',
        'چشمان ستاره ای',
        'چشم ستاره ای',
        'صورت Wow',
        'چهره با چشمان ستاره ای',
        'چهره خندان با چشمان ستاره ای'
    ],
    '😘': [
        'صورت درحال بوسه زدن',
        ':*',
        ':-*',
        'بوسه بزن',
        'بوسه زدن',
        'بوسیدن',
        'صورت درحال پرتاب یک بوسه'
    ],
    '😗': [
        'صورت بوسه زن',
        ':*',
        ':-*',
        'صورت اردکی',
        'صورت بوسه زن',
        'سوت زدن'
    ],
    '☺️': [
        'صورت خندان',
        'صورت شاد',
        'صورت خندان',
        'لبخند زدن',
        'صورت لبخند سفید'
    ],
    '😚': [
        'صورت بوسه زن با چشمانی بسته',
        'صورت بوسه',
        'صورت بوسه زن'
    ],
    '😙': [
        'صورت در حال بوسه زدن با چشمانی خندان',
        'صورت بوسه',
        'بوسه ای',
        'سوت',
        'سوت زدن'
    ],
    '😋': [
        'صورت غذای خوش طعم',
        'مسخره',
        'گرسنه',
        'صورت خندان درحال لیسیدن لب ها',
        'صورت درحال چشیدن غذای لذیذ',
        'یام یام'
    ],
    '😛': [
        'صورت با زبان',
        ':P',
        'پر رو',
        'صورت زبان',
        'زبان بیرون آمده',
        'صورت با زبان بیرون گیر کرده'
    ],
    '😜': [
        'صورت چشمک زن با زبان',
        ';P',
        'دیوانه',
        'صورت دیوانه',
        'صورت چشمک زن با زبان بیرون گیر کرده',
        'صورت با زبان بیرون گیر کرده و چشم چشمک زن'
    ],
    '🤪': [
        'صورت مسخره',
        'صورت دیوانه',
        'هیجان زده',
        'وحشی',
        'صورت مسخره',
        'صورت پوزخند با یک چشم بزرگ و یک چشم کوچک'
    ],
    '😝': [
        'صورت چشم نیمه باز با زبان',
        'xP',
        'زبان بیرون آمده',
        'صورت با زبان بیرون گیر کرده و چشم های محکم بسته شده',
        'صورت با زبان بیرون گیر کرده و چشم های محکم بسته شده'
    ],
    '🤑': [
        'صورت با دهان پولی',
        ':$',
        'چشمان علامت دلار',
        'صورت پول',
        'پولدار'
    ],
    '🤗': [
        'صورت بغل کردن',
        'بغل',
        'بغل کردن',
        'آغوش ها',
        'صورت شاد با دستان درحال بغل کردن'
    ],
    '🤭': [
        'اوه',
        'صورت با دست روی دهان',
        'صورت لبخندزنان با چشمان لبخندزنان و دست درحال پوشیدن دهان'
    ],
    '🤫': [
        'هیس',
        'سکوت کن',
        'صورت هیس',
        'هیسس',
        'ساکت',
        'سکوت',
        'ساکت',
        'صورت با انگشت پوشاننده لب های بسته'
    ],
    '🤔': [
        'چی',
        '?',
        'هومم',
        'صورت درحال فکر کردن',
        'انگشت شست روی چانه',
        'متفکر',
        'سایه انداختن'
    ],
    '🤐': [
        'زیپ',
        'صورت دهان زیپ شده',
        'لب های مهر و موم شده',
        'لب مهر و موم شده',
        'دهنتو ببند',
        'صورت با یک دهان زیپ شده'
    ],
    '🤨': [
        'صورت با ابروی بالا زده',
        'کولبرت',
        'راک',
        'صورت با ابروی بالا زده',
        'صورت با یک ابروی بالازده'
    ],
    '😐': [
        'چهره خنثی',
        ':|',
        'صورت با دهان صاف',
        'صورت صاف'
    ],
    '😑': [
        'چهره بی بیان',
        '-_-',
        'صورت با دهان صاف',
        'صورت صاف'
    ],
    '😶': [
        'صورت بدون دهان',
        'صورت خالی',
        'بی دهان',
        'سکوت',
        'ساکت'
    ],
    '😏': [
        'چهره پوزخند زنان',
        'لاس زدن',
        'صورت سکسی',
        'چهره از خود راضی',
        'لبخند وسوسه کننده'
    ],
    '😒': [
        'چهره بی حوصله',
        'ناراضی',
        'هعی',
        'چشم کناری',
        'تحت تاثیر قرار نگرفته'
    ],
    '🙄': [
        'صورت با چشمان چرخان',
        'چرخش چشم'
    ],
    '😬': [
        'صورت پوزخند',
        'بی دست و پا',
        'Eek',
        'پا در دهان',
        'مضطرب',
        'بهترین دوست شماره 1 اسنپ چت متقابل'
    ],
    '🤥': [
        'چهره دروغگو',
        'دروغ گو',
        'بینی بلند',
        'پینوکیو'
    ],
    '😌': [
        'صورت تسکین یافته',
        'محتوا',
        'راضی'
    ],
    '😔': [
        'متأسفم',
        'چهره افسرده',
        'افسرده',
        'ناراحت',
        'چهره غمگین',
        'غمگین',
        'چهره متفکر غمگین'
    ],
    '😪': [
        'صورت خواب آلود',
        'اشک جانبی',
        'حباب آب بینی'
    ],
    '🤤': [
        'بزاق',
        'صورت آب دهان',
        'آب دهان'
    ],
    '😴': [
        'صورت خواب',
        'خروپف',
        'صورت خواب',
        'خروپف کردن',
        'صورت خروپف کنان'
    ],
    '🥱': ['صورت خمیازه'],
    '😷': [
        'صورت با ماسک پزشکی',
        'ویروس کرونا',
        'COVID-19',
        'صورت ماسک',
        'ماسک جراحی'
    ],
    '🤒': [
        'صورت با دماسنج',
        'Ill',
        'بیمار'
    ],
    '🤕': [
        'صورت با بانداژ سر',
        'سر بانداژ شده',
        'دست و پا چلفتی',
        'زخمی'
    ],
    '🤢': [
        'صورت تهوع',
        'تنفر',
        'صورت سبز',
        'استفراغ'
    ],
    '🤮': [
        'صورت درحال استفراغ کردن',
        'قی کردن',
        'Bleah',
        'Bleurgh',
        'Yuk',
        'اوه',
        'اوق',
        'قی کردن',
        'بالا اوردن',
        'استفراغ',
        'صورت با دهان باز درحال استفراغ کردن'
    ],
    '🤧': [
        'صورت عطسه',
        'سلامتی'
    ],
    '🥵': [
        'صورت داغ',
        'صورت بیش از حد گرم شده'
    ],
    '🥶': [
        'صورت سرد',
        'خنک',
        'خنک',
        'صورت یخ زده'
    ],
    '🥴': [
        'صورت گیج و منگ',
        'صورت مست',
        'صورت با چشم های ناهموار و دهان موج دار'
    ],
    '😵': [
        'صورت گیج',
        'صورت چشم های متقاطع',
        'صورت چشم های مارپیچی'
    ],
    '🤯': [
        'انفجار سر',
        'ذهن منفجر شده',
        'چهره شوکه شده با سر در حال انفجار'
    ],
    '🤠': [
        'صورت کلاه کابوی',
        'کابوی',
        'صورت کابوی',
        'چهره با کلاه کابوی'
    ],
    '🥳': [
        'صورت جشن',
        'صورت جشن',
        'چهره با فوت فوتک و کلاه مهمانی'
    ],
    '😎': [
        'چهره خندان با عینک آفتابی',
        'باحال',
        'بهترین دوستان متقابل (اسنپ چت)',
        'عینک آفتابی'
    ],
    '🤓': [
        'صورت خرخون',
        'بی عرضه'
    ],
    '🧐': ['صورت با مونوکل'],
    '😕': [
        'چهره گیج',
        ':/',
        ':S',
        'بی حوصله',
        'متحیر'
    ],
    '😟': [
        'چهره نگران',
        ':(',
        'ناراحت',
        'چهره ناراحت'
    ],
    '🙁': [
        'صورت کمی اخم کرده',
        ':(',
        'کمی غمگین'
    ],
    '☹️': [
        'چهره اخم کرده',
        ':(',
        'خیلی اخم کردن',
        'صورت اخم سفید'
    ],
    '😮': [
        'واو',
        ':O',
        'یا خدا!',
        'چهره با دهان باز',
        'دهان باز',
        'صورت متعجب با دهان باز'
    ],
    '😯': [
        'چهره خاموش',
        ':O',
        'غافلگیری',
        'چهره غافلگیر شده'
    ],
    '😲': [
        'چهره شوکه شده',
        ':O',
        'واو',
        'چهره حیرت زده',
        'صورت مست',
        'صورت نفس زنان'
    ],
    '😳': [
        'صورت برافروخته',
        ':$',
        'صورت سرخ شده',
        'خجالت زده',
        'شرمنده',
        'چهره با چشمان کاملا باز'
    ],
    '🥺': [
        'صورت ملتمسانه',
        ':(',
        'التماس کردن',
        'چشم های براق',
        'کصلیس',
        'چهره ای با چشم های ملتمسانه'
    ],
    '😦': [
        'چهره اخم کرده با دهان باز',
        'D:',
        'خمیازه کشیدن'
    ],
    '😨': [
        'ترسیده',
        'D:',
        'چهره ترسیده',
        'شگفت زده'
    ],
    '😰': [
        'چهره مضطرب با عرق',
        'صورت آبی',
        'چهره نگران',
        'صورت مضطرب',
        'صورت با دهان باز و عرق سرد'
    ],
    '😥': [
        'چهره غمگین اما آسوده',
        ':\'(',
        'عرق ابرو',
        'چهره ناامید اما آسوده'
    ],
    '😢': [
        'صورت گریان',
        ':\'(',
        'گریان',
        'اشک'
    ],
    '😭': [
        'چهره گریان با صدای بلند',
        'داد و بیداد کردن',
        'گریان',
        'اشک از روی غم',
        'گریه کردن'
    ],
    '😱': [
        'فریاد چهره از ترس',
        'تنها در خانه',
        'جیغ',
        'صورت فریاد زنان'
    ],
    '😖': [
        'چهره گیج',
        'دهان لرزان',
        'صورت به هم فشرده شده'
    ],
    '😣': [
        'چهره ماندگار',
        'صورت بیچاره',
        'چشم های به هم فشرده شده'
    ],
    '😞': [
        'چهره ناامید',
        ':(',
        'ناراحت',
        'چهره ناراحت'
    ],
    '😓': [
        'چهره افسرده با عرق',
        'کار سخت',
        'صورت با عرق از روی ناراحتی',
        'چهره با عرق سرد'
    ],
    '😩': [
        'چهره خسته',
        'چهره پریشان',
        'ناله کردن'
    ],
    '😫': [
        'چهره خسته',
        'خسته',
        'خسته شده',
        'چهره پریشان'
    ],
    '😤': [
        'صورت با بخار از بینی',
        'بازدم عمیق از روی نارضایتی',
        'ناامید',
        'صورت عصبی',
        'بازدم عمیق',
        'صورت عصبی با بازدم عمیق'
    ],
    '😡': [
        'صورت اخم کرده',
        'چهره عصبانی',
        'چهره بداخلاق',
        'صورت عصبی',
        'صورت قرمز'
    ],
    '😠': [
        'چهره عصبانی',
        'عصبانی',
        'چهره بداخلاق'
    ],
    '🤬': [
        'فحش دادن',
        '@#$%&!',
        'چهره با نمادهای روی دهان',
        'ناسزاگویی',
        'دشنام دادن',
        'Grawlix',
        'چهره با نمادهای روی دهان',
        'چهره جدی با نمادهایی که دهان را می پوشاند'
    ],
    '🥲': ['چهره خندان با اشک'],
    '😮‍💨': [
        'صورت بازدم کنان',
        'آه'
    ],
    '😵‍💫': [
        'صورت با چشم های مارپیچ',
        'صورت گیج'
    ],
    '🥸': ['صورت نمایشی'],
    '🫠': ['صورت ذوب شده'],
    '🫢': ['صورت با چشمان باز و دست روی دهان'],
    '🫣': ['چهره با چشم نیم نگاهی'],
    '🫡': [
        'چهره سلام نظامی',
        'نظامی',
        'ارتش',
        'نیروی هوایی بریتانیای کبیر',
        'نیروی دریایی'
    ],
    '🫥': ['صورت نقطه ای'],
    '🫤': ['صورت با دهان مورب'],
    '🥹': ['صورت درحال نگهداشتن اشک'],
    '😈': [
        'چهره خنده شیطانی با شاخ ها',
        'شیطان',
        'شاخ شیطانی',
        'شیطان شاد',
        'شیطان بنفش',
        'شیطان قرمز'
    ],
    '👿': [
        'چهره عصبانی با شاخ',
        'شیطان',
        'شاخ شیطانی',
        'شیطان بنفش',
        'گابلین بنفش',
        'شیطان ناراحت',
        'بچه شیطان'
    ],
    '🤡': [
        'چهره دلقک',
        'دلقک گوشت تلخ',
        'دلقک شیطانی',
        'دلقک ترسناک'
    ],
    '👽': [
        'آدم فضایی',
        'ET'
    ],
    '👾': [
        'آدم فضایی هیولا',
        'مهاجم فضایی',
        'هیولای بازی ویدیویی'
    ],
    '🤖': [
        'ربات',
        'صورت ربات'
    ],
    '👹': [
        'غول',
        'صورت ماسک',
        'اونی (افسانه ژاپنی)',
        'هیولای سرخ',
        'هیولای ژاپنی'
    ],
    '👻': [
        'روح',
        'هالووین'
    ],
    '🗣️': [
        'سر درحال صحبت کردن',
        'توضیح دادن',
        'فریاد',
        'داد زدن',
        'سر درحال صحبت کردن در سایه نما'
    ],
    '👤': [
        'نیم تنه در سایه نما',
        'سایه',
        'سایه نما',
        'کاربر',
        'سایه نمای شخص'
    ],
    '🫂': ['مردم در آغوش گرفتن'],
    '👥': [
        'نیم تنه ها در سایه نما',
        'سایه ها',
        'سایه نما ها',
        'کاربران',
        'سایه نمای دو شخص'
    ]
};
/*people*/
emojis_language[lang][3] = {
    '👶{{skin_tone}}': [
        'کودک',
        'بچه',
        'کودک تازه به راه افتاده'
    ],
    '🧒{{skin_tone}}': [
        'بچه',
        'کودک جنسیت خنثی'
    ],
    '👧{{skin_tone}}': ['دختر'],
    '🧑{{skin_tone}}': [
        'شخص',
        'بزرگسال جنسیت خنثی',
        'بزرگسال'
    ],
    '👱{{skin_tone}}': [
        'شخص: موهای بلوند',
        'بلوند',
        'طلایی',
        'مرد بلوند',
        'فردی با موهای بلوند',
        'مردی با موهای بلوند'
    ],
    '👨{{skin_tone}}': [
        'مرد',
        'آقا',
        'مرد سبیل'
    ],
    '🧔{{skin_tone}}': [
        'مرد: ریش',
        'ریش',
        'مرد ریشو',
        'شخص ریشو'
    ],
    '👨{{skin_tone}}‍🦰': [
        'مرد: موهای قرمز',
        'مرد زنجبیلی',
        'مرد مو قرمز',
        'مردی با موهای قرمز'
    ],
    '👨{{skin_tone}}‍🦱': [
        'مرد: موهای مجعد',
        'مردی با موهای مجعد'
    ],
    '👨{{skin_tone}}‍🦳': [
        'مرد: موهای سفید',
        'مردی با موهای خاکستری'
    ],
    '👨{{skin_tone}}‍🦲': [
        'مرد: کچل',
        'مرد بدون مو'
    ],
    '👩{{skin_tone}}': [
        'زن',
        'زن',
        'خانم',
        'زن زرد'
    ],
    '👩{{skin_tone}}‍🦰': [
        'زن: موهای قرمز',
        'زن زنجبیلی',
        'زن مو قرمز',
        'زن با موهای قرمز'
    ],
    '👩{{skin_tone}}‍🦱': [
        'زن: موهای مجعد',
        'زن با موهای مجعد'
    ],
    '👩{{skin_tone}}‍🦳': [
        'زن: موهای سفید',
        'زن با موهای خاکستری'
    ],
    '👩{{skin_tone}}‍🦲': [
        'زن: کچل',
        'زن بدون مو'
    ],
    '👱{{skin_tone}}‍♀️': [
        'زن: موهای بلوند',
        'زن با موهای بلوند'
    ],
    '👱{{skin_tone}}‍♂️': [
        'مرد: موهای بلوند',
        'مردی با موهای بلوند'
    ],
    '🧓{{skin_tone}}‍': [
        'مسن تر',
        'بزرگسال پیر جنسیت خنثی'
    ],
    '👴{{skin_tone}}‍': [
        'پیرمرد',
        'مرد مسن',
        'پدربزرگ',
        'مرد مسن تر'
    ],
    '👵{{skin_tone}}‍': [
        'پیرزن',
        'زن مسن',
        'مادربزرگ',
        'مادربزرگ',
        'خانم پیر',
        'زن مسن تر'
    ],
    '👼{{skin_tone}}‍': [
        'بچه فرشته',
        'فرشته',
        'کروب',
        'کوپیدو',
        'پوتو'
    ],
    '🎅{{skin_tone}}‍': [
        'بابانوئل',
        'نیکلاس مقدس',
        'سینترکلاس',
        'پدر کریسمس'
    ],
    '🤶{{skin_tone}}‍': [
        'خانم کلاوس',
        'مادر کریسمس'
    ],
    '🤴{{skin_tone}}‍': ['شاهزاده'],
    '👸{{skin_tone}}‍': [
        'شاهدخت',
        'دختر بلوند',
        'دختر با تاج',
        'دختر با تاج'
    ],
    '🫅{{skin_tone}}‍': ['شخص با تاج'],
    '👳{{skin_tone}}‍♂️': ['مرد عمامه پوش'],
    '👳{{skin_tone}}‍♀️': ['زن عمامه پوش'],
    '👲{{skin_tone}}‍': [
        'شخص با کلاه',
        'مرد آسیایی',
        'مردی با کلاه چینی',
        'مردی با گوا پی مائو'
    ],
    '🧕{{skin_tone}}‍': [
        'زنی با روسری',
        'حجاب',
        'فرد با روسری'
    ],
    '🤵{{skin_tone}}‍': [
        'شخصی با لباس تاکسیدو',
        'داماد',
        'مرد با کت و شلوار',
        'مرد اسموکینگ پوشیده',
        'ازدواج'
    ],
    '👰{{skin_tone}}‍': [
        'شخص با پوشش عروس',
        'عروس',
        'عروس با پوشش',
        'ازدواج'
    ],
    '👰{{skin_tone}}‍♂️': [
        'مرد با پوشش عروس',
        'ازدواج'
    ],
    '👰{{skin_tone}}‍♀️': [
        'زن با پوشش عروس',
        'ازدواج'
    ],
    '👩{{skin_tone}}‍🍼': ['زن در حال تغذیه نوزاد'],
    '👨{{skin_tone}}‍🍼': ['مرد در حال تغذیه کودک'],
    '🥷': ['نینجا'],
    '🤰{{skin_tone}}‍': [
        'زن باردار',
        'بارداری',
        'خانم حامله'
    ],
    '🫃{{skin_tone}}‍': [
        'مرد باردار',
        'بارداری'
    ],
    '🫄{{skin_tone}}‍': [
        'فرد باردار',
        'بارداری'
    ],
    '🤱{{skin_tone}}‍': [
        'شیر دادن',
        'شیردهی'
    ],
    '🦸{{skin_tone}}‍♂️': ['مرد ابرقهرمان'],
    '🦸{{skin_tone}}‍‍♀️': ['ابرقهرمان زن'],
    '🦹{{skin_tone}}‍‍♂️': ['مرد ابرشرور'],
    '🦹{{skin_tone}}‍‍♀️': ['زن ابرشرور'],
    '🧙{{skin_tone}}‍': [
        'جادوگر',
        'جادوگر',
        'جادوگر',
        'جادوگر',
        'جادوگر'
    ],
    '🧙{{skin_tone}}‍‍♀️': [
        'زن جادوگر',
        'جادوگر',
        'جادوگر'
    ],
    '🧚{{skin_tone}}‍‍♂️': ['مرد پری'],
    '🧚{{skin_tone}}‍♀️': ['زن پری'],
    '🧛{{skin_tone}}‍♂️': [
        'مرد خون آشام',
        'دراکولا'
    ],
    '🧛{{skin_tone}}‍‍♀️': ['زن خون آشام'],
    '🧜{{skin_tone}}‍♂️': ['مرد دریایی'],
    '🧜{{skin_tone}}‍♀️': ['پری دریایی'],
    '🧝{{skin_tone}}‍♂️': ['مرد الف'],
    '🧝{{skin_tone}}‍♀️': ['زن الف'],
    '🧞‍♂️': ['مرد جن'],
    '🧞‍♀️': ['زن جن'],
    '🕵️{{skin_tone}}‍♂️': [
        'کارآگاه مرد',
        'چشم خصوصی',
        'کارآگاه',
        'جاسوس',
        'مرد کارآگاه',
        'کاراگاه یا جاسوس',
        '007',
        'بازرس'
    ],
    '🕵️{{skin_tone}}‍♀️': ['کارآگاه زن'],
    '💂{{skin_tone}}‍': [
        'نگهبان',
        'نگهبان بریتانیایی',
        'نگهبان پیاده‌رو'
    ],
    '💂{{skin_tone}}‍‍♀️': ['نگهبان زن'],
    '👮{{skin_tone}}‍': [
        'مامور پلیس',
        'پلیس',
        'پلیس',
        'مرد پلیس',
        'زن پلیس'
    ],
    '👮{{skin_tone}}‍‍♀️': ['زن افسر پلیس'],
    '👨{{skin_tone}}‍🚒': ['مرد آتش نشان'],
    '👩{{skin_tone}}‍‍🚒': ['زن آتش نشان'],
    '👨{{skin_tone}}‍‍✈️': ['مرد خلبان'],
    '👩{{skin_tone}}‍‍✈️': ['زن خلبان'],
    '👨{{skin_tone}}‍💻': [
        'مرد مختصص تکنولوژی',
        'مرد مختصص تکنولوژی'
    ],
    '👩{{skin_tone}}‍💻': ['زن مختصص تکنولوژی'],
    '👨{{skin_tone}}‍🎤': [
        'مرد خواننده',
        'بووی',
        'علاءالدین صانع'
    ],
    '👩{{skin_tone}}‍🎤': ['زن خواننده'],
    '👨{{skin_tone}}‍🎨': ['مرد هنرمند'],
    '👩{{skin_tone}}‍🎨': ['مرد هنرمند'],
    '👷{{skin_tone}}‍‍♂️': ['مرد کارگر ساختمانی'],
    '👷{{skin_tone}}‍‍♀️': ['زن کارگر ساختمانی'],
    '👨{{skin_tone}}‍‍🚀': [
        'مرد فضانورد',
        'مرد فضانورد'
    ],
    '👩{{skin_tone}}‍‍🚀': [
        'زن فضانورد',
        'زن فضانورد'
    ],
    '👨{{skin_tone}}‍‍🔬': ['مرد دانشمند'],
    '👩{{skin_tone}}‍‍🔬': ['زن دانشمند'],
    '👨{{skin_tone}}‍💼': [
        'مرد کارمند اداری',
        'مدیر عامل',
        'بازرگان'
    ],
    '👩{{skin_tone}}‍💼': [
        'زن کارمند اداری',
        'بازرگان زن',
        'مدیر عامل'
    ],
    '👨{{skin_tone}}‍🏭': ['کارگر کارخانه مرد'],
    '👩{{skin_tone}}‍‍🏭': ['زن کارگر کارخانه'],
    '👨{{skin_tone}}‍‍🔧': ['مرد مکانیک'],
    '👩{{skin_tone}}‍‍🔧': ['مکانیک زن'],
    '👨{{skin_tone}}‍‍🍳': [
        'مرد درحال پختن غذا',
        'مرد آشپز',
        'سرآشپز مرد'
    ],
    '👩{{skin_tone}}‍‍🍳': [
        'زن درحال پختن غذا',
        'زن آشپز',
        'سرآشپز زن'
    ],
    '👨{{skin_tone}}‍‍🌾': ['مرد کشاورز'],
    '👩{{skin_tone}}‍‍🌾': ['زن کشاورز'],
    '👨{{skin_tone}}‍‍⚖️': ['مرد قاضی'],
    '👩{{skin_tone}}‍‍⚖️': ['قاضی زن'],
    '👨{{skin_tone}}‍‍🏫': ['معلم مرد'],
    '👩{{skin_tone}}‍‍🏫': ['معلم زن'],
    '👨{{skin_tone}}‍🎓': ['دانشجوی مرد'],
    '👩{{skin_tone}}‍🎓': ['دانشجوی زن'],
    '👨{{skin_tone}}‍‍⚕️': [
        'کارمند بهداشت مرد',
        'کارمند بهداشت مرد',
        'پرستار مرد'
    ],
    '👩{{skin_tone}}‍⚕️': [
        'کارمند بهداشت زن',
        'دکتر خانم',
        'پرستار زن'
    ],
    '🤷{{skin_tone}}‍♂️': [
        'مردی که شانه بالا می اندازد',
        'مرد نمیدانم',
        'نمیدانم'
    ],
    '🤷{{skin_tone}}‍♀️': [
        'زنی که شانه بالا می اندازد',
        'زن نمیدانم',
        'نمیدانم'
    ],
    '🤦{{skin_tone}}‍♂️': ['دست زدن به صورت مرد'],
    '🤦{{skin_tone}}‍♀️': ['دست زدن به صورت زن'],
    '🙇{{skin_tone}}‍‍♂️': ['مرد درحال تعظیم'],
    '🙇{{skin_tone}}‍‍♀️': ['زن درحال تعظیم'],
    '🧏{{skin_tone}}‍♂️': ['مرد ناشنوا'],
    '🧏{{skin_tone}}‍‍♀️': ['زن ناشنوا'],
    '🙋{{skin_tone}}‍‍♂️': [
        'مرد بالا بردن دست',
        'مرد شادی که یک دست را بالا می برد'
    ],
    '🙋{{skin_tone}}‍♀️': [
        'زن در حال بالا بردن دست',
        'زن شادی که یک دستش را بالا می برد'
    ],
    '💁{{skin_tone}}‍♂️': [
        'مرد بالا بردن کف دست',
        'مرد میز اطلاعات'
    ],
    '💁{{skin_tone}}‍♀️': [
        'زن بالا بردن کف دست',
        'زن میز اطلاعات'
    ],
    '🙆{{skin_tone}}‍♂️': ['مرد درحال گرفتن ژست "خوبه"'],
    '🙆{{skin_tone}}‍♀️': ['زن درحال گرفتن ژست "خوبه"'],
    '🙅{{skin_tone}}‍♂️': ['مرد درحال گرفتن ژست "نه"'],
    '🙅{{skin_tone}}‍♀️': ['زن درحال گرفتن ژست "نه"'],
    '🙎{{skin_tone}}‍♂️': ['مرد درحال اخم کردن'],
    '🙎{{skin_tone}}‍‍♀️': ['زن درحال اخم کردن'],
    '🙍{{skin_tone}}‍‍♂️': ['مرد اخم کرده'],
    '🙍{{skin_tone}}‍‍♀️': ['زن اخم کرده'],
    '🧟‍‍♂️': ['مرد زامبی'],
    '🧟‍‍♀️': ['زن زامبی'],
    '🚶{{skin_tone}}‍♂️': ['مرد درحال راه رفتن'],
    '🚶{{skin_tone}}‍♀️': ['زن درحال راه رفتن'],
    '🧍{{skin_tone}}‍‍♂️': ['مرد ایستاده'],
    '🧍{{skin_tone}}‍‍♀️': ['زن ایستاده'],
    '🧎{{skin_tone}}‍♂️': ['مرد زانو زده'],
    '🧎{{skin_tone}}‍‍♀️': ['زن زانو زده'],
    '👨{{skin_tone}}‍🦯': ['مرد با عصای سفید'],
    '👩{{skin_tone}}‍🦯': ['زن با عصای سفید'],
    '👨{{skin_tone}}‍🦼': ['مرد با ویلچر موتوری'],
    '👩{{skin_tone}}‍🦼': ['زن با ویلچر موتوری'],
    '👨{{skin_tone}}‍🦽': ['مرد با ویلچر دستی'],
    '👩{{skin_tone}}‍🦽': ['زن با ویلچر دستی'],
    '👯‍♂️': ['مردان با گوش های خرگوشی'],
    '👯‍♀️': ['زنان با گوش های خرگوشی'],
    '🕴️{{skin_tone}}‍': [
        'فرد معلق با کت و شلوار',
        'مرد معلق',
        'پسر بی‌ادب',
        'والت جابسکو',
        'مرد معلق با کت و شلوار رسمی'
    ],
    '🧖{{skin_tone}}‍♂️': [
        'مردی در اتاق بخار',
        'مردی در سونا'
    ],
    '🧖{{skin_tone}}‍♀️': [
        'زنی در اتاق بخار',
        'زن در سونا'
    ],
    '👫{{skin_tone}}‍': [
        'زن و مرد دست در دست',
        'زوج دگرجنسگرا',
        'زوج استریت',
        'مرد و زن دست در دست'
    ],
    '👭{{skin_tone}}‍': [
        'زوج لزبین',
        'زنان دست در دست',
        'دو زن دست در دست'
    ],
    '👬{{skin_tone}}‍': [
        'مردان دست در دست',
        'زوج مرد همجنس گرا',
        'دو مرد دست در دست'
    ],
    '👨‍👩‍👦': [
        'خانواده با مادر، پدر و پسر',
        'خانواده: مرد، زن، پسر'
    ],
    '👨‍👩‍👧': [
        'خانواده با مادر، پدر و دختر',
        'خانواده: مرد، زن، دختر'
    ],
    '👨‍👩‍👧‍👦': [
        'خانواده با مادر، پدر، پسر و دختر',
        'Family: Man, Woman, Girl, Boy'
    ],
    '👨‍👩‍👦‍👦': [
        'Family With Mother, Father and Two Sons',
        'Family: Man, Woman, Boy, Boy'
    ],
    '👨‍👩‍👧‍👧': [
        'Family With Mother, Father and Two Daughters',
        'Family: Man, Woman, Girl, Girl'
    ],
    '👨‍👨‍👦': [
        'Family With Two Fathers and Son',
        'Family: Man, Man, Boy'
    ],
    '👨‍👨‍👧': [
        'Family With Two Fathers and Daughter',
        'Family: Man, Man, Girl'
    ],
    '👨‍👨‍👧‍👦': [
        'Family With Two Fathers, Son and Daughter',
        'Family: Man, Man, Girl, Boy'
    ],
    '👨‍👨‍👦‍👦': [
        'Family With Two Fathers and Two Sons',
        'Family: Man, Man, Boy, Boy'
    ],
    '👨‍👨‍👧‍👧': [
        'Family With Two Fathers and Two Daughters',
        'Family: Man, Man, Girl, Girl'
    ],
    '👩‍👩‍👦': [
        'Family With Two Mothers and Son',
        'Family: Woman, Woman, Boy'
    ],
    '👩‍👩‍👧': [
        'Family With Two Mothers and Daughter',
        'Family: Woman, Woman, Girl'
    ],
    '👩‍👩‍👧‍👦': [
        'Family With Two Mothers, Son and Daughter',
        'Family: Woman, Woman, Girl, Boy'
    ],
    '👩‍👩‍👦‍👦': [
        'Family With Two Mothers and Two Sons',
        'Family: Woman, Woman, Boy, Boy'
    ],
    '👩‍👩‍👧‍👧': [
        'Family With Two Mothers and Two Daughters',
        'Family: Woman, Woman, Girl, Girl'
    ],
    '👨‍👦': [
        'Family With Father and Son',
        'Family: Man, Boy'
    ],
    '👨‍👦‍👦': [
        'Family: Man, Boy, Boy',
        'Family With Father and Two Sons'
    ],
    '👨‍👧': [
        'Family With Father and Daughter',
        'Family: Man, Girl'
    ],
    '👨‍👧‍👦': [
        'Family With Father, Son and Daughter',
        'Family: Man, Girl, Boy'
    ],
    '👨‍👧‍👧': [
        'Family With Father and Two Daughters',
        'Family: Man, Girl, Girl'
    ],
    '👩‍👦': [
        'Family With Mother and Son',
        'Family: Woman, Boy'
    ],
    '👩‍👦‍👦': [
        'Family With Mother and Two Sons',
        'Family: Woman, Boy, Boy'
    ],
    '👩‍👧': [
        'Family With Mother and Daughter',
        'Family: Woman, Girl'
    ],
    '👩‍👧‍👦': [
        'Family With Mother, Son and Daughter',
        'Family: Woman, Girl, Boy'
    ],
    '👩‍👧‍👧': [
        'Family With Mother and Two Daughters',
        'Family: Woman, Girl, Girl'
    ],
    '💑': [
        'Couple with Heart',
        'Couple In Love',
        'Loving Couple',
        'Man and Woman With Heart'
    ],
    '👨‍❤️‍👨': [
        'Two Men With Heart',
        'Couple with Heart: Man, Man'
    ],
    '👩‍❤️‍👩': [
        'Two Women With Heart',
        'Couple with Heart: Woman, Woman'
    ],
    '💆{{skin_tone}}‍‍♂️': ['Man Getting Head Massage'],
    '💆{{skin_tone}}‍♀️': ['Woman Getting Head Massage'],
    '💇{{skin_tone}}‍‍♂️': ['Man Getting Haircut'],
    '💇{{skin_tone}}‍‍♀️': ['Woman Getting Haircut'],
    '🛌{{skin_tone}}‍': [
        'Sleeping Accommodation',
        'Person in Bed',
        'Person Sleeping'
    ],
    '🧌': ['Troll'],
    '👺': [
        'Goblin',
        'Long Nose Face',
        'Red Mask',
        'Tengu',
        'Japanese Goblin'
    ]
};
/*animals*/
emojis_language[lang][4] = {
    '😺': [
        'Grinning Cat',
        ':)',
        'Happy Cat',
        'Smiling Cat',
        'Happy Cat Face',
        'Smiling Cat Face with Open Mouth'
    ],
    '😸': [
        'Grinning Cat with Smiling Eyes',
        ':D',
        'Grinning Cat',
        'Happy Cat',
        'Grinning Cat Face'
    ],
    '😹': [
        'Cat with Tears of Joy',
        ':\')',
        'Happy Tears Cat',
        'Laughing Cat'
    ],
    '😻': [
        'Smiling Cat with Heart-Eyes',
        '*.*',
        'Heart Eyes Cat',
        'Loving Cat',
        'Smiling Cat Face with Heart-Shaped Eyes'
    ],
    '😼': [
        'Cat with Wry Smile',
        'Smirking Cat',
        'Smirking Cat Face',
        'Cat Face with Wry Smile'
    ],
    '😽': [
        'Kissing Cat',
        ':*',
        'Kissing Cat Face',
        'Kissing Cat Face with Closed Eyes'
    ],
    '🙀': [
        'Weary Cat',
        ':O',
        'Scared Cat',
        'Screaming Cat',
        'Cat Face Screaming in Fear',
        'Weary Cat Face'
    ],
    '😿': [
        'Crying Cat',
        ':\'(',
        'Sad Cat',
        'Crying Cat Face'
    ],
    '😾': [
        'Pouting Cat',
        'Grumpy Cat',
        'Pouting Cat Face'
    ],
    '🙈': [
        'Monkey',
        'See-No-Evil Monkey',
        'Mizaru',
        'Monkey Covering Eyes'
    ],
    '🙉': [
        'Monkey',
        'Hear-No-Evil Monkey',
        'Kikazaru',
        'Monkey Covering Ears'
    ],
    '🙊': [
        'Monkey',
        'Speak-No-Evil Monkey',
        'Iwazaru',
        'Monkey Covering Mouth',
        'No Speaking'
    ],
    '🐵': [
        'Monkey',
        'Monkey Face',
        'Monkey Head'
    ],
    '🐒': [
        'Monkey',
        'Cheeky Monkey'
    ],
    '🦍': ['Gorilla'],
    '🐶': [
        'Dog Face',
        'Dog',
        'Puppy'
    ],
    '🐕': [
        'Dog',
        'Doggo',
        'Akita Inu'
    ],
    '🦮': [
        'Seeing Eye Dog',
        'Guide Dog'
    ],
    '🐕‍🦺': ['Service Dog'],
    '🐩': [
        'Poodle',
        'Dog',
        'Miniature Poodle',
        'Standard Poodle',
        'Toy Poodle'
    ],
    '🐺': [
        'Wolf',
        'Wolf Face'
    ],
    '🦊': [
        'Fox',
        'Fox Face'
    ],
    '🦝': ['Raccoon'],
    '🐱': [
        'Cat Face',
        ':3',
        'Kitten',
        'Kitty'
    ],
    '🐈': [
        'Cat',
        'Domestic Cat',
        'Feline',
        'Housecat'
    ],
    '🐈‍⬛': ['Black Cat'],
    '🦁': [
        'Lion',
        'Lion Face'
    ],
    '🐯': [
        'Tiger Face',
        'Cute Tiger'
    ],
    '🐅': [
        'Tiger',
        'Bengal Tiger'
    ],
    '🐆': [
        'Leopard',
        'African Leopard',
        'Jaguar'
    ],
    '🐴': [
        'Horse Face',
        'Horse Head'
    ],
    '🐎': [
        'Horse',
        'Galloping Horse',
        'Racehorse'
    ],
    '🦄': [
        'Unicorn',
        'Unicorn Face'
    ],
    '🦓': [
        'Zebra',
        'Zebra Face'
    ],
    '🦌': [
        'Deer',
        'Buck',
        'Reindeer',
        'Stag'
    ],
    '🐮': [
        'Cow Face',
        'Cow',
        'Happy Cow'
    ],
    '🐂': [
        'Ox',
        'Bull',
        'Bullock',
        'Oxen',
        'Steer'
    ],
    '🐃': [
        'Water Buffalo',
        'Buffalo',
        'Domestic Water Buffalo'
    ],
    '🐄': [
        'Cow',
        'Dairy Cow'
    ],
    '🐷': [
        'Pig Face',
        'Pig',
        'Pig Head'
    ],
    '🐖': [
        'Pig',
        'Hog',
        'Sow'
    ],
    '🐗': [
        'Boar',
        'Warthog',
        'Wild Boar',
        'Wild Pig'
    ],
    '🐽': ['Pig Nose'],
    '🐏': [
        'Ram',
        'Sheep'
    ],
    '🐑': [
        'Ewe',
        'Lamb',
        'Sheep'
    ],
    '🐐': ['Goat'],
    '🐪': [
        'Camel',
        'Arabian Camel',
        'Dromedary Camel',
        'One-Bump Camel'
    ],
    '🐫': [
        'Two-Hump Camel',
        'Asian Camel',
        'Bactrian Camel'
    ],
    '🦙': [
        'Llama',
        'Alpaca'
    ],
    '🦒': [
        'Giraffe',
        'Giraffe Face'
    ],
    '🐘': ['Elephant'],
    '🦏': [
        'Rhinoceros',
        'Rhino'
    ],
    '🦛': [
        'Hippopotamus',
        'Hippo'
    ],
    '🐭': [
        'Mouse Face',
        'Mouse'
    ],
    '🐁': [
        'Mouse',
        'Dormouse',
        'Rodent',
        'Mice'
    ],
    '🐀': [
        'Rat',
        'Rodent'
    ],
    '🐹': [
        'Hamster',
        'Hamster Face'
    ],
    '🐰': [
        'Rabbit Face',
        'Easter Bunny'
    ],
    '🐇': [
        'Rabbit',
        'Bunny',
        'Bunny Rabbit'
    ],
    '🐿️': [
        'Chipmunk',
        'Squirrel'
    ],
    '🦔': ['Hedgehog'],
    '🦇': [
        'Bat',
        'Batman'
    ],
    '🐻': [
        'Bear',
        'Bear Face',
        'Teddy Bear'
    ],
    '🐨': [
        'Koala',
        'Koala Face'
    ],
    '🐼': [
        'Panda',
        'Panda Face'
    ],
    '🦘': [
        'Kangaroo',
        'Roo'
    ],
    '🦡': ['Badger'],
    '🦃': [
        'Turkey',
        'Thanksgiving Turkey',
        'Wild Turkey'
    ],
    '🐔': [
        'Chicken',
        'Hen'
    ],
    '🐓': [
        'Cock',
        'Rooster',
        'Cockerel'
    ],
    '🐣': [
        'Hatching Chick',
        'Baby Chicken',
        'Chick Hatching'
    ],
    '🐤': [
        'Baby Chick',
        'Yellow Bird'
    ],
    '🐥': [
        'Front-Facing Baby Chick',
        'Baby Chick',
        'Standing Chick'
    ],
    '🐦': ['Bird'],
    '🐧': ['Penguin'],
    '🕊️': [
        'Dove',
        'Dove of Peace'
    ],
    '🦅': [
        'Eagle',
        'Bald Eagle'
    ],
    '🦆': ['Duck'],
    '🦢': ['Swan'],
    '🦉': ['Owl'],
    '🦚': ['Peacock'],
    '🦜': ['Parrot'],
    '🐸': [
        'Frog',
        'Toad',
        'Frog Face'
    ],
    '🐊': [
        'Crocodile',
        'Alligator',
        'Croc'
    ],
    '🐢': [
        'Turtle',
        'Tortoise'
    ],
    '🦎': [
        'Lizard',
        'Gecko'
    ],
    '🐍': [
        'Snake',
        'Serpent'
    ],
    '🐲': [
        'Dragon Face',
        'Dragon Head'
    ],
    '🐉': ['Dragon'],
    '🦕': [
        'Sauropod',
        'Brachiosaurus',
        'Brontosaurus',
        'Dinosaur'
    ],
    '🦖': [
        'T-Rex',
        'Tyrannosaurus Rex'
    ],
    '🐳': [
        'Spouting Whale',
        'Cute Whale'
    ],
    '🐋': ['Whale'],
    '🐬': ['Dolphin'],
    '🐟': [
        'Fish',
        'Freshwater Fish'
    ],
    '🐠': [
        'Tropical Fish',
        'Fish',
        'Yellow-Blue Fish'
    ],
    '🐡': [
        'Blowfish',
        'Fugu',
        'Pufferfish'
    ],
    '🦈': [
        'Shark',
        'Great White Shark'
    ],
    '🐙': ['Octopus'],
    '🐚': [
        'Spiral Shell',
        'Seashell',
        'Shell'
    ],
    '🐌': [
        'Snail',
        'Slug',
        'Garden Snail'
    ],
    '🦋': ['Butterfly'],
    '🐛': [
        'Bug',
        'Caterpillar',
        'Insect'
    ],
    '🐜': [
        'Ant',
        'Bug',
        'Insect'
    ],
    '🐝': [
        'Honeybee',
        'Bee',
        'Bumblebee'
    ],
    '🐞': [
        'Lady Beetle',
        'Ladybird',
        'Ladybug',
        'Bug'
    ],
    '🦗': [
        'Cricket',
        'Grasshopper'
    ],
    '🕷️': ['Spider'],
    '🦂': ['Scorpion'],
    '🦟': ['Mosquito'],
    '🦀': ['Crab'],
    '🦞': ['Lobster'],
    '🦐': [
        'Shrimp',
        'Prawn'
    ],
    '🦑': ['Squid'],
    '🦧': ['Orangutan'],
    '🦥': ['Sloth'],
    '🦦': ['Otter'],
    '🦨': ['Skunk'],
    '🦬': ['Bison'],
    '🦣': ['Mammoth'],
    '🦫': ['Beaver'],
    '🐻‍❄️': ['Polar Bear'],
    '🦤': ['Dodo'],
    '🦭': ['Seal'],
    '🪲': ['Beetle'],
    '🪳': ['Cockroach'],
    '🪰': ['Fly'],
    '🪱': ['Worm'],
    '🦩': ['Flamingo'],
    '🪹': [
        'Empty Nest',
        'Birds',
        'Home'
    ],
    '🪺': [
        'Nest with Eggs',
        'Birds',
        'Home'
    ]
};
/*symbols*/
emojis_language[lang][5] = {
    '💘': [
        'Heart with Arrow',
        'Cupid Arrow',
        'Lovestruck'
    ],
    '💝': [
        'Heart with Ribbon',
        'Chocolate Box',
        'Gift Box',
        'Gift Heart'
    ],
    '💖': [
        'Sparkling Heart',
        'Sparkle Heart',
        'Sparkly Heart',
        'Stars Heart'
    ],
    '💗': [
        'Growing Heart',
        'Multiple Heart',
        'Triple Heart'
    ],
    '💓': [
        'Beating Heart',
        'Heart Alarm',
        'Heartbeat',
        'Wifi Heart'
    ],
    '💞': [
        'Revolving Hearts',
        'Two Hearts'
    ],
    '💕': [
        'Two Hearts',
        'Small Hearts',
        'Two Pink Hearts'
    ],
    '💟': ['Heart Decoration'],
    '❣️': [
        'Heart Exclamation',
        'Heart Above Dot',
        'Red Heart As an Exclamation Mark',
        'Heavy Heart Exclamation Mark Ornament'
    ],
    '💔': [
        'Broken Heart',
        'Breaking Heart',
        'Brokenhearted',
        'Heart Broken'
    ],
    '❤️': [
        'I Love You',
        'Red Heart',
        'Heart',
        'Love Heart',
        'Red Heart'
    ],
    '🧡': [
        'Orange Heart',
        'I Love You'
    ],
    '💛': [
        'Yellow Heart',
        '#1 BF Snapchat',
        'Gold Heart',
        'I Love You'
    ],
    '💚': [
        'Green Heart',
        'Jealous Heart',
        'I Love You'
    ],
    '💙': [
        'Blue Heart',
        'I Love You'
    ],
    '💜': [
        'Purple Heart',
        'I Love You'
    ],
    '🖤': [
        'Black Heart',
        'Dark Heart',
        'I Love You'
    ],
    '🤍': [
        'White Heart',
        'I Love You'
    ],
    '🤎': [
        'Brown Heart',
        'I Love You'
    ],
    '💯': [
        'Hundred Points',
        '100',
        '%',
        'Keep It 100',
        'Perfect Score',
        'Hundred Points Symbol'
    ],
    '💬': [
        'Speech Balloon',
        'Chat Bubble',
        'Speech Bubble'
    ],
    '💭': [
        'Thought Balloon',
        'Thinking Bubble',
        'Thought Bubble'
    ],
    '💤': [
        'Zzz',
        'Bedtime',
        'Boring',
        'Sleep',
        'Sleeping Sign',
        'Sleeping Symbol'
    ],
    '♠️': [
        'Spade Suit',
        'Spades',
        'Black Spade Suit'
    ],
    '♥️': [
        'Heart Suit',
        'Card With Heart',
        'Hearts',
        'Black Heart Suit'
    ],
    '♦️': [
        'Diamond Suit',
        'Diamonds',
        'Black Diamond Suit'
    ],
    '♣️': [
        'Club Suit',
        'Clubs',
        'Black Club Suit'
    ],
    '🏧': [
        'ATM Sign',
        'ATM',
        'Automated Telleer Machine'
    ],
    '🎧': [
        'Headphone',
        'Earphone',
        'iPod',
        'Headphones'
    ],
    '🎵': [
        'Musical Note',
        'Beamed Pair Of Eighth Notes',
        'Beamed Pair Of Quavers',
        'Music Note'
    ],
    '🎶': [
        'Musical Notes',
        'Music',
        'Music Notes',
        'Singing',
        'Multiple Musical Notes'
    ],
    '🔔': [
        'Bell',
        'Liberty Bell',
        'Ringer',
        'Wedding Bell'
    ],
    '🔕': [
        'Bell with Slash',
        'Notifications',
        'Ringer Disabled',
        'Muted Bell',
        'Bell with Cancellation Stroke'
    ],
    '🔈': [
        'Speaker Low Volume',
        'Volume',
        'Speaker'
    ],
    '🔉': [
        'Speaker Medium Volume',
        'Reduce Volume',
        'Speaker With Medium Volume',
        'Speaker with One Sound Wave'
    ],
    '🔊': [
        'Speaker High Volume',
        'Increase Volume',
        'Speaker with Three Sound Waves'
    ],
    '🔇': [
        'Muted Speaker',
        'Mute Volume',
        'Speaker with Cancellation Stroke'
    ],
    '📢': [
        'Loudspeaker',
        'Announcement',
        'PA System',
        'Public Address Loudspeaker',
        ''
    ],
    '⛔': ['No Entry'],
    '🚫': [
        'Prohibited',
        'Banned',
        'Circle Backslash',
        'No',
        'Red Circle Crossed',
        'Restricted',
        'No Entry Sign'
    ],
    '🚳': [
        'No Bicycles',
        'No Bikes Sign'
    ],
    '🚭': [
        'No Smoking',
        'No Smoking Symbol'
    ],
    '🚯': [
        'No Littering',
        'Do Not No Litter Symbol'
    ],
    '🚱': [
        'Non-Potable Water',
        'No Water',
        'No Drinking Water',
        'Non-Potable Water Symbol'
    ],
    '🚷': [
        'No Pedestrians',
        'No People',
        'No Walking'
    ],
    '🔞': [
        'No One Under Eighteen',
        'NSFW',
        'No One Under Eighteen Symbol',
        '18'
    ],
    '📵': [
        'No Mobile Phones',
        'No Cell Phones',
        'No Phones',
        'No Smartphones'
    ],
    '☢️': [
        'Radioactive',
        'International Radiation Symbol',
        'Nuclear',
        'Radioactive Sign'
    ],
    '☣️': [
        'Biohazard',
        'Biohazard Sign'
    ],
    '⬆️': [
        'Up Arrow',
        'Arrow Pointing Up',
        'Upwards Black Arrow'
    ],
    '↗️': [
        'Up-Right Arrow',
        'Diagonal Up-Right Arrow',
        'North East Arrow'
    ],
    '➡️': [
        'Right Arrow',
        'Arrow Pointing Right',
        'Black Rightwards Arrow'
    ],
    '↘️': [
        'Down-Right Arrow',
        'Diagonal Down-Right Arrow',
        'South East Arrow'
    ],
    '⬇️': [
        'Down Arrow',
        'Arrow Pointing Down',
        'Downwards Black Arrow'
    ],
    '↙️': [
        'Down-Left Arrow',
        'Diagonal Down-Left Arrow',
        'South West Arrow'
    ],
    '⬅️': [
        'Left Arrow',
        'Arrow Pointing Left',
        'Leftwards Black Arrow'
    ],
    '↖️': [
        'Up-Left Arrow',
        'Diagonal Up-Left Arrow',
        'North West Arrow'
    ],
    '↕️': [
        'Up-Down Arrow',
        'Vertical Arrows',
        'Up Down Arrow'
    ],
    '↔️': [
        'Left-Right Arrow',
        'Horizontal Arrows',
        'Sideways Arrows',
        'Left Right Arrow'
    ],
    '↩️': [
        'Right Arrow Curving Left',
        'Email Reply',
        'Left Curved Arrow',
        'Leftwards Arrow with Hook'
    ],
    '↪️': [
        'Left Arrow Curving Right',
        'Email Forward',
        'Right Curved Arrow',
        'Rightwards Arrow with Hook'
    ],
    '⤴️': [
        'Right Arrow Curving Up',
        'Arrow Pointing Rightwards Then Curving Upwards'
    ],
    '⤵️': [
        'Right Arrow Curving Down',
        'Curved Down Arrow',
        'Arrow Pointing Rightwards Then Curving Downwards'
    ],
    '🔃': [
        'Clockwise Vertical Arrows',
        'Clockwise Downwards and Upwards Open Circle Arrows'
    ],
    '🔄': [
        'Counterclockwise Arrows Button',
        'Refresh',
        'Rotate',
        'Switch',
        'Counter-Clockwise Arrows',
        'Anticlockwise Downwards and Upwards Open Circle Arrows'
    ],
    '🔙': [
        'Back Arrow',
        'Back',
        'Back with Leftwards Arrow Above'
    ],
    '🔚': [
        'End Arrow',
        'End',
        'End with Leftwards Arrow Above'
    ],
    '🔛': [
        'On! Arrow',
        'On',
        'On with Exclamation Mark with Left Right Arrow Above'
    ],
    '🔜': [
        'Soon Arrow',
        'Soon',
        'Soon with Rightwards Arrow Above'
    ],
    '🔝': [
        'Top Arrow',
        'Top',
        'Top with Upwards Arrow Above'
    ],
    '🛐': [
        'Place of Worship',
        'Religious Building'
    ],
    '⚛️': ['Atom Symbol'],
    '🕉️': [
        'Om Symbol',
        'Om',
        'Aumkara',
        'Omkara',
        'Pranava'
    ],
    '✡️': [
        'Star of David',
        'Magen David'
    ],
    '☸️': [
        'Wheel of Dharma',
        'Helm'
    ],
    '☯️': ['Yin Yang'],
    '✝️': [
        'Latin Cross',
        'Christian Cross'
    ],
    '☦️': ['Orthodox Cross'],
    '☪️': ['Star and Crescent'],
    '☮️': [
        'Peace Symbol',
        'Peace Sign'
    ],
    '🕎': [
        'Menorah',
        'Candelabrum',
        'Candles',
        'Chanukiah',
        'Menorah',
        'Menorah with Nine Branches'
    ],
    '🔯': [
        'Dotted Six-Pointed Star',
        'Six Pointed Star with Middle Dot'
    ],
    '♈': [
        'Aries',
        'Zodiac'
    ],
    '♉': [
        'Taurus',
        'Zodiac'
    ],
    '♊': [
        'Gemini',
        'Zodiac'
    ],
    '♋': [
        'Cancer',
        'Zodiac'
    ],
    '♌': [
        'Leo',
        'Zodiac'
    ],
    '♍': [
        'Virgo',
        'Zodiac'
    ],
    '♎': [
        'Libra',
        'Zodiac'
    ],
    '♏': [
        'Scorpio',
        'Scorpius',
        'Zodiac'
    ],
    '♐': [
        'Sagittarius',
        'Zodiac'
    ],
    '♑': [
        'Capricorn',
        'Zodiac'
    ],
    '♒': [
        'Aquarius',
        'Zodiac'
    ],
    '♓': [
        'Pisces',
        'Zodiac'
    ],
    '⛎': [
        'Ophiuchus',
        'Zodiac'
    ],
    '⏭️': [
        'Black Right-Pointing Double Triangle with Vertical Bar',
        'Skip Forward Symbol',
        'Next Track Button'
    ],
    '⏮️': [
        'Black Left-Pointing Double Triangle with Vertical Bar',
        'Skip Backward Symbol',
        'Last Track Button',
        'Previous Track'
    ],
    '⏯️': [
        'Black Right-Pointing Triangle with Double Vertical Bar',
        'Play or Pause Button',
        'Play Symbol',
        'Pause Symbol'
    ],
    '🔀': [
        'Shuffle Tracks Button',
        'Twisted Rightwards Arrows'
    ],
    '🔁': [
        'Repeat Button',
        'Loop Symbol',
        'Retweet',
        'Clockwise Rightwards and Leftwards Open Circle Arrows'
    ],
    '🔂': [
        'Repeat Single Button',
        'Circle Arrows With Number 1',
        'Loop Once Symbol',
        'Repeat Single Track Symbol',
        'Clockwise Rightwards and Leftwards Open Circle Arrows with Circled One Overlay'
    ],
    '▶️': [
        'Play Button',
        'Right Triangle',
        'Right-Pointing Triangle',
        'Black Right-Pointing Triangle'
    ],
    '◀️': [
        'Reverse Button',
        'Left Triangle',
        'Left-Pointing Triangle',
        'Black Left-Pointing Triangle'
    ],
    '🔼': [
        'Upwards Button',
        'Up Triangle',
        'Up-Pointing Triangle',
        'Up-Pointing Small Red Triangle'
    ],
    '🔽': [
        'Downwards Button',
        'Down Triangle',
        'Down-Pointing Triangle',
        'Down-Pointing Small Red Triangle'
    ],
    '⏩': [
        'Fast-Forward Button',
        'Fast Forward',
        'Fast Forward Symbol',
        'Black Right-Pointing Double Triangle'
    ],
    '⏪': [
        'Fast Reverse Button',
        'Rewind',
        'Rewind Symbol',
        'Black Left-Pointing Double Triangle'
    ],
    '⏫': [
        'Fast Up Button',
        'Up-Pointing Double Triangle',
        'Black Up-Pointing Double Triangle'
    ],
    '⏬': [
        'Fast Down Button',
        'Down-Pointing Double Triangle',
        'Black Down-Pointing Double Triangle'
    ],
    '⏹️': [
        'Stop Button',
        'Stop',
        'Stop Symbol',
        'Black Square for Stop'
    ],
    '⏏️': [
        'Eject Button',
        'Eject Symbol'
    ],
    '🎦': [
        'Cinema',
        'Cinema Screen',
        'Movies',
        'Film',
        'Cinema Symbol',
        'Recording'
    ],
    '📶': [
        'Antenna Bars',
        'Reception Bars',
        'Signal Strength',
        'Stairs',
        'Signal Strength Symbol',
        'Antenna with Bars'
    ],
    '✖️': [
        'Multiply',
        'Multiplication Symbol',
        'Heavy Multiplication X'
    ],
    '➕': [
        'Plus',
        'Plus Symbol',
        'Heavy Plus Sign'
    ],
    '➖': [
        'Minus',
        'Minus Symbol',
        'Heavy Minus Sign'
    ],
    '➗': [
        'Divide',
        'Division Symbol',
        'Heavy Division Sign'
    ],
    '🟰': [
        'Heavy Equals Sign',
        'Result'
    ],
    '♾️': [
        'Infinity',
        'Permanent Paper Sign'
    ],
    '‼️': [
        'Double Exclamation Mark',
        'Red Double Exclamation Mark'
    ],
    '⁉️': [
        'Exclamation Question Mark',
        'Red Exclamation Mark and Question Mark'
    ],
    '❓': [
        'Question Mark',
        'Red Question Mark',
        'Black Question Mark Ornament'
    ],
    '❔': [
        'White Question Mark',
        'White Question Mark',
        'White Question Mark Ornament'
    ],
    '❗': [
        'Exclamation Mark',
        'Red Exclamation Mark',
        'Heavy Exclamation Mark Symbol'
    ],
    '❕': [
        'White Exclamation Mark',
        'White Exclamation Mark Ornament'
    ],
    '⭕': [
        'Hollow Red Circle',
        'Circle',
        'Correct',
        'Red Circle',
        'Circle Mark',
        'Heavy Large Circle'
    ],
    '☑️': [
        'Check Box with Check',
        'V',
        'Checkbox',
        'Check Mark In Box',
        'Ballot Box with Check'
    ],
    '✔️': [
        'Check Mark',
        'V',
        'Check',
        'Tick',
        'Checkmark',
        'Heavy Check Mark'
    ],
    '❌': [
        'Cross Mark',
        'Cross',
        'X'
    ],
    '✅': [
        'Check Mark Button',
        'V',
        'Green Check Mark',
        'Green Tick',
        'Check Mark',
        'White Heavy Check Mark'
    ],
    '❎': [
        'Cross Mark Button',
        'Cross',
        'X',
        'X Mark',
        'Negative Squared Cross Mark'
    ],
    '#️⃣': [
        'Keycap Number Sign',
        '#',
        'Hash Key',
        'Hashtag',
        'Octothorpe',
        'Pound Key',
        'Number Sign'
    ],
    '*️⃣': [
        'Keycap Asterisk',
        'Star Keycap'
    ],
    '0️⃣': [
        'Number 0',
        'Keycap Digit Zero'
    ],
    '1️⃣': [
        'Number 1',
        'Keycap Digit One'
    ],
    '2️⃣': [
        'Number 2',
        'Keycap Digit Two'
    ],
    '3️⃣': [
        'Number 3',
        'Keycap Digit Three'
    ],
    '4️⃣': [
        'Number 4',
        'Keycap Digit Four'
    ],
    '5️⃣': [
        'Number 5',
        'Keycap Digit Five'
    ],
    '6️⃣': [
        'Number 6',
        'Keycap Digit Six'
    ],
    '7️⃣': [
        'Number 7',
        'Keycap Digit Seven'
    ],
    '8️⃣': [
        'Number 8',
        'Keycap Digit Eight'
    ],
    '9️⃣': [
        'Number 09',
        'Keycap Digit Nine'
    ],
    '🔟': [
        'Number 10',
        'Keycap: 10',
        '10',
        'Keycap Digit Ten'
    ],
    '🔠': [
        'Input Latin Uppercase',
        'ABCD',
        'Uppercase',
        'Input Symbol for Uppercase Letters',
        'Input Symbol for Latin Capital Letters'
    ],
    '🔡': [
        'Input Latin Lowercase',
        'abcd',
        'Lowercase',
        'Input Symbol for Lowercase Letters',
        'Input Symbol for Latin Small Letters'
    ],
    '🔢': [
        'Input Numbers',
        '1234',
        'Numbers',
        'Numeric Input',
        'Input Symbol for Numbers'
    ],
    '🔣': [
        'Input Symbol for Symbols',
        'Symbols',
        'Symbol Input'
    ],
    '🔤': [
        'Input Latin Letters',
        'abc',
        'Alphabet',
        'Letters',
        'Input Symbol for Letters',
        'Input Symbol for Latin Letters'
    ],
    '🅰️': [
        'A Button (Blood Type)',
        'Blood Type A',
        'Letter A',
        'Negative Squared Latin Capital Letter A'
    ],
    '🆎': [
        'AB Button (Blood Type)',
        'Letters AB',
        'Blood Type AB',
        'Negative Squared AB'
    ],
    '🅱️': [
        'B Button (Blood Type)',
        'Letter B',
        'Blood Type B',
        'Negative Squared Latin Capital Letter B'
    ],
    '🆑': [
        'CL Button',
        'Clear',
        'Clear Button',
        'CL Sign',
        'Squared CL',
        'Letters CL'
    ],
    '🆒': [
        'Cool Button',
        'Cool Square',
        'Cool Sign',
        'Squared Cool'
    ],
    '🆓': [
        'Free Button',
        'Free',
        'Free Sign',
        'Squared Free'
    ],
    'ℹ️': [
        'Information',
        'Info',
        'Lowercase I',
        'Tourist Information',
        'Information Source'
    ],
    '🆔': [
        'ID Button',
        'ID',
        'Identification',
        'Identification Sign',
        'Squared ID'
    ],
    '🆕': [
        'New Button',
        'New',
        'New Sign',
        'Squared New'
    ],
    '🅾️': [
        'O Button (Blood Type)',
        'Blood Type O',
        '0',
        'Negative Squared Latin Capital Letter',
        'Letter O'
    ],
    '🆗': [
        'OK Button',
        'OK',
        'Okay',
        'Square',
        'OK Sign',
        'Squared OK'
    ],
    '🅿️': [
        'P Button',
        'Parking Sign',
        'Negative Squared Latin Capital Letter',
        'Letter P'
    ],
    '🆘': [
        'SOS Button',
        'Distress Signal',
        'Emergency',
        'SOS',
        'SOS Sign',
        'Squared SOS',
        'Help'
    ],
    '🆙': [
        'Up! Button',
        'Level Up',
        'Up',
        'Up Sign',
        'Squared Up with Exclamation Mark'
    ],
    '🆚': [
        'Vs Button',
        'Squared Vs'
    ],
    '🆖': [
        'NG Button',
        'Squared NG',
        'No Good Sign',
        'No Good',
        'NG',
        'Blooper'
    ],
    '🔴': [
        'Red Circle',
        'Large Red Circle'
    ],
    '🟠': [
        'Orange Circle',
        'Large Orange Circle'
    ],
    '🟡': [
        'Yellow Circle',
        'Large Yellow Circle'
    ],
    '🟢': [
        'Green Circle',
        'Large Green Circle'
    ],
    '🔵': [
        'Blue Circle',
        'Large Blue Circle'
    ],
    '🟣': [
        'Purple Circle',
        'Large Purple Circle'
    ],
    '🟤': [
        'Brown Circle',
        'Large Brown Circle'
    ],
    '⚫': [
        'Black Circle',
        'Medium Black Circle'
    ],
    '⚪': [
        'White Circle',
        'Medium White Circle'
    ],
    '🟥': [
        'Red Square',
        'Large Red Square'
    ],
    '🟧': [
        'Orange Square',
        'Large Orange Square'
    ],
    '🟨': [
        'Yellow Square',
        'Large Yellow Square'
    ],
    '🟩': [
        'Green Square',
        'Large Green Square'
    ],
    '🟦': [
        'Blue Square',
        'Large Blue Square'
    ],
    '🟪': [
        'Purple Square',
        'Large Purple Square'
    ],
    '🟫': [
        'Brown Square',
        'Large Brown Square'
    ],
    '⬛': ['Black Large Square'],
    '⬜': ['White Large Square'],
    '◼️': ['Black Medium Square'],
    '◻️': ['White Medium Square'],
    '◾': ['Black Medium Small Square'],
    '◽': ['White Medium Small Square'],
    '▪️': ['Black Small Square'],
    '▫️': ['White Small Square'],
    '💠': [
        'Diamond Shape with a Dot Inside',
        'Cuteness',
        'Diamond Flower',
        'Kawaii'
    ],
    '🔶': ['Large Orange Diamond'],
    '🔷': ['Large Blue Diamond'],
    '🔸': ['Small Orange Diamond'],
    '🔹': ['Small Blue Diamond'],
    '🔺': [
        'Red Triangle Pointed Up',
        'Up-Pointing Red Triangle'
    ],
    '🔻': [
        'Red Triangle Pointed Down',
        'Down-Pointing Red Triangle'
    ],
    '🔳': ['White Square Button'],
    '🔲': ['Black Square Button'],
    '©️': [
        'Copyright',
        'Copyright Sign'
    ],
    '®️': [
        'Registered',
        'Registered Sign'
    ],
    '™️': [
        'Trade Mark',
        'TM',
        'Trademark',
        'Trade Mark Sign'
    ],
    '🛅': [
        'Left Luggage',
        'Bag With Key',
        'Locked Suitcase'
    ],
    '🛄': ['Baggage Claim'],
    '🛃': ['Customs'],
    '🛂': [
        'Passport Control',
        'Border Control'
    ],
    '🚰': [
        'Potable Water Symbol',
        'Thirst',
        'Thirsty',
        'Water Tap',
        'Water Faucet'
    ],
    '🇦': [
        'Letter A',
        'Regional Indicator Symbol Letter A'
    ],
    '🇧': [
        'Letter B',
        'Regional Indicator Symbol Letter B'
    ],
    '🇨': [
        'Letter C',
        'Regional Indicator Symbol Letter C'
    ],
    '🇩': [
        'Letter D',
        'Regional Indicator Symbol Letter D'
    ],
    '🇪': [
        'Letter E',
        'Regional Indicator Symbol Letter E'
    ],
    '🇫': [
        'Letter F',
        'Regional Indicator Symbol Letter F'
    ],
    '🇬': [
        'Letter G',
        'Regional Indicator Symbol Letter G'
    ],
    '🇭': [
        'Letter H',
        'Regional Indicator Symbol Letter H'
    ],
    '🇮': [
        'Letter I',
        'Regional Indicator Symbol Letter I'
    ],
    '🇯': [
        'Letter J',
        'Regional Indicator Symbol Letter J'
    ],
    '🇰': [
        'Letter K',
        'Regional Indicator Symbol Letter K'
    ],
    '🇱': [
        'Letter L',
        'Regional Indicator Symbol Letter L'
    ],
    '🇲': [
        'Letter M',
        'Regional Indicator Symbol Letter M'
    ],
    '🇳': [
        'Letter N',
        'Regional Indicator Symbol Letter N'
    ],
    '🇴': [
        'Letter O',
        'Regional Indicator Symbol Letter O'
    ],
    '🇵': [
        'Letter P',
        'Regional Indicator Symbol Letter P'
    ],
    '🇶': [
        'Letter Q',
        'Regional Indicator Symbol Letter Q'
    ],
    '🇷': [
        'Letter R',
        'Regional Indicator Symbol Letter R'
    ],
    '🇸': [
        'Letter S',
        'Regional Indicator Symbol Letter S'
    ],
    '🇹': [
        'Letter T',
        'Regional Indicator Symbol Letter T'
    ],
    '🇺': [
        'Letter U',
        'Regional Indicator Symbol Letter U'
    ],
    '🇻': [
        'Letter V',
        'Regional Indicator Symbol Letter V'
    ],
    '🇼': [
        'Letter W',
        'Regional Indicator Symbol Letter W'
    ],
    '🇽': [
        'Letter X',
        'Regional Indicator Symbol Letter X'
    ],
    '🇾': [
        'Letter Y',
        'Regional Indicator Symbol Letter Y'
    ],
    '🇿': [
        'Letter Z',
        'Regional Indicator Symbol Letter Z'
    ],
    '💌': [
        'Love Letter',
        'Heart Envelope',
        'Love Note'
    ],
    '🚮': [
        'Put Litter in Its Place Symbol',
        'Litter in Bin Sign',
        'Put Litter In Trash',
        'Person With Trash'
    ],
    '♿': [
        'Wheelchair Symbol',
        'Accessible Bathroom'
    ],
    '🚹': [
        'Mens Symbol',
        'Men\'s Symbol',
        'Mens Toilet',
        'Male Restroom',
        'Male WC'
    ],
    '🚺': [
        'Womens Symbol',
        'Women\'s Symbol',
        'Womens Toilet',
        'Female Restroom',
        'Famale WC'
    ],
    '🚻': [
        'Restroom',
        'Bathroom Sign',
        'Toilet Sign',
        'WC Sign'
    ],
    '🚼': [
        'Baby Symbol',
        'Baby Change Station',
        'Baby Change Symbol',
        'Nursery'
    ],
    '🚾': [
        'Water Closet',
        'WC',
        'Toilet WC'
    ],
    '⚠️': [
        'Warning Sign',
        'Alert Symbol'
    ],
    '🚸': [
        'Children Crossing',
        'Kids Crossing',
        'School Crossing'
    ],
    '🔅': [
        'Low Brightness Symbol',
        'Dim Button',
        'Decrease Brightness'
    ],
    '🔆': [
        'High Brightness Symbol',
        'Increase Brightness',
        'Bright Button'
    ],
    '📳': [
        'Vibration Mode',
        'Phone Heart',
        'Silent Mode'
    ],
    '📴': ['Mobile Phone Off'],
    '〰️': [
        'Wave',
        'Wavy Dash'
    ],
    '💱': ['Currency Exchange'],
    '💲': [
        'Heavy Dollar Sign',
        'Dollar',
        'Dollar Sign'
    ],
    '♻️': [
        'Black Universal Recycling Symbol',
        'Recycling Symbol',
        'Recycle Logo'
    ],
    '🔱': [
        'Trident Emblem',
        'Pitchfork',
        'Trident'
    ],
    '📛': [
        'Name Badge',
        'Tofu On Fire',
        'Name Tag',
        'Fire Tag'
    ],
    '🔰': [
        'Japanese Symbol for Beginner',
        'Shoshinsha Mark',
        'Yellow Green Shield'
    ],
    '➰': [
        'Curly Loop',
        'Curling Loop',
        'Loop'
    ],
    '➿': [
        'Double Curly Loop',
        'Double Curling Loop',
        'Voicemail'
    ],
    '〽️': [
        'Part Alternation Mark',
        'M',
        'McDonald\'s'
    ],
    '✳️': ['Eight Spoked Asterisk'],
    '✴️': [
        'Eight Pointed Black Star',
        'Orange Star',
        'Eight Pointed Star'
    ],
    '❇️': ['Sparkle'],
    'Ⓜ️': [
        'Circled Latin Capital Letter M',
        'Circled M',
        'Metro',
        'Circled M'
    ],
    '🈁': [
        'Squared Katakana Koko',
        'Japanese Word Sign Meaning Here',
        'Here',
        'Destination'
    ],
    '🈂️': [
        'Squared Katakana Sa',
        'Japanese Sign Meaning Service or Service Charge'
    ],
    '🈷️': [
        'Squared CJK Unified Ideograph-6708',
        'Radical 74',
        '月',
        'Japanese Sign Meaning Monthly Amount'
    ],
    '🈶': [
        'Squared CJK Unified Ideograph-6709',
        'Japanese Sign Meaning Not Free of Charge',
        'Own',
        '有'
    ],
    '🈯': [
        'Squared CJK Unified Ideograph-6307',
        'Japanese Sign Meaning Reserved',
        'Finger Point',
        '指'
    ],
    '🉐': [
        'Circled Ideograph Advantage',
        'Japanese Sign Meaning Bargain',
        'Acquire',
        '得'
    ],
    '🈹': [
        'Squared CJK Unified Ideograph-5272',
        'Japanese Sign Meaning Discount',
        'Bargain Sale',
        '割'
    ],
    '🈚': [
        'Squared CJK Unified Ideograph-7121',
        'Japanese Sign Meaning Free of Charge',
        'Lacking',
        '無'
    ],
    '🈲': [
        'Squared CJK Unified Ideograph-7981',
        'Japanese Sign Meaning Prohibited',
        'Forbid',
        '禁'
    ],
    '🉑': [
        'Circled Ideograph Accept',
        'Japanese Sign Meaning Acceptable',
        'Accept'
    ],
    '🈸': [
        'Squared CJK Unified Ideograph-7533',
        'Japanese Sign Meaning Application',
        'Application Form',
        'Monkey',
        'Request',
        '申'
    ],
    '🈴': [
        'Squared CJK Unified Ideograph-5408',
        'Japanese Sign Meaning Passing (Grade)',
        'Agreement',
        '合'
    ],
    '🈳': [
        'Squared CJK Unified Ideograph-7a7a',
        'Japanese Sign Meaning Vacancy',
        'Empty and Available',
        '空'
    ],
    '㊗️': [
        'Circled Ideograph Congratulation',
        'Japanese Sign Meaning Congratulations'
    ],
    '㊙️': [
        'Circled Ideograph Secret',
        'Japanese Sign Meaning Secret',
        'Secret'
    ],
    '🈺': [
        'Squared CJK Unified Ideograph-55b6',
        'Japanese Sign Meaning Open for Business',
        'Work',
        '営'
    ],
    '🈵': [
        'Squared CJK Unified Ideograph-6e80',
        'Japanese Sign Meaning Full; No Vacancy',
        'Full',
        '満'
    ],
    '🔘': ['Radio Button'],
    '🛑': [
        'Stop Sign',
        'Octagonal Sign'
    ],
    '⚕️': [
        'Medical Symbol',
        'Aesculapius',
        'Asklepios',
        'Rod of Asclepius',
        'Staff of Aesculapius'
    ],
    '🛗': [
        'Elevator',
        'Lift'
    ],
    '⚧️': ['Transgender Symbol'],
    '♂️': [
        'Male Sign',
        'Man Symbol',
        'Mars Symbol'
    ],
    '♀️': [
        'Female Sign',
        'Woman Symbol',
        'Venus Symbol'
    ]
};
/*food and drinks*/
emojis_language[lang][6] = {
    '🍇': [
        'Grapes',
        'Grape'
    ],
    '🍈': [
        'Melon',
        'Cantaloupe',
        'Honeydew',
        'Muskmelon'
    ],
    '🍉': ['Watermelon'],
    '🍊': [
        'Tangerine',
        'Mandarin',
        'Orange'
    ],
    '🍋': [
        'Lemon',
        'Lemonade'
    ],
    '🍌': [
        'Banana',
        'Plantain'
    ],
    '🍍': ['Pineapple'],
    '🥭': ['Mango'],
    '🍎': [
        'Red Apple',
        'Red Delicious Apple'
    ],
    '🍏': [
        'Green Apple',
        'Golden Delicious Apple',
        'Granny Smith Apple'
    ],
    '🍐': ['Pear'],
    '🍑': [
        'Peach',
        'Bottom',
        'Butt'
    ],
    '🍒': [
        'Cherries',
        'Cherry',
        'Wild Cherry'
    ],
    '🍓': ['Strawberry'],
    '🥝': [
        'Kiwi Fruit',
        'Chinese Gooseberry',
        'Kiwi',
        'Kiwifruit'
    ],
    '🍅': ['Tomato'],
    '🥥': [
        'Coconut',
        'Cocoanut'
    ],
    '🥑': ['Avocado'],
    '🍆': [
        'Eggplant',
        'Aubergine',
        'Phallic',
        'Purple Vegetable'
    ],
    '🥔': [
        'Potato',
        'Baked Potato',
        'Idaho Potato'
    ],
    '🥕': ['Carrot'],
    '🌽': [
        'Ear of Corn',
        'Corn',
        'Corn On The Cob',
        'Maize',
        'Ear of Maize'
    ],
    '🌶️': [
        'Hot Pepper',
        'Chili Pepper',
        'Spicy'
    ],
    '🥒': [
        'Cucumber',
        'Gherkin',
        'Pickle'
    ],
    '🥬': [
        'Leafy Green',
        'Bok Choy',
        'Chinese Cabbage',
        'Cos Lettuce',
        'Romaine Lettuce',
        'Leafy Greens'
    ],
    '🥦': ['Broccoli'],
    '🍄': [
        'Mushroom',
        'Shroom',
        'Toadstool'
    ],
    '🥜': [
        'Peanuts',
        'Nuts'
    ],
    '🌰': [
        'Chestnut',
        'Acorn',
        'Nut'
    ],
    '🍞': [
        'Bread',
        'Loaf Of Bread'
    ],
    '🥐': ['Croissant'],
    '🥖': [
        'Baguette Bread',
        'French Bread',
        'Baguette'
    ],
    '🥨': ['Pretzel'],
    '🥯': ['Bagel'],
    '🥞': [
        'Pancakes',
        'Crêpes',
        'Hotcakes'
    ],
    '🧀': [
        'Cheese Wedge',
        'Cheese'
    ],
    '🍖': [
        'Meat on Bone',
        'BBQ',
        'Barbecue',
        'Manga Meat'
    ],
    '🍗': [
        'Poultry Leg',
        'Drumstick',
        'Turkey Leg'
    ],
    '🥩': [
        'Cut of Meat',
        'Meat',
        'Steak'
    ],
    '🥓': [
        'Bacon',
        'Rashers'
    ],
    '🍔': [
        'Hamburger',
        'Burger',
        'Cheeseburger'
    ],
    '🍟': [
        'French Fries',
        'Chips',
        'Fries',
        'McDonald\'s Fries'
    ],
    '🍕': [
        'Pizza',
        'Pepperoni Pizza',
        'Slice of Pizza'
    ],
    '🌭': [
        'Hot Dog',
        'Hotdog',
        'Sausage'
    ],
    '🥪': ['Sandwich'],
    '🌮': ['Taco'],
    '🌯': [
        'Burrito',
        'Wrap'
    ],
    '🥙': [
        'Stuffed Flatbread',
        'Doner Kebab',
        'Gyro',
        'Shawarma',
        'Pita Sandwich'
    ],
    '🍳': [
        'Cooking',
        'Breakfast',
        'Fried Egg',
        'Frying Pan'
    ],
    '🥘': [
        'Shallow Pan of Food',
        'Paella',
        'Pan of Food'
    ],
    '🍲': [
        'Pot of Food',
        'Bowl Of Food',
        'Soup',
        'Stew'
    ],
    '🥣': [
        'Bowl with Spoon',
        'Cereal Bowl'
    ],
    '🥗': [
        'Green Salad',
        'Salad'
    ],
    '🍿': [
        'Popcorn',
        'Popping Corn'
    ],
    '🧂': [
        'Salt',
        'Salt Shaker'
    ],
    '🥫': [
        'Canned Food',
        'Can of Food',
        'Tin Can',
        'Tinned Food'
    ],
    '🍱': [
        'Bento Box',
        'Lunch Box'
    ],
    '🍘': [
        'Rice Cracker',
        'Cracker'
    ],
    '🍙': [
        'Rice Ball',
        'Onigiri'
    ],
    '🍚': [
        'Cooked Rice',
        'Boiled Rice',
        'Rice',
        'Bowl Of Rice',
        'Steamed Rice'
    ],
    '🍛': [
        'Curry Rice',
        'Curry',
        'Indian Food',
        'Curry and Rice'
    ],
    '🍜': [
        'Steaming Bowl',
        'Noodles',
        'Noodles With Chopsticks',
        'Ramen'
    ],
    '🍝': [
        'Spaghetti',
        'Pasta'
    ],
    '🍠': [
        'Roasted Sweet Potato',
        'Sweet Potato',
        'Yam'
    ],
    '🍢': [
        'Oden',
        'Kebab',
        'Skewer'
    ],
    '🍣': [
        'Sushi',
        'Sashimi',
        'Seafood'
    ],
    '🍤': [
        'Fried Shrimp',
        'Fried Prawn',
        'Shrimp Tempura'
    ],
    '🍥': [
        'Fish Cake with Swirl',
        'Fishcake',
        'Pink Swirl'
    ],
    '🥮': [
        'Moon Cake',
        'Mooncake'
    ],
    '🍡': [
        'Dango',
        'Dessert Stick',
        'Pink White Green Balls'
    ],
    '🥟': [
        'Dumpling',
        'Empanada',
        'Pierogi'
    ],
    '🥠': ['Fortune Cookie'],
    '🥡': [
        'Takeout Box',
        'Chinese Food Box',
        'Oyster Pail'
    ],
    '🍦': [
        'Soft Ice Cream',
        'Mr Whippy',
        'Soft Serve'
    ],
    '🍧': [
        'Shaved Ice',
        'Snow Cone'
    ],
    '🍨': [
        'Ice Cream',
        'Bowl Of Ice Cream',
        'Dessert'
    ],
    '🍩': [
        'Doughnut',
        'Donut'
    ],
    '🍪': [
        'Cookie',
        'Biscuit',
        'Chocolate Chip Cookie'
    ],
    '🎂': [
        'Birthday Cake',
        'Birthday',
        'Cake',
        'Cake With Candles'
    ],
    '🍰': [
        'Shortcake',
        'Cake',
        'Piece Of Cake',
        'Strawberry Shortcake'
    ],
    '🧁': [
        'Cupcake',
        'Fairy Cake'
    ],
    '🥧': ['Pie'],
    '🍫': [
        'Chocolate Bar',
        'Candy Bar',
        'Chocolate'
    ],
    '🍬': [
        'Candy',
        'Lolly',
        'Sweet'
    ],
    '🍭': [
        'Lollipop',
        'Sucker'
    ],
    '🍮': [
        'Custard',
        'Creme Caramel',
        'Dessert',
        'Flan',
        'Pudding'
    ],
    '🍯': [
        'Honey Pot',
        'Honey',
        'Pot'
    ],
    '🍼': [
        'Baby Bottle',
        'Bottle Feeding'
    ],
    '🥛': [
        'Glass of Milk',
        'Milk'
    ],
    '☕': [
        'Hot Beverage',
        'Coffee',
        'Espresso',
        'Hot Chocolate',
        'Tea'
    ],
    '🍵': [
        'Teacup Without Handle',
        'Green Tea',
        'Matcha',
        'Matcha Green Tea'
    ],
    '🍶': [
        'Sake',
        'Bottle',
        'Rice Wine',
        'Sake Bottle and Cup'
    ],
    '🍾': [
        'Bottle with Popping Cork',
        'Celebration',
        'Champagne',
        'Sparkling Wine'
    ],
    '🍷': [
        'Wine Glass',
        'Alcohol',
        'Red Wine',
        'Wine'
    ],
    '🍸': [
        'Cocktail Glass',
        'Cocktail',
        'Martini'
    ],
    '🍹': [
        'Tropical Drink',
        'Fruit Punch',
        'Tiki Drink'
    ],
    '🍺': [
        'Beer Mug',
        'Beer',
        'Beer Stein'
    ],
    '🍻': [
        'Clinking Beer Mugs',
        'Beers',
        'Cheers'
    ],
    '🥂': [
        'Clinking Glasses',
        'Celebration',
        'Champagne Glasses',
        'Cheers'
    ],
    '🥃': [
        'Tumbler Glass',
        'Bourbon',
        'Liquor',
        'Rum',
        'Whiskey',
        'Whisky'
    ],
    '🥤': [
        'Cup with Straw',
        'Milkshake',
        'Smoothie',
        'Soda Pop',
        'Soft Drink'
    ],
    '🥢': ['Chopsticks'],
    '🍽️': [
        'Fork and Knife with Plate',
        'Dinner'
    ],
    '🍴': [
        'Fork and Knife',
        'Cutlery',
        'Knife And Fork',
        'Silverware'
    ],
    '🥄': ['Spoon'],
    '🧄': ['Garlic'],
    '🧅': ['Onion'],
    '🧇': ['Waffle'],
    '🧆': ['Falafel'],
    '🧈': ['Butter'],
    '🦪': ['Oyster'],
    '🧃': [
        'Beverage Box',
        'Juice Box'
    ],
    '🧉': [
        'Mate Drink',
        'Chimarrão',
        'Cimarrón',
        'Yerba Mate'
    ],
    '🫒': ['Olive'],
    '🫐': ['Blueberries'],
    '🫑': ['Bell Pepper'],
    '🫓': ['Flatbread'],
    '🫔': [
        'Tamale',
        'Tamal'
    ],
    '🫕': ['Fondue'],
    '🧋': [
        'Bubble Tea',
        'Boba'
    ],
    '🧊': ['Ice Cube'],
    '🫘': [
        'Beans',
        'Flatulence',
        'Coffee beans',
        'Spill the beans'
    ],
    '🫗': [
        'Pouring Liquid',
        'Pouring Drink',
        'Water'
    ]
};
/*flags*/
emojis_language[lang][7] = {
    '🏁': [
        'Chequered Flag',
        'Checkered Flag',
        'Grid Girl',
        'Racing Flag'
    ],
    '🚩': [
        'Triangular Flag',
        'Flag on Pole',
        'Red Flag',
        'Triangular Flag on Post'
    ],
    '🎌': [
        'Crossed Flags',
        'Two Flags'
    ],
    '🏴': [
        'Black Flag',
        'Waving Black Flag'
    ],
    '🏳️': [
        'White Flag',
        'Waving White Flag'
    ],
    '🏳️‍🌈': [
        'Rainbow Flag',
        'Pride Flag',
        'LGBT',
        'LGBTQIA+',
        'Gay',
        'Transgender',
        'Lesbian',
        'Asexual',
        'Queer'
    ],
    '🏳️‍⚧️': [
        'Transgender Flag',
        'Blue, Pink, and White Flag',
        'Trans Flag'
    ],
    '🏴‍☠️': [
        'Pirate Flag',
        'Jolly Roger'
    ],
    '🇺🇳': [
        'Flag of United Nations',
        'UN',
        'Flag: United Nations'
    ],
    '🇪🇺': [
        'Flag of European Union',
        'EU',
        'Flag: European Union',
        'EU Flag'
    ],
    '🇦🇨': [
        'Flag of Ascension Island',
        'AC'
    ],
    '🇦🇩': [
        'Flag of Andorra',
        'AD',
        'Andorran Flag'
    ],
    '🇦🇪': [
        'Flag of United Arab Emirates',
        'AE',
        'Emirati Flag',
        'UAE Flag'
    ],
    '🇦🇫': [
        'Flag of Afghanistan',
        'AF',
        'Afghan Flag'
    ],
    '🇦🇬': [
        'Flag of Antigua & Barbuda',
        'AG'
    ],
    '🇦🇮': [
        'Flag of Anguilla',
        'AI',
        'Anguillan Flag'
    ],
    '🇦🇱': [
        'Flag of Albania',
        'AL',
        'Albanian Flag'
    ],
    '🇦🇲': [
        'Flag of Armenia',
        'AM',
        '🇦🇲'
    ],
    '🇦🇴': [
        'Flag of Angola',
        'AO',
        'Angolan Flag'
    ],
    '🇦🇶': [
        'Flag of Antarctica',
        'AQ',
        'Antarctic Flag'
    ],
    '🇦🇷': [
        'Flag of Argentina',
        'AR',
        'Argentinian Flag'
    ],
    '🇦🇸': [
        'Flag of American Samoa',
        'AS',
        'American Samoan Flag'
    ],
    '🇦🇹': [
        'Flag of Austria',
        'AT',
        'Austrian Flag'
    ],
    '🇦🇺': [
        'Flag of Australia',
        'AU',
        'Aussie Flag',
        'Australian Flag'
    ],
    '🇦🇼': [
        'Flag of Aruba',
        'AW',
        'Aruban Flag'
    ],
    '🇦🇽': [
        'Flag of Åland Islands',
        'AX',
        'Åland Flag'
    ],
    '🇦🇿': [
        'Flag of Azerbaijan',
        'AZ',
        'Azerbaijani Flag'
    ],
    '🇧🇦': [
        'Flag of Bosnia & Herzegovina',
        'BA'
    ],
    '🇧🇧': [
        'Flag of Barbados',
        'BB',
        'Barbadian Flag',
        'Bajan Flag'
    ],
    '🇧🇩': [
        'Flag of Bangladesh',
        'BD',
        'Bangladeshi Flag'
    ],
    '🇧🇪': [
        'Flag of Belgium',
        'BE',
        'Belgian Flag'
    ],
    '🇧🇫': [
        'Flag of Burkina Faso',
        'BF',
        'Burkinabe Flag'
    ],
    '🇧🇬': [
        'Flag of Bulgaria',
        'BG',
        'Bulgarian Flag'
    ],
    '🇧🇭': [
        'Flag of Bahrain',
        'BH',
        'Bahrani Flag',
        'Bahrainian Flag'
    ],
    '🇧🇮': [
        'Flag of Burundi',
        'BI',
        'Burundian Flag'
    ],
    '🇧🇯': [
        'Flag of Benin',
        'BJ',
        'Beninese Flag'
    ],
    '🇧🇱': [
        'Flag of Saint Barthélemy',
        'BL',
        'Flag: St. Barthélemy'
    ],
    '🇧🇲': [
        'Flag of Bermuda',
        'BM',
        'Bermudan Flag'
    ],
    '🇧🇳': [
        'Flag of Brunei',
        'BN',
        'Bruneian Flag'
    ],
    '🇧🇴': [
        'Flag of Bolivia',
        'BO',
        'Bolivian Flag'
    ],
    '🇧🇶': [
        'Flag of Caribbean Netherlands',
        'BQ',
        'Bonaire'
    ],
    '🇧🇷': [
        'Flag of Brazil',
        'BR',
        'Brazilian Flag'
    ],
    '🇧🇸': [
        'Flag of Bahamas',
        'Bahamian Flag',
        'BS'
    ],
    '🇧🇹': [
        'Flag of Bhutan',
        'BT',
        'Bhutanese Flag'
    ],
    '🇧🇻': [
        'Flag of Bouvet Island',
        'BV',
        'Flag: Bouvet Island'
    ],
    '🇧🇼': [
        'Flag of Botswana',
        'BW',
        'Batswana Flag'
    ],
    '🇧🇾': [
        'Flag of Belarus',
        'BY',
        'Belarusian Flag'
    ],
    '🇧🇿': [
        'Flag of Belize',
        'BZ',
        'Belizean Flag'
    ],
    '🇨🇦': [
        'Flag of Canada',
        'Canadian Flag',
        'CA'
    ],
    '🇨🇨': [
        'Flag of Cocos (Keeling) Islands',
        'CC'
    ],
    '🇨🇩': [
        'Flag of Congo - Kinshasa',
        'CD'
    ],
    '🇨🇫': [
        'Flag of Central African Republic',
        'CF',
        'Central African Flag'
    ],
    '🇨🇬': [
        'Flag of Congo - Brazzaville',
        'CG'
    ],
    '🇨🇭': [
        'Flag of Switzerland',
        'CH',
        'Red Cross',
        'Swiss Flag'
    ],
    '🇨🇮': [
        'Flag of Ivory Coast',
        'CI',
        'Flag: Côte d’Ivoire'
    ],
    '🇨🇰': [
        'Flag of Cook Islands',
        'CK',
        'Cook Islander Flag'
    ],
    '🇨🇱': [
        'Flag of Chile',
        'CL',
        'Chilean Flag'
    ],
    '🇨🇲': [
        'Flag of Cameroon',
        'CM',
        'Cameroonian Flag'
    ],
    '🇨🇳': [
        'Flag of China',
        'CN',
        'Chinese Flag'
    ],
    '🇨🇴': [
        'Flag of Colombia',
        'CO',
        'Colombian Flag'
    ],
    '🇨🇵': [
        'Flag of Clipperton Island',
        'CP',
        'Flag: Clipperton Island'
    ],
    '🇨🇷': [
        'Flag of Costa Rica',
        'CR',
        'Costa Rican Flag'
    ],
    '🇨🇺': [
        'Flag of Cuba',
        'CU',
        'Cuban Flag'
    ],
    '🇨🇻': [
        'Flag of Cape Verde',
        'CV',
        'Cape Verdian Flag'
    ],
    '🇨🇼': [
        'Flag of Curaçao',
        'CW',
        'Curaçaoan Flag'
    ],
    '🇨🇽': [
        'Flag of Christmas Island',
        'CX',
        'Christmas Island Flag'
    ],
    '🇨🇾': [
        'Flag of Cyprus',
        'CY',
        'Cypriot Flag'
    ],
    '🇨🇿': [
        'Flag of Czechia',
        'CZ',
        'Czech Flag'
    ],
    '🇩🇪': [
        'Flag of Germany',
        'DE',
        'Deutsch Flag',
        'German Flag'
    ],
    '🇩🇬': [
        'Flag of Diego Garcia',
        'DG',
        'Flag: Diego Garcia'
    ],
    '🇩🇯': [
        'Flag of Djibouti',
        'DJ',
        'Djibouti Flag',
        'Djiboutian Flag'
    ],
    '🇩🇰': [
        'Flag of Denmark',
        'DK',
        'Danish Flag'
    ],
    '🇩🇲': [
        'Flag of Dominica',
        'DM'
    ],
    '🇩🇴': [
        'Flag of Dominican Republic',
        'DO',
        'Dom Rep Flag',
        'Dominican Flag'
    ],
    '🇩🇿': [
        'Flag of Algeria',
        'DZ',
        'Algerian Flag'
    ],
    '🇪🇦': [
        'Flag of Ceuta & Melilla',
        'Flag of Melilla',
        'Flag of Ceuta',
        'EA',
        'Flag: Ceuta & Melilla'
    ],
    '🇪🇨': [
        'Flag of Ecuador',
        'EC',
        'Ecuadorian Flag'
    ],
    '🇪🇪': [
        'Flag of Estonia',
        'EE',
        'Estonian Flag'
    ],
    '🇪🇬': [
        'Flag of Egypt',
        'EG',
        'Egyptian Flag'
    ],
    '🇪🇭': [
        'Flag of Western Sahara',
        'EH',
        'Western Saharan Flag'
    ],
    '🇪🇷': [
        'Flag of Eritrea',
        'ER',
        'Eritrean Flag'
    ],
    '🇪🇹': [
        'Flag of Ethiopia',
        'ET',
        'Ethiopian Flag'
    ],
    '🇪🇸': [
        'Flag of Spain',
        'ES',
        'Spanish Flag'
    ],
    '🇫🇮': [
        'Flag of Finland',
        'FI',
        'Finnish Flag'
    ],
    '🇫🇯': [
        'Flag of Fiji',
        'FJ',
        'Fijian Flag'
    ],
    '🇫🇰': [
        'Flag of Falkland Islands',
        'FK',
        'Falklander Flag'
    ],
    '🇫🇲': [
        'Flag of Micronesia',
        'FM',
        'Micronesian Flag'
    ],
    '🇫🇴': [
        'Flag of Faroe Islands',
        'FO',
        'Faroe Islander Flag'
    ],
    '🇫🇷': [
        'Flag of France',
        'FR',
        'French Flag'
    ],
    '🇬🇦': [
        'Flag of Gabon',
        'GA',
        'Gabonese Flag'
    ],
    '🇬🇧': [
        'Flag of Great Britain',
        'GB',
        'British Flag',
        'United Kingdom',
        'Union Jack',
        'UK'
    ],
    '🏴󠁧󠁢󠁥󠁮󠁧󠁿󠁧󠁢󠁥󠁮󠁧󠁿': [
        'Flag of England',
        'St George\'s Cross',
        'English Flag'
    ],
    '🏴󠁧󠁢󠁷󠁬󠁳󠁿󠁧󠁢󠁷󠁬󠁳󠁿': [
        'Flag of Wales',
        'Welsh Flag',
        'Red Dragon',
        'Baner Cymru',
        'Y Ddraig Goch'
    ],
    '🏴󠁧󠁢󠁳󠁣󠁴󠁿󠁧󠁢󠁳󠁣󠁴󠁿': [
        'Flag of Scotland',
        'Scottish Flag',
        'St Andrew\'s Cross',
        'Saltire'
    ],
    '🇬🇩': [
        'Flag of Grenada',
        'GD',
        'Grenadian Flag'
    ],
    '🇬🇪': [
        'Flag of Georgia',
        'GE',
        'Georgian Flag'
    ],
    '🇬🇫': [
        'Flag of French Guiana',
        'GF',
        'French Guinean Flag'
    ],
    '🇬🇬': [
        'Flag of Guernsey',
        'GG',
        'Guernsey Flag'
    ],
    '🇬🇭': [
        'Flag of Ghana',
        'GH',
        'Ghanaian Flag'
    ],
    '🇬🇮': [
        'Flag of Gibraltar',
        'GI',
        'Gibraltarian Flag'
    ],
    '🇬🇱': [
        'Flag of Greenland',
        'GL',
        'Greenlandic Flag'
    ],
    '🇬🇲': [
        'Flag of Gambia',
        'GM',
        'Gambian Flag'
    ],
    '🇬🇳': [
        'Flag of Guinea',
        'GN',
        'Guinean Flag'
    ],
    '🇬🇵': [
        'Flag of Guadeloupe',
        'GP',
        'Guadeloupean Flag'
    ],
    '🇬🇶': [
        'Flag of Equatorial Guinea',
        'GQ',
        'Equatorial Guinean Flag',
        'Equatoguinean Flag'
    ],
    '🇬🇷': [
        'Flag of Greece',
        'GR',
        'Greek Flag'
    ],
    '🇬🇸': [
        'Flag of South Georgia & the South Sandwich Islands',
        'GS'
    ],
    '🇬🇹': [
        'Flag of Guatemala',
        'GT',
        'Guatemalan Flag'
    ],
    '🇬🇺': [
        'Flag of Guam',
        'GU',
        'Guamanian Flag',
        'Chamorro Flag'
    ],
    '🇬🇼': [
        'Flag of Guinea-Bissau',
        'GW',
        'Guinea-Bissau Flag'
    ],
    '🇬🇾': [
        'Flag of Guyana',
        'GY',
        'Guyanese Flag'
    ],
    '🇭🇰': [
        'Flag of Hong Kong SAR China',
        'HK',
        'Hong Kong Flag'
    ],
    '🇭🇲': [
        'Flag of Heard & McDonald Islands',
        'HM',
        'Flag: Heard & McDonald Islands'
    ],
    '🇭🇳': [
        'Flag of Honduras',
        'HN',
        'Honduran Flag'
    ],
    '🇭🇷': [
        'Flag of Croatia',
        'HR',
        'Croatian Flag'
    ],
    '🇭🇹': [
        'Flag of Haiti',
        'HT',
        'Haitian Flag'
    ],
    '🇭🇺': [
        'Flag of Hungary',
        'HU',
        'Hungarian Flag'
    ],
    '🇮🇨': [
        'Flag of Canary Islands',
        'IC'
    ],
    '🇮🇩': [
        'Flag of Indonesia',
        'ID',
        'Indonesian Flag'
    ],
    '🇮🇪': [
        'Flag of Ireland',
        'IE',
        'Irish Flag'
    ],
    '🇮🇱': [
        'Flag of Israel',
        'IL',
        'Israeli Flag'
    ],
    '🇮🇲': [
        'Flag of Isle of Man',
        'IM',
        'Manx Flag'
    ],
    '🇮🇳': [
        'Flag of India',
        'IN',
        'Indian Flag'
    ],
    '🇮🇴': [
        'Flag of British Indian Ocean Territory',
        'IO',
        'British Indian Ocean Territory Flag'
    ],
    '🇮🇶': [
        'Flag of Iraq',
        'IQ',
        'Iraqi Flag'
    ],
    '🇮🇷': [
        'Flag of Iran',
        'IR',
        'Iranian Flag'
    ],
    '🇮🇸': [
        'Flag of Iceland',
        'IS',
        'Icelandic Flag'
    ],
    '🇮🇹': [
        'Flag of Italy',
        'IT',
        'Italian Flag'
    ],
    '🇯🇪': [
        'Flag of Jersey',
        'JE',
        'Jersey Flag'
    ],
    '🇯🇲': [
        'Flag of Jamaica',
        'JM',
        'Jamaican Flag'
    ],
    '🇯🇴': [
        'Flag of Jordan',
        'JO',
        'Jordanian Flag'
    ],
    '🇯🇵': [
        'Flag of Japan',
        'JP',
        'Japanese Flag'
    ],
    '🇰🇪': [
        'Flag of Kenya',
        'KE',
        'Kenyan Flag'
    ],
    '🇰🇬': [
        'Flag of Kyrgyzstan',
        'KG',
        'Kyrgyzstani Flag'
    ],
    '🇰🇭': [
        'Flag of Cambodia',
        'KH',
        'Cambodian Flag'
    ],
    '🇰🇮': [
        'Flag of Kiribati',
        'KI',
        'I-Kiribati Flag'
    ],
    '🇰🇲': [
        'Flag of Comoros',
        'KM',
        'Comoran Flag'
    ],
    '🇰🇳': [
        'Flag of Saint Kitts & Nevis',
        'KN'
    ],
    '🇰🇵': [
        'Flag of North Korea',
        'KP',
        'North Korean Flag'
    ],
    '🇰🇷': [
        'Flag of South Korea',
        'KR',
        'South Korean Flag'
    ],
    '🇰🇼': [
        'Flag of Kuwait',
        'KW',
        'Kuwaiti Flag'
    ],
    '🇰🇾': [
        'Flag of Cayman Islands',
        'KY',
        'Caymanian Flag'
    ],
    '🇰🇿': [
        'Flag of Kazakhstan',
        'KZ',
        'Kazakhstani Flag',
        'Kazakh Flag'
    ],
    '🇱🇦': [
        'Flag of Laos',
        'LA',
        'Laotian Flag',
        'Lao Flag'
    ],
    '🇱🇧': [
        'Flag of Lebanon',
        'LB',
        'Lebanese Flag'
    ],
    '🇱🇨': [
        'Flag of Saint Lucia',
        'LC'
    ],
    '🇱🇮': [
        'Flag of Liechtenstein',
        'LI',
        'Liechtensteiner Flag'
    ],
    '🇱🇰': [
        'Flag of Sri Lanka',
        'LK',
        'Sri Lankan Flag'
    ],
    '🇱🇷': [
        'Flag of Liberia',
        'LR',
        'Liberian Flag'
    ],
    '🇱🇸': [
        'Flag of Lesotho',
        'LS',
        'Basotho Flag'
    ],
    '🇱🇹': [
        'Flag of Lithuania',
        'LT',
        'Lithuanian Flag'
    ],
    '🇱🇺': [
        'Flag of Luxembourg',
        'LU',
        'Luxembourger Flag'
    ],
    '🇱🇻': [
        'Flag of Latvia',
        'LV',
        'Latvian Flag'
    ],
    '🇱🇾': [
        'Flag of Libya',
        'LY',
        'Libyan Flag'
    ],
    '🇲🇦': [
        'Flag of Morocco',
        'MA',
        'Moroccan Flag'
    ],
    '🇲🇨': [
        'Flag of Monaco',
        'MC',
        'Monégasque Flag'
    ],
    '🇲🇩': [
        'Flag of Moldova',
        'MD',
        'Moldovan Flag'
    ],
    '🇲🇪': [
        'Flag of Montenegro',
        'ME',
        'Montenegrin Flag'
    ],
    '🇲🇫': [
        'Flag of Saint Martin',
        'MF',
        'Flag: St. Martin'
    ],
    '🇲🇬': [
        'Flag of Madagascar',
        'MG',
        'Madagascan Flag'
    ],
    '🇲🇭': [
        'Flag of Marshall Islands',
        'MH',
        'Marshallese Flag'
    ],
    '🇲🇰': [
        'Flag of Macedonia',
        'MK',
        'Macedonian Flag'
    ],
    '🇲🇱': [
        'Flag of Mali',
        'ML',
        'Malian Flag'
    ],
    '🇲🇲': [
        'Flag of Myanmar (Burma)',
        'MM',
        'Myanmarese Flag',
        'Flag For Burma',
        'Burmese Flag'
    ],
    '🇲🇳': [
        'Flag of Mongolia',
        'MN',
        'Mongolian Flag'
    ],
    '🇲🇴': [
        'Flag of Macau Sar China',
        'MO',
        'Macanese Flag'
    ],
    '🇲🇵': [
        'Flag of Northern Mariana Islands',
        'MP',
        'Micronesian Flag'
    ],
    '🇲🇶': [
        'Flag of Martinique',
        'MQ',
        'Snake Flag of Martinique',
        'Martiniquais Flag'
    ],
    '🇲🇷': [
        'Flag of Mauritania',
        'MR',
        'Mauritanian Flag'
    ],
    '🇲🇸': [
        'Flag of Montserrat',
        'MS',
        'Montserratian Flag'
    ],
    '🇲🇹': [
        'Flag of Malta',
        'MT',
        'Maltese Flag'
    ],
    '🇲🇺': [
        'Flag of Mauritius',
        'MU',
        'Mauritian Flag'
    ],
    '🇲🇻': [
        'Flag of Maldives',
        'MV',
        'Maldivian Flag'
    ],
    '🇲🇼': [
        'Flag of Malawi',
        'MW',
        'Malawian Flag'
    ],
    '🇲🇽': [
        'Flag of Mexico',
        'MX',
        'Mexican Flag'
    ],
    '🇲🇾': [
        'Flag of Malaysia',
        'MY',
        'Malaysian Flag'
    ],
    '🇲🇿': [
        'Flag of Mozambique',
        'MZ',
        'Mozambican Flag'
    ],
    '🇳🇦': [
        'Flag of Namibia',
        'NA',
        'Namibian Flag'
    ],
    '🇳🇨': [
        'Flag of New Caledonia',
        'NC',
        'New Caledonian Flag'
    ],
    '🇳🇪': [
        'Flag of Niger',
        'NE',
        'Nigerien Flag'
    ],
    '🇳🇫': [
        'Flag of Norfolk Island',
        'NF',
        'Norfolk Island Flag'
    ],
    '🇳🇬': [
        'Flag of Nigeria',
        'NG',
        'Nigerian Flag'
    ],
    '🇳🇮': [
        'Flag of Nicaragua',
        'NI',
        'Nicaraguan Flag'
    ],
    '🇳🇱': [
        'Flag of Netherlands',
        'NL',
        'Dutch Flag',
        'Holland'
    ],
    '🇳🇴': [
        'Flag of Norway',
        'NO',
        'Norwegian Flag'
    ],
    '🇳🇵': [
        'Flag of Nepal',
        'NP',
        'Nepalese Flag'
    ],
    '🇳🇷': [
        'Flag of Nauru',
        'NR',
        'Nauruan Flag'
    ],
    '🇳🇺': [
        'Flag of Niue',
        'NU',
        'Niuean Flag'
    ],
    '🇳🇿': [
        'Flag of New Zealand',
        'NZ',
        'Kiwi Flag'
    ],
    '🇴🇲': [
        'Flag of Oman',
        'OM',
        'Omani Flag'
    ],
    '🇵🇦': [
        'Flag of Panama',
        'PA',
        'Panamanian Flag'
    ],
    '🇵🇪': [
        'Flag of Peru',
        'PE',
        'Peruvian Flag'
    ],
    '🇵🇫': [
        'Flag of French Polynesia',
        'PF',
        'French Polynesian Flag'
    ],
    '🇵🇬': [
        'Flag of Papua New Guinea',
        'PG',
        'PNG Flag',
        'Papua New Guinean Flag'
    ],
    '🇵🇭': [
        'Flag of Philippines',
        'PH'
    ],
    '🇵🇰': [
        'Flag of Pakistan',
        'PK',
        'Pakistani Flag'
    ],
    '🇵🇱': [
        'Flag of Poland',
        'PL',
        'Polish Flag'
    ],
    '🇵🇲': [
        'Flag of Saint Pierre & Miquelon',
        'PM'
    ],
    '🇵🇳': [
        'Flag of Pitcairn',
        'PN'
    ],
    '🇵🇷': [
        'Flag of Puerto Rico',
        'PR',
        'Puerto Rican Flag'
    ],
    '🇵🇸': [
        'Flag of Palestinian Territories',
        'PS',
        'Palestine'
    ],
    '🇵🇹': [
        'Flag of Portugal',
        'PT',
        'Portugese Flag'
    ],
    '🇵🇼': [
        'Flag of Palau',
        'PW',
        'Palauan Flag'
    ],
    '🇵🇾': [
        'Flag of Paraguay',
        'PY',
        'Paraguayan Flag'
    ],
    '🇶🇦': [
        'Flag of Qatar',
        'QA',
        'Qatari Flag'
    ],
    '🇷🇪': [
        'Flag of Réunion',
        'RE',
        'Réunionnais Flag'
    ],
    '🇷🇴': [
        'Flag of Romania',
        'RO',
        'Romanian Flag'
    ],
    '🇷🇸': [
        'Flag of Serbia',
        'RS',
        'Serbian Flag'
    ],
    '🇷🇺': [
        'Flag of Russia',
        'RU',
        'Russian Flag'
    ],
    '🇷🇼': [
        'Flag of Rwanda',
        'RW',
        'Rwandan Flag'
    ],
    '🇸🇦': [
        'Flag of Saudi Arabia',
        'SA',
        'Saudi Arabian Flag'
    ],
    '🇸🇧': [
        'Flag of Solomon Islands',
        'SB',
        'Solomon Islander Flag'
    ],
    '🇸🇨': [
        'Flag of Seychelles',
        'SC',
        'Seychellois Flag'
    ],
    '🇸🇩': [
        'Flag of Sudan',
        'SD',
        'Sudanese Flag'
    ],
    '🇸🇪': [
        'Flag of Sweden',
        'SE',
        'Swedish Flag'
    ],
    '🇸🇬': [
        'Flag of Singapore',
        'SG',
        'Singaporean Flag'
    ],
    '🇸🇭': [
        'Flag of Saint Helena',
        'SH'
    ],
    '🇸🇮': [
        'Flag of Slovenia',
        'SI',
        'Slovenian Flag'
    ],
    '🇸🇯': [
        'Flag of Svalbard & Jan Mayen',
        'SJ'
    ],
    '🇸🇰': [
        'Flag of Slovakia',
        'SK',
        'Slovak Flag',
        'Slovakian Flag'
    ],
    '🇸🇱': [
        'Flag of Sierra Leone',
        'SL',
        'Sierra Leonean Flag'
    ],
    '🇸🇲': [
        'Flag of San Marino',
        'SM',
        'Sammarinese Flag'
    ],
    '🇸🇳': [
        'Flag of Senegal',
        'SN',
        'Sengalese Flag'
    ],
    '🇸🇴': [
        'Flag of Somalia',
        'SO',
        'Somalian Flag'
    ],
    '🇸🇷': [
        'Flag of Suriname',
        'SR',
        'Surinamer Flag'
    ],
    '🇸🇸': [
        'Flag of South Sudan',
        'SS',
        'South Sudanese Flag'
    ],
    '🇸🇹': [
        'Flag of São Tomé & Príncipe',
        'ST'
    ],
    '🇸🇻': [
        'Flag of El Salvador',
        'SV',
        'El Salvadoran Flag'
    ],
    '🇸🇽': [
        'Flag of Sint Maarten',
        'SX'
    ],
    '🇸🇾': [
        'Flag of Syria',
        'SY',
        'Syrian Flag'
    ],
    '🇸🇿': [
        'Flag of Swaziland',
        'SZ',
        'Eswatini Flag'
    ],
    '🇹🇦': [
        'Flag of Tristan Da Cunha',
        'TA'
    ],
    '🇹🇨': [
        'Flag of Turks & Caicos Islands',
        'TC'
    ],
    '🇹🇩': [
        'Flag of Chad',
        'TD',
        'Chadian Flag'
    ],
    '🇹🇫': [
        'Flag of French Southern Territories',
        'TF',
        'French Southern Lands Flag'
    ],
    '🇹🇬': [
        'Flag of Togo',
        'TG',
        'Togolese Flag'
    ],
    '🇹🇭': [
        'Flag of Thailand',
        'TH',
        'Thai Flag'
    ],
    '🇹🇯': [
        'Flag of Tajikistan',
        'TJ',
        'Tajik Flag'
    ],
    '🇹🇰': [
        'Flag of Tokelau',
        'TK',
        'Tokelauan Flag'
    ],
    '🇹🇱': [
        'Flag of Timor-Leste',
        'TL',
        'Timor-Leste Flag',
        'East Timorese Flag'
    ],
    '🇹🇲': [
        'Flag of Turkmenistan',
        'TM',
        'Turkmen Flag'
    ],
    '🇹🇳': [
        'Flag of Tunisia',
        'TN',
        'Tunisian Flag'
    ],
    '🇹🇴': [
        'Flag of Tonga',
        'TO',
        'Tongan Flag'
    ],
    '🇹🇷': [
        'Flag of Turkey',
        'TR',
        'Turkish Flag'
    ],
    '🇹🇹': [
        'Flag of Trinidad & Tobago',
        'TT'
    ],
    '🇹🇻': [
        'Flag of Tuvalu',
        'TV',
        'Tuvaluan Flag'
    ],
    '🇹🇼': [
        'Flag of Taiwan',
        'TW',
        'Taiwanese Flag'
    ],
    '🇹🇿': [
        'Flag of Tanzania',
        'TZ',
        'Tanzanian Flag'
    ],
    '🇺🇦': [
        'Flag of Ukraine',
        'UA',
        'Ukrainian Flag'
    ],
    '🇺🇬': [
        'Flag of Uganda',
        'UG',
        'Ugandan Flag'
    ],
    '🇺🇲': [
        'Flag: U.S. Outlying Islands',
        'UM'
    ],
    '🇺🇸': [
        'Flag of United States',
        'US',
        'American Flag',
        'USA Flag'
    ],
    '🇺🇾': [
        'Flag of Uruguay',
        'UY',
        'Uruguayan Flag'
    ],
    '🇺🇿': [
        'Flag of Uzbekistan',
        'UZ',
        'Uzbekistani Flag',
        'Uzbek Flag'
    ],
    '🇻🇦': [
        'Flag of Holy See',
        'VA',
        'Vanticanien Flag'
    ],
    '🇻🇨': [
        'Flag of Saint Vincent & Grenadines',
        'VC'
    ],
    '🇻🇪': [
        'Flag of Venezuela',
        'VE',
        'Venezuelan Flag'
    ],
    '🇻🇬': [
        'Flag of British Virgin Islands',
        'VG',
        'British Virgin Islander Flag'
    ],
    '🇻🇮': [
        'Flag of US Virgin Islands',
        'VI',
        'US Virgin Islander Flag'
    ],
    '🇻🇳': [
        'Flag of Vietnam',
        'VN',
        'Vietnamese Flag'
    ],
    '🇻🇺': [
        'Flag of Vanuatu',
        'VU',
        'Ni-Vanuatu Flag'
    ],
    '🇼🇫': [
        'Flag of Wallis & Futuna',
        'WF'
    ],
    '🇼🇸': [
        'Flag of Samoa',
        'WS',
        'Samoan Flag'
    ],
    '🇽🇰': [
        'Flag of Kosovo',
        'XK',
        'Kosovar Flag'
    ],
    '🇾🇪': [
        'Flag of Yemen',
        'YE',
        'Yemeni Flag'
    ],
    '🇾🇹': [
        'Flag of Mayotte',
        'YT',
        'Mayotte Flag'
    ],
    '🇿🇦': [
        'Flag of South Africa',
        'ZA',
        'South African Flag'
    ],
    '🇿🇲': [
        'Flag of Zambia',
        'ZM',
        'Zambian Flag'
    ],
    '🇿🇼': [
        'Flag of Zimbabwe',
        'ZW',
        'Zimbabwean Flag',
        'Zim Flag'
    ]
};
/*sports*/
emojis_language[lang][8] = {
    '🧗{{skin_tone}}‍‍♂️': [
        'Man Climbing',
        'Male Rock Climber',
        'Man Climber'
    ],
    '🧗{{skin_tone}}‍‍♀️': [
        'Woman Climbing',
        'Female Rock Climber',
        'Woman Climber'
    ],
    '🏇{{skin_tone}}‍': [
        'Horse Racing',
        'Horse Race',
        'Jockey'
    ],
    '⛷{{skin_tone}}‍️': ['Skier'],
    '🏂{{skin_tone}}‍': ['Snowboarder'],
    '🏌️{{skin_tone}}‍♂️': ['Man Golfer'],
    '🏌️{{skin_tone}}‍♀️': ['Woman Golfer'],
    '🏄{{skin_tone}}‍♂️': ['Man Surfer'],
    '🏄{{skin_tone}}‍♀️': ['Woman Surfer'],
    '🚣{{skin_tone}}‍‍♂️': ['Man Rowing Boat'],
    '🚣{{skin_tone}}‍‍♀️': ['Woman Rowing Boat'],
    '🏊{{skin_tone}}‍‍♂️': [
        'Man Swimming',
        'Man Swimmer'
    ],
    '🏊{{skin_tone}}‍‍♀️': [
        'Woman Swimming',
        'Woman Swimmer'
    ],
    '⛹{{skin_tone}}‍️‍♂️': [
        'Man Bouncing Ball',
        'Man Basketball Player'
    ],
    '⛹️{{skin_tone}}‍♀️': [
        'Woman Bouncing Ball',
        'Woman Basketball Player'
    ],
    '🏋️{{skin_tone}}‍♂️': [
        'Man Lifting Weights',
        'Man Weightlifter'
    ],
    '🏋️{{skin_tone}}‍♀️': [
        'Woman Lifting Weights',
        'Woman Weightlifter'
    ],
    '🚴{{skin_tone}}‍♂️': [
        'Man Biking',
        'Man Cyclist',
        'Male Cyclist'
    ],
    '🚴{{skin_tone}}‍♀️': [
        'Woman Biking',
        'Woman Cyclist',
        'Female Cyclist'
    ],
    '🚵{{skin_tone}}‍♂️': [
        'Man Mountain Biking',
        'Man Mountain Biker'
    ],
    '🚵{{skin_tone}}‍♀️': [
        'Woman Mountain Biking',
        'Woman Mountain Biker'
    ],
    '🤸{{skin_tone}}‍♂️': [
        'Man Cartwheeling',
        'Man Doing Cartwheel'
    ],
    '🤸{{skin_tone}}‍♀️': [
        'Woman Cartwheeling',
        'Woman Doing Cartwheel'
    ],
    '🤼': [
        'People Wrestling',
        'Wrestling',
        'Wrestlers'
    ],
    '🤽{{skin_tone}}‍♂️': ['Man Playing Water Polo'],
    '🤽{{skin_tone}}‍♀️': ['Woman Playing Water Polo'],
    '🤾{{skin_tone}}‍♂️': ['Man Playing Handball'],
    '🤾{{skin_tone}}‍♀️': ['Woman Playing Handball'],
    '🤹{{skin_tone}}‍♂️': [
        'Man Juggling',
        'Male Juggler'
    ],
    '🤹{{skin_tone}}‍♀️': [
        'Woman Juggling',
        'Female Juggler'
    ],
    '🧘{{skin_tone}}‍♂️': [
        'Man in Lotus Position',
        'Meditation',
        'Zen',
        'Cross legged',
        'Yoga'
    ],
    '🧘{{skin_tone}}‍♀️': [
        'Woman in Lotus Position',
        'Meditation',
        'Zen',
        'Cross legged',
        'Yoga'
    ],
    '🏃{{skin_tone}}‍♂️': ['Man Running'],
    '🏃{{skin_tone}}‍♀️': ['Woman Running'],
    '🕺{{skin_tone}}‍': ['Man Dancing'],
    '💃{{skin_tone}}‍': ['Woman Dancing'],
    '🤺': [
        'Person Fencing',
        'Fancer'
    ],
    '🏆': [
        'Trophy',
        'Championship',
        'Winner'
    ],
    '🏅': ['Sports Medal'],
    '🥇': [
        '1st Place Medal',
        'First Place Medal',
        'Gold Medal'
    ],
    '🥈': [
        '2nd Place Medal',
        'Second Place Medal',
        'Silver Medal'
    ],
    '🥉': [
        '3rd Place Medal',
        'Third Place Medal',
        'Bronze Medal'
    ],
    '⚽': [
        'Soccer',
        'Football'
    ],
    '⚾': [
        'Baseball',
        'Softball',
        'White'
    ],
    '🥎': [
        'Softball',
        'Yellow'
    ],
    '🏀': [
        'Basketball',
        'Orange',
        'Basketball and Hoop'
    ],
    '🏐': ['Volleyball'],
    '🏈': [
        'American Football',
        'Football',
        'Gridiron',
        'Superbowl'
    ],
    '🏉': [
        'Rugby Football',
        'Football',
        'League',
        'Rugby',
        'Union'
    ],
    '🎾': [
        'Tennis',
        'Ball',
        'Racket',
        'Racquet'
    ],
    '🥏': [
        'Flying Disk',
        'Frisbee'
    ],
    '🎳': [
        'Bowling',
        'Pins',
        'Skittles',
        'Ten Pin Bowling'
    ],
    '🏏': [
        'Cricket',
        'Bat',
        'Ball'
    ],
    '🏑': [
        'Field Hockey',
        'Ball',
        'Stick'
    ],
    '🏒': [
        'Ice Hockey',
        'Ball',
        'Stick'
    ],
    '🥍': [
        'Lacrosse',
        'Ball',
        'Stick'
    ],
    '🏓': [
        'Ping Pong',
        'Table Tennis',
        'Ball',
        'Paddle'
    ],
    '🏸': [
        'Badminton',
        'Shuttlecock',
        'Racquet'
    ],
    '🥊': ['Boxing Glove'],
    '🥋': [
        'Martial Arts Uniform',
        'Judo',
        'Karate',
        'Kung fu'
    ],
    '⛳': [
        'Flag in Hole',
        'Golf Flag'
    ],
    '⛸️': [
        'Ice Skate',
        'Skating'
    ],
    '🎣': [
        'Fishing',
        'Pole',
        'Fish',
        'Fishing Rod'
    ],
    '🎽': [
        'Running Shirt',
        'Singlet',
        'Sash'
    ],
    '🎿': [
        'Skis',
        'Skiing',
        'Boot'
    ],
    '🛷': ['Sled'],
    '🥌': ['Curling Stone'],
    '🎯': [
        'Direct Hit',
        'Archery',
        'Bullseye',
        'Darts'
    ],
    '🎱': [
        'Pool 8 Ball',
        'Cue Ball',
        'Snooker',
        'Billiards'
    ],
    '🎮': [
        'Video Game Controller',
        'Playstation',
        'Xbox',
        'Gamepad',
        'Wii U'
    ],
    '♟️': ['Black Chess Pawn'],
    '🏹': [
        'Bow and Arrow',
        'Arch'
    ],
    '🥅': ['Goal Net'],
    '🀄': [
        'Mahjong Red Dragon',
        'Mahjong',
        'Mahjong Tile',
        '中',
        'Mahjong Tile Red Dragon'
    ],
    '🎴': [
        'Flower Playing Cards',
        'Deck Of Cards',
        'Hanafuda',
        'Hwatu',
        'Playing Cards'
    ]
};
/*travel and places*/
emojis_language[lang][9] = {
    '🚂': [
        'Steam Locomotive',
        'Steam Train',
        'Railway Locomotive'
    ],
    '🚃': [
        'Railway Car',
        'Railcar',
        'Railroad Car',
        'Railway Carriage',
        'Railway Wagon'
    ],
    '🚄': ['High-Speed Train'],
    '🚅': [
        'Bullet Train',
        'Shinkansen',
        'High-Speed Train with Bullet Nose'
    ],
    '🚆': ['Train'],
    '🚇': [
        'Underground',
        'Tube',
        'Metro',
        'Subway'
    ],
    '🚈': [
        'Light Rail',
        'Train'
    ],
    '🚊': ['Tram'],
    '🚝': [
        'Monorail',
        'Train'
    ],
    '🚞': [
        'Mountain Railway',
        'Funicular',
        'Train And Mountain'
    ],
    '🚋': ['Tram Car'],
    '🚌': [
        'Bus',
        'Coach'
    ],
    '🚍': [
        'Oncoming Bus',
        'Front Of Bus'
    ],
    '🚎': [
        'Trolleybus',
        'Electical Bus'
    ],
    '🚐': [
        'Minibus',
        'Minivan',
        'People-Mover'
    ],
    '🚑': ['Ambulance'],
    '🚒': [
        'Fire Engine',
        'Fire Department',
        'Fire Truck'
    ],
    '🚓': [
        'Police Car',
        'Cop Car',
        'Side Of Police Car'
    ],
    '🚔': [
        'Oncoming Police Car',
        'Front of Police Car',
        'Cop Car'
    ],
    '🚕': [
        'Taxi',
        'New York Taxi',
        'Side Of Taxi'
    ],
    '🚖': [
        'Taxi',
        'Oncoming Taxi',
        'Front of Taxi'
    ],
    '🚗': [
        'Automobile',
        'Red Car',
        'Side Of Car'
    ],
    '🚘': [
        'Oncoming Automobile',
        'Red Car',
        'Front Of Car'
    ],
    '🚚': [
        'Delivery Truck',
        'Lorry'
    ],
    '🚛': [
        'Articulated Lorry',
        'Green Truck'
    ],
    '🚜': [
        'Tractor',
        'Farm'
    ],
    '🛻': ['Pickup Truck'],
    '🏎️': [
        'Racing Car',
        'F1',
        'Formula One',
        'Race Car'
    ],
    '🏍️': [
        'Motorbike',
        'Motorcycle'
    ],
    '🛵': [
        'Motor Scooter',
        'Vespa',
        'Motor Bike',
        'Motor Bike'
    ],
    '🛺': [
        'Auto Rickshaw',
        'Tuk Tuk'
    ],
    '🚲': [
        'Bicycle',
        'Bike'
    ],
    '🛴': ['Kick Scooter'],
    '⛵': [
        'Sailboat',
        'Dinghy',
        'Yacht'
    ],
    '🚤': [
        'Speedboat',
        'Motorboat',
        'Powerboat'
    ],
    '🛳️': ['Passenger Ship'],
    '⛴️': ['Ferry'],
    '🛥️': ['Motor Boat'],
    '🚢': ['Cruise Ship'],
    '✈️': [
        'Airplane',
        'Aeroplane'
    ],
    '🛩️': [
        'Small Airplane',
        'Small Aeroplane'
    ],
    '🚁': ['Helicopter'],
    '🚟': ['Suspension Railway'],
    '🚠': ['Mountain Cableway'],
    '🚡': [
        'Aerial Tramway',
        'Cable Car',
        'Gondola',
        'Ropeway'
    ],
    '🛰️': ['Satellite'],
    '🚀': [
        'Rocket',
        'Space Shuttle'
    ],
    '🛸': [
        'Flying Saucer',
        'UFO'
    ],
    '🚉': [
        'Train Station',
        'Train Platform'
    ],
    '🚏': ['Bus Stop'],
    '🛣️': [
        'Motorway',
        'Road',
        'Highway',
        'Interstate'
    ],
    '🛤️': ['Railway Track'],
    '🛫': [
        'Airplane Departure',
        'Taking Off'
    ],
    '🛬': [
        'Airplane Arrival',
        'Airplane Arriving',
        'Landing'
    ],
    '🗾': ['Map of Japan'],
    '⛰️': ['Mountain'],
    '🏔️': ['Snow Capped Mountain'],
    '🌋': ['Volcano'],
    '🗻': [
        'Mount Fuji',
        'Fuji-san'
    ],
    '🏕️': [
        'Camping',
        'Campsite'
    ],
    '🏖️': ['Beach With Umbrella'],
    '🏜️': ['Desert'],
    '🏝️': ['Desert Island'],
    '🏞️': ['National Park'],
    '🏟️': [
        'Stadium',
        'Grandstand',
        'Sport Stadium'
    ],
    '🏛️': [
        'Classical Building',
        'Greek',
        'Rome'
    ],
    '🏗️': [
        'Building Construction',
        'Crane'
    ],
    '🏘️': [
        'Houses Buildings',
        'Group of Houses'
    ],
    '🏚️': [
        'Derelict House Building',
        'Abandoned House',
        'Old House',
        'Haunted House'
    ],
    '🏠': [
        'Home',
        'House Building'
    ],
    '🏡': [
        'House With Garden',
        'House And Tree'
    ],
    '🏢': [
        'Office Building',
        'City Building',
        'High-Rise Building'
    ],
    '🏣': [
        'Japanese Post Office',
        'Japanese Postal Mark'
    ],
    '🏤': [
        'Post Office',
        'European Post Office'
    ],
    '🏥': [
        'Hospital',
        'ER',
        'A&E',
        'Emergency Room',
        'Accident And Emergency',
        'Red Cross',
        'Medical'
    ],
    '🏦': [
        'Bank',
        'BK',
        'Bakkureru',
        'Bank Branch',
        'Bakkureru'
    ],
    '🏨': [
        'Hotel',
        'Accommodation',
        'H Building'
    ],
    '🏩': [
        'Love Hotel',
        'Love Heart Hotel'
    ],
    '🏪': [
        'Convenience Store',
        '24-Hour Store',
        '7-Eleven',
        'Corner Shop',
        'Kwik-E-Mart'
    ],
    '🏫': [
        'School',
        'Clock Tower',
        'Elementary High Middle'
    ],
    '🏬': [
        'Department Store',
        'Shopping Center',
        'Shop'
    ],
    '🏭': [
        'Factory',
        'Industrial',
        'Industry',
        'Pollution',
        'Smog'
    ],
    '🏯': [
        'Japanese Castle',
        'Fortress'
    ],
    '🏰': [
        'European Castle',
        'Turrets'
    ],
    '💒': [
        'Church Heart',
        'Church Wedding',
        'Marriage'
    ],
    '🗼': [
        'Tokyo Tower',
        'Eiffel Tower',
        'Red Tower'
    ],
    '🗽': [
        'Statue of Liberty',
        'New York'
    ],
    '⛪': [
        'Church Building',
        'Cross'
    ],
    '🕌': [
        'Mosque',
        'Domed Roof',
        'Minaret'
    ],
    '🛕': ['Hindu Temple'],
    '🕍': [
        'Synagogue',
        'Jewish',
        'Synagog',
        'Temple'
    ],
    '⛩️': [
        'Shinto Shrine',
        'Kami-no-michi'
    ],
    '🕋': [
        'Kaaba',
        'Mecca'
    ],
    '🏙️': ['Cityscape'],
    '⛱️': [
        'Umbrella on Ground',
        'Beach Umbrella'
    ],
    '🌄': [
        'Sunrise Over Mountains',
        'Morning',
        'Sunrise'
    ],
    '🌅': [
        'Sunrise',
        'Sunset'
    ],
    '🌆': [
        'Cityscape at Dusk',
        'Orange Sky City',
        'Dusk City'
    ],
    '🌇': [
        'Sunset Over Buildings',
        'City Sunset'
    ],
    '🌃': [
        'Night with Stars',
        'City At Night',
        'Starry Night'
    ],
    '🗿': [
        'Moai',
        'Easter Island Statue',
        'Human Rock Carving',
        'Moyai Statue'
    ],
    '🌠': [
        'Shooting Star',
        'When You Wish Upon A Star',
        'Meteoroid'
    ],
    '🌌': [
        'Milky Way',
        'Galaxy',
        'Night Sky',
        'Space',
        'Stars',
        'Universe'
    ],
    '🎑': [
        'Moon Viewing Ceremony',
        'Grass, Dumplings and Moon',
        'Harvest Moon',
        'Mid-Autumn Festival',
        'Tsukimi'
    ],
    '🛖': ['Hut'],
    '🎢': [
        'Roller Coaster',
        'Rollercoaster',
        'Theme Park'
    ],
    '🛝': [
        'Playground Slide',
        'Baby',
        'Children'
    ]
};
/*technologies (smartphones, laptops, etc.) and office*/
emojis_language[lang][10] = {
    '⌚': [
        'Watch',
        'Timepiece',
        'Apple Watch'
    ],
    '⏰': ['Alarm Clock'],
    '⏱️': ['Stopwatch'],
    '⏲️': ['Timer Clock'],
    '🕰️': ['Mantelpiece Clock'],
    '🧭': ['Compass'],
    '🕹️': ['Joystick'],
    '🎙️': ['Studio Microphone'],
    '🎚️': ['Level Slider'],
    '🎛️': ['Control Knobs'],
    '📻': [
        'Radio',
        'Digital Radio',
        'Wireless'
    ],
    '📱': [
        'Mobile Phone',
        'Smartphone',
        'iPhone',
        'Cell Phone'
    ],
    '📲': [
        'Mobile Phone with Rightwards Arrow at Left',
        'Phone Call',
        'Pointing To Phone'
    ],
    '☎️': [
        'Black Telephone',
        'Rotary Phone',
        'Red'
    ],
    '📞': [
        'Telephone Receiver',
        'Handset',
        'Phone',
        'Black'
    ],
    '📟': [
        'Pager',
        'Beeper',
        'Bleeper'
    ],
    '📠': [
        'Fax Machine',
        'Facsimile',
        'Fax'
    ],
    '🔋': [
        'Battery',
        'AA Battery',
        'Phone Battery'
    ],
    '🔌': [
        'Electric Plug',
        'AC Adaptor',
        'Power Cable',
        'Power Plug'
    ],
    '💻': [
        'Laptop',
        'Notebook',
        'Personal Computer',
        'PC',
        'MacBook',
        'Surface'
    ],
    '🖥️': [
        'Desktop Computer',
        'iMac',
        'PC'
    ],
    '🖨️': ['Printer'],
    '⌨️': ['Keyboard'],
    '🖱️': [
        'Computer Mouse',
        'Three Button Mouse'
    ],
    '🖲️': ['Trackball'],
    '💽': [
        'Computer Disk',
        'Minidisc',
        'CD',
        'DVD',
        'Blu-ray',
        'BD'
    ],
    '💾': [
        'Floppy Disk',
        '3.5″ Disk',
        'Disk'
    ],
    '💿': [
        'Optical Disk',
        'CD',
        'CD-ROM',
        'Compact Disc'
    ],
    '📀': [
        'DVD',
        'DVD-ROM',
        'DVD Video'
    ],
    '🎥': [
        'Movie Camera',
        'Film Camera',
        'Hollywood',
        'Movie'
    ],
    '🎞️': ['Film Frames'],
    '📽️': ['Film Projector'],
    '📺': [
        'Television',
        'TV'
    ],
    '📷': ['Digital Camera'],
    '📸': ['Camera with Flash'],
    '📹': [
        'Video Camera',
        'Camcorder'
    ],
    '📼': [
        'Videocassette',
        'VHS',
        'VCR',
        'Video Tape',
        'Videocassette'
    ],
    '💳': [
        'Credit Card',
        'Debit Card',
        'AMEX',
        'Diners Club',
        'American Express',
        'Mastercard',
        'VISA Card'
    ],
    '📧': [
        'E-Mail',
        'Email'
    ],
    '📨': [
        'Incoming Envelope',
        'Envelope With Lines',
        'Fast Envelope'
    ],
    '📩': [
        'Envelope with Downwards Arrow Above',
        'Down Arrow Envelope',
        'Insert In Envelope'
    ],
    '📤': ['Outbox Tray'],
    '📥': ['Inbox Tray'],
    '📮': ['Postbox'],
    '📁': [
        'File Folder',
        'Folder',
        'Directory',
        'Closed File Folder'
    ],
    '📂': [
        'Open File Folder',
        'Directory'
    ],
    '🗂️': ['Card Index Dividers'],
    '📅': [
        'Calendar',
        'July 17',
        'World Emoji Day'
    ],
    '🗓️': ['Spiral Calendar Pad'],
    '📆': [
        'Tear-Off Calendar',
        'Day Calendar',
        'Desk Calendar'
    ],
    '📈': [
        'Chart Increasing',
        'Positive Chart',
        'Up Pointing Graph',
        'Chart with Upwards Trend'
    ],
    '📉': [
        'Chart Decreasing',
        'Negative Chart',
        'Down Pointing Graph',
        'Chart with Downwards Trend'
    ],
    '📊': [
        'Bar Chart',
        'Bar Graph'
    ],
    '📌': [
        'Pushpin',
        'Thumb Tack'
    ],
    '📋': ['Clipboard'],
    '📍': [
        'Round Pushpin',
        'Dropped Pin',
        'Map Pin',
        'Pin',
        'Red Pin'
    ],
    '📎': [
        'Paperclip',
        'Clippy'
    ],
    '📏': ['Straight Ruler'],
    '📐': [
        'Triangular Ruler',
        'Triangle Ruler'
    ],
    '✂️': [
        'Scissors',
        'Cutting'
    ],
    '✏️': [
        'Pencil',
        'Lead Pencil'
    ],
    '✒️': [
        'Black Nib',
        'Pen Nib',
        'Fountain Pen'
    ],
    '🖋️': [
        'Fountain Pen',
        'Lower Left Fountain Pen'
    ],
    '🖊️': [
        'Pen',
        'Lower Left Ballpoint Pen',
        'Ballpoint Pen'
    ],
    '🖌️': [
        'Paintbrush',
        'Brush',
        'Lower Left Paintbrush'
    ],
    '🖍️': [
        'Crayon',
        'Lower Left Crayon'
    ],
    '📝': [
        'Memo',
        'Memorandum',
        'Note',
        'Pencil And Paper'
    ],
    '🗑️': [
        'Wastepaper Basket',
        'Garbage Can',
        'Rubbish Bin',
        'Trash Can'
    ],
    '📡': ['Satellite Antenna'],
    '🔭': [
        'Telescope',
        'Stargazing'
    ],
    '🔬': [
        'Microscope',
        'Magnify',
        'Science'
    ],
    '📭': ['Open Mailbox with Lowered Flag'],
    '📬': [
        'Open Mailbox with Raised Flag',
        'Open Mailbox'
    ],
    '📪': ['Closed Mailbox with Lowered Flag'],
    '📫': ['Closed Mailbox with Raised Flag'],
    '📰': ['Newspaper'],
    '🗞️': [
        'Rolled-Up Newspaper',
        'Newspaper Delivery'
    ],
    '📄': [
        'Page Facing Up',
        'Printed Page'
    ],
    '📜': [
        'Scroll',
        'Degree',
        'Parchment'
    ],
    '📃': [
        'Page with Curl',
        'Curled Page',
        'Curly Page'
    ],
    '📓': [
        'Notebook',
        'Black And White Book'
    ],
    '📔': ['Notebook with Decorative Cover'],
    '📕': [
        'Closed Book',
        'Red Book',
        'Red Textbook'
    ],
    '📖': [
        'Open Book',
        'Book',
        'Novel'
    ],
    '📗': [
        'Green Book',
        'Green Textbook'
    ],
    '📘': [
        'Blue Book',
        'Blue Textbook'
    ],
    '📙': [
        'Orange Book',
        'Orange Textbook'
    ],
    '📚': [
        'Books',
        'Pile Of Books',
        'Stack Of Books'
    ],
    '🗳️': [
        'Ballot Box with Ballot',
        'Vote Box',
        'Voting'
    ],
    '🗄️': [
        'File Cabinet',
        'Filing Cabinet'
    ],
    '🗃️': ['Card File Box'],
    '🖇️': ['Linked Paperclips']
};
/*clothes and accessories*/
emojis_language[lang][11] = {
    '👣': [
        'Footprints',
        'Feet',
        'Footsteps'
    ],
    '👓': [
        'Glasses',
        'Eveyglasses'
    ],
    '🕶️': ['Sunglasses'],
    '🥽': ['Goggles'],
    '🥼': ['Lab Coat'],
    '👔': [
        'Necktie',
        'Business Shirt',
        'Shirt And Tie'
    ],
    '👕': [
        'T-Shirt',
        'Tee Shirt',
        'Polo Shirt'
    ],
    '👖': [
        'Jeans',
        'Denim',
        'Pants',
        'Trousers'
    ],
    '🧣': ['Scarf'],
    '🧤': ['Gloves'],
    '🧥': ['Coat'],
    '🧦': ['Socks'],
    '👗': [
        'Dress',
        'Skirt',
        'Gown'
    ],
    '👘': [
        'Kimono',
        'Dressing Gown',
        'Japanese Dress'
    ],
    '👙': [
        'Bikini',
        'Bathers',
        'Swimsuit'
    ],
    '🩱': ['One-Piece Swimsuit'],
    '👚': [
        'Womans Clothes',
        'Blouse',
        'Pink Shirt',
        'Blouse'
    ],
    '👛': [
        'Purse',
        'Wallet'
    ],
    '👜': ['Handbag'],
    '👝': [
        'Clutch Bag',
        'Clutch',
        'Small Bag',
        'Pouch'
    ],
    '🎒': [
        'Backpack',
        'Bag',
        'School Bag',
        'School Satchel',
        'Rucksack'
    ],
    '👞': [
        'Mans Shoe',
        'Shoe',
        'Brown Show',
        'Dress Show'
    ],
    '👟': [
        'Athletic Shoe',
        'Running Shoe',
        'Sneaker',
        'Runner',
        'Trainer',
        'Tennis Shoe'
    ],
    '🥾': ['Hiking Boot'],
    '🥿': ['Flat Shoe'],
    '👠': [
        'High-Heeled Shoe',
        'Stiletto',
        'High Heels'
    ],
    '👡': [
        'Womans Sandal',
        'Heeled Sandal',
        'Saldals'
    ],
    '👢': [
        'Womans Boots',
        'Cowgirl Boots',
        'Knee High Boots',
        'Heeled Boots'
    ],
    '👑': [
        'Crown',
        'King',
        'Queen',
        'Prince',
        'Royal',
        'Princess'
    ],
    '👒': [
        'Hat',
        'Womans Hat',
        'Hat With Bow',
        'Ladies Hat'
    ],
    '🎩': [
        'Top Hat',
        'Formal Wear',
        'Groom'
    ],
    '🎓': [
        'Graduation Cap',
        'College',
        'Graduate',
        'Mortar Board',
        'University',
        'Square Academic Cap'
    ],
    '🧢': [
        'Billed Cap',
        'Baseball Cap'
    ],
    '⛑️': [
        'Helmet with White Cross',
        'Rescue Worker\'s Helmet'
    ],
    '💼': [
        'Briefcase',
        'Suitcase'
    ],
    '🎖️': [
        'Military Medal',
        'Medal',
        'Medallion',
        'Military Decoration'
    ],
    '📿': [
        'Prayer Beads',
        'Dhikr Beads',
        'Rosary Beads'
    ],
    '🎗️': ['Reminder Ribbo'],
    '🤿': ['Diving Mask'],
    '🦺': ['Safety Vest'],
    '🥻': [
        'Sari',
        'Saree',
        'Shari'
    ],
    '🩲': ['Briefs'],
    '🩳': ['Shorts'],
    '🛼': [
        'Roller Skate',
        'Inline Skate',
        'Roller Derby'
    ],
    '🩴': [
        'Flip Flop',
        'Thong Sandal'
    ],
    '🪖': ['Military Helmet'],
    '🩰': [
        'Ballet Shoes',
        'Pointe Shoe'
    ],
    '🩼': ['Crutch']
};
/*hands and parts of body*/
emojis_language[lang][12] = {
    '👋{{skin_tone}}': [
        'Waving Hand Sign',
        'Goodbye',
        'Hand Wave',
        'Hello',
        'Hi'
    ],
    '🖐️{{skin_tone}}': [
        'Hand with Fingers Splayed',
        'Five Hand',
        'Splayed Hand',
        'Raised Hand with Fingers Splayed'
    ],
    '🤚{{skin_tone}}': [
        'Raised Back of Hand',
        'Backhand'
    ],
    '✋{{skin_tone}}': [
        'Raised Hand',
        'High Five',
        'Stop'
    ],
    '🖖{{skin_tone}}': [
        'Vulcan Salute',
        'Spock',
        'Star Trek',
        'Raised Hand with Part Between Middle and Ring Fingers'
    ],
    '👌{{skin_tone}}': [
        'OK Hand Sign',
        'Okay',
        'Perfect',
        'Yes'
    ],
    '✌️{{skin_tone}}': [
        'Victory Hand',
        'Air Quotes',
        'Peace Sign',
        'V Sign'
    ],
    '🤞{{skin_tone}}': [
        'Crossed Fingers',
        'Fingers Crossed',
        'Good Luck',
        'Hand with Index and Middle Fingers Crossed'
    ],
    '🤟{{skin_tone}}': [
        'Love-You Gesture',
        'I Love You Hand Sign'
    ],
    '🤘{{skin_tone}}': [
        'Sign of the Horns',
        'Devil Fingers',
        'Heavy Metal',
        'Rock On'
    ],
    '🤙{{skin_tone}}': [
        'Call Me Hand Sign',
        'Phone Hand',
        'Shaka'
    ],
    '🤏{{skin_tone}}': ['Pinching Hand'],
    '👈{{skin_tone}}': [
        'Backhand Index Pointing Left',
        'White Left Pointing Backhand Index'
    ],
    '👉{{skin_tone}}': [
        'Backhand Index Pointing Right',
        'White Right Pointing Backhand Index'
    ],
    '👆{{skin_tone}}': [
        'Backhand Index Pointing Up',
        'White Up Pointing Backhand Index',
        'Middle Finger'
    ],
    '👇{{skin_tone}}': [
        'Backhand Index Pointing Down',
        'White Down Pointing Backhand Index',
        'Pointing Down'
    ],
    '☝{{skin_tone}}': [
        'Index Finger Pointing Up',
        'White Up Pointing Index',
        'Secret'
    ],
    '🖕{{skin_tone}}': [
        'Middle Finger',
        'Flipping The Bird',
        'Rude Finger',
        'Reversed Hand with Middle Finger Extended'
    ],
    '👍{{skin_tone}}': [
        'Thumbs Up Sign',
        'Like',
        'Yes'
    ],
    '👎{{skin_tone}}': [
        'Thumbs Down Sign',
        'Bad',
        'Dislike',
        'No'
    ],
    '✊{{skin_tone}}': [
        'Raised Fist',
        'Fist Pump'
    ],
    '👊{{skin_tone}}': [
        'Oncoming Fist',
        'Bro Fist',
        'Brofist',
        'Fist Bump',
        'Punch',
        'Closed Fist',
        'Fisted Hand Sign'
    ],
    '🤛{{skin_tone}}': ['Left Fist Bump'],
    '🤜{{skin_tone}}': ['Right Fist Bump'],
    '🤌{{skin_tone}}': [
        'Pinched Fingers',
        'Ma Che Vuoi',
        'Finger Purse'
    ],
    '👏{{skin_tone}}': [
        'Clapping Hands Sign',
        'Applause',
        'Clap',
        'Clapping',
        'Golf Clap',
        'Round Of Applause'
    ],
    '🙌{{skin_tone}}': [
        'Person Raising Both Hands in Celebration',
        'Raising Hands',
        'Arms In The Air',
        'Banzai',
        'Festivus Miracle',
        'Hallelujah',
        'Praise Hands',
        'Two Hands'
    ],
    '👐{{skin_tone}}': [
        'Open Hands Sign',
        'Hug',
        'Jazz Hands'
    ],
    '🤲{{skin_tone}}': [
        'Palms Up Together',
        'Dua'
    ],
    '🤝{{skin_tone}}': [
        'Handshake',
        'Shaking Hands'
    ],
    '🙏{{skin_tone}}': [
        'Folded Hands',
        'Namaste',
        'Please',
        'Prayer',
        'Thank You',
        'Person with Folded Hands'
    ],
    '✍{{skin_tone}}': ['Writing Hand'],
    '🫱{{skin_tone}}': ['Rightwards Hand'],
    '🫲{{skin_tone}}': ['Leftwards Hand'],
    '🫳{{skin_tone}}': ['Palm Down Hand'],
    '🫴{{skin_tone}}': ['Palm Up Hand'],
    '🫰{{skin_tone}}': ['Hand with Index Finger and Thumb Crossed'],
    '🫵{{skin_tone}}': ['Index Pointing at the Viewer'],
    '🫶{{skin_tone}}': [
        'Heart Hands',
        'Love'
    ],
    '💪{{skin_tone}}': [
        'Flexed Biceps',
        'Flexing Arm Muscles',
        'Musle',
        'Strong'
    ],
    '🦵{{skin_tone}}': ['Leg'],
    '🦶{{skin_tone}}': ['Foot'],
    '👂{{skin_tone}}': [
        'Ear',
        'Ears',
        'Hearing',
        'Listening'
    ],
    '👃{{skin_tone}}': [
        'Nose',
        'Smelling',
        'Sniffing',
        'Stinky'
    ],
    '🦻{{skin_tone}}': ['Ear with Hearing Aid'],
    '🧠': ['Brain'],
    '🦷': ['Tooth'],
    '👀': [
        'Eyes',
        'Eyeballs',
        'Shifty Eyes',
        'Wise Eyes'
    ],
    '👁️': [
        'Eye',
        'Single Eye'
    ],
    '👅': ['Tongue Out'],
    '👄': [
        'Mouth',
        'Kissing Lips',
        'Lips'
    ],
    '🫦': [
        'Biting Lip',
        'Lips'
    ],
    '🫀': ['Anatomical Heart'],
    '🫁': ['Lungs'],
    '🦾': ['Mechanical Arm'],
    '🦿': ['Mechanical Leg'],
    '💅{{skin_tone}}': [
        'Nail Polish',
        'Fingers',
        'Manicure',
        'Nonchalant'
    ],
    '🤳{{skin_tone}}': [
        'Selfie',
        'Phone Camera',
        'Selfie Hand'
    ]
};
/*other*/
emojis_language[lang][13] = {
    '💩': [
        'Pile of Poo',
        'Dog Dirt',
        'Smiling Poop'
    ],
    '💋': [
        'Kiss Mark',
        'Kissing Lips'
    ],
    '🦴': ['Bone'],
    '🧳': [
        'Luggage',
        'Suitcase'
    ],
    '🌂': [
        'Closed Umbrella',
        'Collapsed Umbrella',
        'Pink Umbrella'
    ],
    '☂️': ['Umbrella'],
    '☔': [
        'Umbrella with Rain Drops',
        'Raining',
        'Rainy'
    ],
    '🧵': ['Spool of Thread'],
    '🧶': ['Ball of Yarn'],
    '💄': [
        'Lipstick',
        'Lip Gloss',
        'Makeup'
    ],
    '💍': [
        'Ring',
        'Diamond Ring',
        'Engagement Ring'
    ],
    '💎': [
        'Gem stone',
        'Diamond',
        'Gem',
        'Gemstone',
        'Jewel'
    ],
    '💥': [
        'Collision Symbol',
        'Bang',
        'Explode',
        'Impact',
        'Red Spark',
        'Explosion'
    ],
    '💫': [
        'Dizzy Symbol',
        'Circle And Star'
    ],
    '💦': [
        'Splashing Sweat Symbol',
        'Plewds',
        'Splashing Water',
        'Water Drops'
    ],
    '💨': [
        'Dashing Away',
        'Fast',
        'Steam',
        'Vaping',
        'Wind',
        'Dash Symbol',
        'Gust of Wind'
    ],
    '🌬️': [
        'Wind Face',
        'Blowing Wind',
        'Mother Nature',
        'Wind Blowing Face'
    ],
    '🐾': [
        'Paw Prints',
        'Cat Paw Prints',
        'Dog Paw Prints',
        'Kitten Paw Prints',
        'Puppy Paw Prints'
    ],
    '🕸️': [
        'Spider Web',
        'Cobweb',
        'Web',
        'Spiderweb'
    ],
    '🦠': [
        'Microbe',
        'Cell',
        'Coronavirus',
        'COVID-19',
        'Germ',
        'Microorganism',
        'Virus'
    ],
    '💐': ['Bouquet Of Flowers'],
    '🌍': [
        'Globe Showing Europe-Africa',
        'Earth',
        'Globe',
        'Planet',
        'World',
        'Globe Showing Europe and Africa'
    ],
    '🌎': [
        'Globe Showing Americas',
        'Earth',
        'Globe',
        'Planet',
        'World'
    ],
    '🌏': [
        'Globe Showing Asia-Australia',
        'Earth',
        'Globe',
        'Planet',
        'World',
        'Globe Showing Asia and Australia'
    ],
    '🌐': [
        'Globe with Meridians',
        'Internet',
        'World Wide Web',
        'WWW'
    ],
    '🌑': [
        'New Moon Symbol',
        'Dark Moon',
        'Shadow Moon',
        'Solar Eclipse'
    ],
    '🌒': ['Waxing Crescent Moon Symbol'],
    '🌓': ['First Quarter Moon Symbol'],
    '🌔': ['Waxing Gibbous Moon Symbol'],
    '🌕': ['Full Moon Symbol'],
    '🌖': ['Waning Gibbous Moon Symbol'],
    '🌗': ['Last Quarter Moon Symbol'],
    '🌘': ['Waning Crescent Moon Symbol'],
    '🌙': ['Crescent Moon'],
    '🌚': [
        'New Moon with Face',
        'Creepy Moon',
        'Dark Moon Face',
        'Molester Moon'
    ],
    '🌝': [
        'Full Moon with Face',
        'Moonface',
        'Smiley Moon',
        'Smiling Moon'
    ],
    '🌛': ['First Quarter Moon with Face'],
    '🌜': ['Last Quarter Moon with Face'],
    '🌞': [
        'Sun with Face',
        'Smiley Sun',
        'Sunface',
        'Smiling Sun'
    ],
    '☀️': [
        'Sun',
        'Sunshine'
    ],
    '⭐': [
        'Star',
        'White Medium Star'
    ],
    '🌟': [
        'Glowing Star',
        'Shining Star'
    ],
    '☁️': [
        'Cloud',
        'Cloudy',
        'Overcast'
    ],
    '⛅': ['Sun Behind Cloud'],
    '⛈️': [
        'Cloud with Lightning and Rain',
        'Thunder Cloud and Rain'
    ],
    '🌤️': ['White Sun with Small Cloud'],
    '🌥️': [
        'Sun Behind Large Cloud',
        'White Sun Behind Cloud'
    ],
    '🌦️': ['White Sun Behind Cloud with Rain'],
    '🌧️': ['Cloud with Rain'],
    '🌨️': ['Cloud with Snow'],
    '🌩️': ['Cloud with Lightning'],
    '🌪️': [
        'Tornado',
        'Cloud with Tornado'
    ],
    '🌫️': ['Fog'],
    '🌈': [
        'Rainbow',
        'Gay Pride',
        'Primary Rainbow',
        'LGBTQIA+',
        'LGBT+'
    ],
    '❄️': [
        'Snowflake',
        'Snow',
        'Snowing'
    ],
    '🌀': [
        'Cyclone',
        'Hurricane',
        'Spiral',
        'Swirl',
        'Tornado'
    ],
    '⚡': [
        'High Voltage Sign',
        'Lightning Bolt',
        'Thunderbolt',
        'High Voltage'
    ],
    '☃️': [
        'Snowman',
        'Snowman With Snowflakes'
    ],
    '⛄': [
        'Snowman Without Snow',
        'Frosty The Snowman',
        'Olaf',
        'Snowman'
    ],
    '☄️': ['Comet'],
    '🔥': [
        'Fire',
        'Flame',
        'Hot',
        'Lit',
        'Snapstreak'
    ],
    '💧': [
        'Droplet',
        'Water',
        'Water Drop'
    ],
    '✨': [
        'Sparkles',
        'Glitter',
        'Shiny',
        'New'
    ],
    '🌊': [
        'Water Wave',
        'Beach',
        'Ocean Wave',
        'Sea'
    ],
    '🎄': [
        'Christmas Tree',
        'Christmas',
        'Xmas Tree'
    ],
    '🎃': [
        'Jack-O-Lantern',
        'Halloween',
        'Pumpkin'
    ],
    '🌻': [
        'Sunflower',
        'Yellow Flower'
    ],
    '🌼': [
        'Blossom',
        'Blossoming Flower',
        'Daisy',
        'Yellow Flower'
    ],
    '🌷': ['Tulip'],
    '🌹': [
        'Rose Flower',
        'Red Rose'
    ],
    '☘️': [
        'Shamrock',
        'Clover',
        'Trefoil'
    ],
    '🍀': [
        'Four Leaf Clover',
        'Clover',
        'Ireland',
        'Lucky',
        'Fortune'
    ],
    '🌸': [
        'Cherry Blossom',
        'Pink Flower',
        'Sakura'
    ],
    '💮': [
        'White Flower',
        'Cherry Blossom',
        'Paper Doily',
        'Well Done Stamp'
    ],
    '🌺': ['Hibiscus'],
    '🏵️': ['Rosette'],
    '🌱': [
        'Seedling',
        'Spring',
        'Sprout',
        'Sprouting'
    ],
    '🌲': [
        'Evergreen Tree',
        'Fir Tree',
        'Pine Tree'
    ],
    '🌳': [
        'Deciduous Tree',
        'Rounded Tree'
    ],
    '🌴': [
        'Palm Tree',
        'Coconut Tree'
    ],
    '🎋': [
        'Tanabata Tree',
        'Tanabata',
        'Wish Tree'
    ],
    '🎍': [
        'Pine Decoration',
        'Bamboo',
        'Kadomatsu',
        'New Year Decoration'
    ],
    '🌵': [
        'Cactus',
        'Desert'
    ],
    '🌾': [
        'Sheaf of Rice',
        'Crop',
        'Farming',
        'Wheat',
        'Ear of Rice'
    ],
    '🌿': [
        'Herb',
        'Crop',
        'Plant'
    ],
    '🍁': [
        'Maple Leaf',
        'Canada',
        'Canadian',
        'Maple'
    ],
    '🍂': [
        'Fallen Leaf',
        'Autumn Leaf',
        'Autumn Leaves',
        'Brown Leaves',
        'Fall Leaves',
        'Fallen Leaves'
    ],
    '🍃': [
        'Leaf Fluttering in Wind',
        'Green Leaves',
        'Spring'
    ],
    '🪴': ['Potted Plant'],
    '🪸': [
        'Coral',
        'Climate change'
    ],
    '🪷': [
        'Lotus',
        'Purity',
        'Beauty',
        'Calmness',
        'Serenity'
    ],
    '🔑': [
        'Key',
        'Gold Key'
    ],
    '🗝️': ['Old Key'],
    '🔨': [
        'Hammer',
        'Claw Hammer',
        'Handyman',
        'Tool'
    ],
    '⛏️': [
        'Pick',
        'Pickaxe'
    ],
    '🪓': ['Axe'],
    '🪚': ['Carpentry Saw'],
    '🪛': ['Screwdriver'],
    '🪝': ['Hook'],
    '🪠': ['Plunger'],
    '🪣': ['Bucket'],
    '🔩': [
        'Nut and Bolt',
        'Bolt',
        'Screw'
    ],
    '🗜️': [
        'Clamp',
        'Table Vice',
        'WinZip',
        'Compression'
    ],
    '🔒': [
        'Locked',
        'Closed Lock',
        'Padlock'
    ],
    '🔓': [
        'Unlocked',
        'Open Padlock',
        'Open Lock'
    ],
    '🔏': [
        'Locked with Pen',
        'Lock And Pen',
        'Lock With Fountain Pen',
        'Lock with Ink Pen'
    ],
    '🔐': [
        'Locked with Key',
        'Closed Lock with Key'
    ],
    '🗡️': [
        'Dagger',
        'Knife Weapon'
    ],
    '⚔️': ['Crossed Swords'],
    '🔫': [
        'Pistol',
        'Gun',
        'Revolver',
        'Squirt Gun',
        'Water Gun',
        'Water Pistol'
    ],
    '🧰': ['Toolbox'],
    '🧲': ['Magnet'],
    '⛓️': ['Chains'],
    '💉': [
        'Syringe',
        'Blood Donation',
        'Blood Test',
        'Needle',
        'Vaccination'
    ],
    '🧬': [
        'DNA',
        'DNA Double Helix'
    ],
    '🧪': ['Test Tube'],
    '🛏️': [
        'Bed',
        'Bedroom'
    ],
    '🛋️': [
        'Couch and Lamp',
        'Lounge',
        'Settee',
        'Sofa'
    ],
    '🪑': ['Chair'],
    '🚪': [
        'Door',
        'Doorway',
        'Front Door'
    ],
    '🪟': ['Window'],
    '🪜': ['Ladder'],
    '🪞': ['Mirror'],
    '🚽': [
        'Toilet',
        'Bathroom',
        'Loo',
        'Restroom',
        'WC'
    ],
    '🚿': [
        'Shower',
        'Shower Head'
    ],
    '🛁': [
        'Bathtub',
        'Bubble Bath'
    ],
    '🧴': ['Lotion Bottle'],
    '🧷': ['Safety Pin'],
    '🧺': ['Basket'],
    '🧻': [
        'Roll of Paper',
        'Toilet Paper'
    ],
    '🧼': ['Bar of Soap'],
    '🧽': ['Sponge'],
    '🧯': ['Fire Extinguisher'],
    '🚬': [
        'Cigarette',
        'Smoke Symbol'
    ],
    '⚰️': [
        'Coffin',
        'Casket',
        'Funeral'
    ],
    '⚱️': [
        'Funeral Urn',
        'Vase'
    ],
    '🛡️': ['Shield'],
    '⚙️': [
        'Gear',
        'Settings',
        'Options'
    ],
    '⚖️': [
        'Balance Scale',
        'Scales of Justice'
    ],
    '🔗': [
        'Link',
        'Chain',
        'Hyperlink',
        'Linked Chain'
    ],
    '💀': [
        'Skull',
        'Death',
        'Grey Skull',
        'Skeleton'
    ],
    '☠️': ['Skull and Crossbones'],
    '🎡': [
        'Ferris Wheel',
        'Big Wheel',
        'Fairgroundl',
        'Observation Wheel'
    ],
    '⛽': [
        'Fuel Pump',
        'Oil',
        'Petrol',
        'Gas'
    ],
    '🚨': [
        'Police Cars Revolving Light',
        'Emergency Light',
        'Flashing Light',
        'Siren'
    ],
    '🚥': ['Horizontal Traffic Light'],
    '🚦': ['Vertical Traffic Light'],
    '⚓': [
        'Admiralty Pattern Anchor',
        'Fisherman'
    ],
    '🚧': [
        'Construction Sign',
        'Black And Yellow Striped Sign'
    ],
    '🪂': ['Parachute'],
    '💺': [
        'Seat',
        'Bus Airplane Train Aeroplane'
    ],
    '🪐': ['Saturn'],
    '🎆': [
        'Fireworks',
        'Explosion'
    ],
    '🎇': [
        'Senko Hanabi',
        'Firework Sparkler'
    ],
    '💷': [
        'Pound Banknote',
        '£20 Note',
        'Twenty Quid Note',
        'Banknote with Pound Sign'
    ],
    '💶': [
        'Euro Banknote',
        '€100 Note',
        'Banknote with Euro Sign'
    ],
    '💵': [
        'Dollar Banknote',
        '$1 Note',
        'Banknote with Dollar Sign',
        'Dollar Bill',
        'American Dollar'
    ],
    '💴': [
        'Yen Banknote',
        '¥1000 Note',
        'Yen Note',
        'Banknote with Yen Sign'
    ],
    '⛺': ['Camping Tent'],
    '⛲': [
        'Fountain',
        'Water Feature',
        'Water Fountain',
        'Park'
    ],
    '💰': [
        'Money Bag',
        'Moneybags',
        'Rich',
        '$'
    ],
    '💸': [
        'Money with Wings',
        'Flying Money',
        ' Losing Money'
    ],
    '🏷️': ['Label Tag'],
    '🔖': [
        'Bookmark',
        'Price Tag',
        'Tag'
    ],
    '📑': ['Bookmark Tabs'],
    '📯': [
        'Postal Horn',
        'Bugle',
        'French Horn'
    ],
    '🪁': ['Kite'],
    '🪀': ['Yo-Yo'],
    '🎊': [
        'Confetti Ball',
        'Confetti'
    ],
    '🎉': [
        'Party Popper',
        'Celebration',
        'Party Hat'
    ],
    '🎈': [
        'Balloon',
        'Party',
        'Red Balloon'
    ],
    '🌡️': [
        'Thermometer',
        'Hot Weather',
        'Temperature'
    ],
    '🛢️': ['Oil Drum'],
    '💣': ['Bomb'],
    '🔪': [
        'Kitchen Knife',
        'Knife',
        'Butchers Knife',
        'Hocho'
    ],
    '🏺': [
        'Amphora',
        'Jar',
        'Vase'
    ],
    '🗺️': ['World Map'],
    '🎼': [
        'Musical Score',
        'Sheet Music',
        'Treble Clef'
    ],
    '🎬': [
        'Clapper Board',
        'Cut',
        'Action',
        'Director',
        'Film Slate'
    ],
    '🎻': [
        'Violin',
        'String Quartet',
        'World\'s Smallest Violin'
    ],
    '🎺': [
        'Trumpet',
        'Horn',
        'Jazz'
    ],
    '🎸': [
        'Guitar',
        'Acoustic Guitar',
        'Bass Guitar',
        'Electric Guitar'
    ],
    '🎷': [
        'Saxophone',
        'Jazz',
        'Sax'
    ],
    '🥁': [
        'Drum',
        'Drum with Drumsticks'
    ],
    '🎹': [
        'Piano',
        'Musical Keyboard'
    ],
    '🪕': ['Banjo'],
    '🪗': ['Accordion'],
    '🪘': ['Long Drum'],
    '🎨': [
        'Artist Palette',
        'Art',
        'Painting'
    ],
    '🧩': [
        'Puzzle Piece',
        'Jigsaw Puzzle Piece'
    ],
    '🎲': [
        'Game Die',
        'Dice'
    ],
    '🎭': [
        'Performing Arts',
        'Theatre',
        'Theater',
        'Drama Masks',
        'Greek Theatre Masks',
        'Tragedy and Comedy Masks',
        'Venice'
    ],
    '🔍': [
        'Magnifying Glass Tilted Left',
        'Magnifier',
        'Search Icon',
        'Left-Pointing Magnifying Glass',
        'Find'
    ],
    '🔎': [
        'Magnifying Glass Tilted Right',
        'Magnifier',
        'Search Icon',
        'Right-Pointing Magnifying Glass',
        'Find'
    ],
    '⌛': [
        'Hourglass Done',
        'Hourglass'
    ],
    '⏳': [
        'Hourglass Not Done',
        'Hourglass with Flowing Sand'
    ],
    '🕐': [
        'One O\'Clock',
        'Clock Face One O\'Clock'
    ],
    '🕜': ['Clock Face One-Thirty'],
    '🕑': [
        'Two O\'Clock',
        'Clock Face Two O\'Clock'
    ],
    '🕝': ['Clock Face Two-Thirty'],
    '🕒': [
        'Three O\'Clock',
        'Clock Face Three O\'Clock'
    ],
    '🕞': ['Clock Face Three-Thirty'],
    '🕓': [
        'Four O\'Clock',
        'Clock Face Four O\'Clock'
    ],
    '🕟': ['Clock Face Four-Thirty'],
    '🕔': [
        'Five O\'Clock',
        'Clock Face Five O\'Clock'
    ],
    '🕠': ['Clock Face Five-Thirty'],
    '🕕': [
        'Six O\'Clock',
        'Clock Face Six O\'Clock'
    ],
    '🕡': ['Clock Face Six-Thirty'],
    '🕖': [
        'Seven O\'Clock',
        'Clock Face Seven O\'Clock'
    ],
    '🕢': ['Clock Face Seven-Thirty'],
    '🕗': [
        'Eight O\'Clock',
        'Clock Face Eight O\'Clock'
    ],
    '🕣': ['Clock Face Eight-Thirty'],
    '🕘': [
        'Nine O\'Clock',
        'Clock Face Nine O\'Clock'
    ],
    '🕤': ['Clock Face Nine-Thirty'],
    '🕙': [
        'Ten O\'Clock',
        'Clock Face Ten O\'Clock'
    ],
    '🕥': ['Clock Face Ten-Thirty'],
    '🕚': [
        'Eleven O\'Clock',
        'Clock Face Eleven O\'Clock'
    ],
    '🕦': ['Clock Face Eleven-Thirty'],
    '🕛': [
        'Twelve O\'Clock',
        'Clock Face Twelve O\'Clock'
    ],
    '🕧': ['Clock Face Twelve-Thirty'],
    '♨️': [
        'Hot Springs',
        'Onsen',
        'Steam'
    ],
    '💈': [
        'Barber Pole',
        'Barber Shop',
        'Barber\'s Stripes',
        'Hairdresser'
    ],
    '🎪': [
        'Circus Tent',
        'Big Top',
        'Circus'
    ],
    '💢': [
        'Anger Symbol',
        'Anger Sign',
        'Vein Pop'
    ],
    '🗯️': [
        'Right Anger Bubble',
        'Zig Zag Bubble',
        'Angry Speech Bubble'
    ],
    '💊': [
        'Pill',
        'Capsule',
        'Tablet',
        'Drugs'
    ],
    '🎎': [
        'Dolls',
        'Hinamatsuri',
        'Imperial Dolls',
        'Japanese Dolls'
    ],
    '🎏': [
        'Carp Streamer',
        'Fish Flag',
        'Koinobori',
        'Wind Socks'
    ],
    '🎐': [
        'Wind Chime',
        'Furin',
        'Jellyfish',
        'Wind Bell'
    ],
    '🎀': [
        'Ribbon Bow',
        'Pink Bow',
        'Bow'
    ],
    '🎁': [
        'Wrapped Gift',
        'Birthday Present',
        'Christmas Present',
        'Gift',
        'Gift Box',
        'Present',
        'Wrapped Present'
    ],
    '🎫': [
        'Ticket',
        'Ticket Stub',
        'World Tour Ticket'
    ],
    '🃏': [
        'Joker',
        'Joker Card',
        'Playing Card Black Joker'
    ],
    '💡': [
        'Light Bulb',
        'Idea',
        'Electric Light Bulb'
    ],
    '🔦': [
        'Flashlight',
        'Torch',
        'Electric Torch'
    ],
    '🏮': [
        'Red Paper Lantern',
        'Asian Lantern',
        'Japanese Lantern',
        'Red Lantern',
        'Izakaya Lantern'
    ],
    '📦': [
        'Package',
        'Box',
        'Parcel'
    ],
    '📇': [
        'Index Card',
        'Rolodex',
        'System Card',
        'Card Index'
    ],
    '🕳️': ['Hole'],
    '👁️‍🗨️': [
        'I Am A Witness',
        'Eye in Speech Bubble'
    ],
    '⚜️': [
        'Fleur-de-lis',
        'Scouts',
        'New Orleans Saints'
    ],
    '⚗️': ['Alembic'],
    '🕯️': ['Candle'],
    '🖼️': [
        'Framed Picture',
        'Painting',
        'Picture Frame',
        'Frame with Picture'
    ],
    '🛍️': ['Shopping Bags'],
    '🎟️': ['Admission Tickets'],
    '🛎️': ['Bellhop Bell'],
    '🧾': ['Receipt'],
    '🧮': ['Abacus'],
    '🧫': ['Petri Dish'],
    '🧹': [
        'Broom',
        'Brush',
        'Sweep'
    ],
    '🎰': [
        'Slot Machine',
        'Casino',
        'Fruit Machine',
        'Gambling',
        'Poker Machine'
    ],
    '🦽': ['Manual Wheelchair'],
    '🦼': ['Motorized Wheelchair'],
    '🪔': ['Diya Lamp'],
    '🦯': ['White Cane'],
    '🩸': ['Drop of Blood'],
    '🩹': ['Adhesive Bandage'],
    '🩺': ['Stethoscope'],
    '🪶': ['Feather'],
    '🪤': ['Mouse Trap'],
    '🫖': ['Teapot'],
    '🪨': ['Rock'],
    '🪵': ['Wood'],
    '🪄': ['Magic Wand'],
    '🔮': [
        'Crystal Ball',
        'Clairvoyant',
        'Fortune Teller',
        'Psychic',
        'Purple Crystal'
    ],
    '🪅': ['Piñata'],
    '🪆': [
        'Nesting Dolls',
        'Matryoshka'
    ],
    '🪡': ['Sewing Needle'],
    '🪢': ['Knot'],
    '🪙': ['Coin'],
    '🪃': ['Boomerang'],
    '🪥': ['Toothbrush'],
    '🪦': ['Headstone'],
    '🪧': ['Placard'],
    '🪒': ['Razor'],
    '🫙': ['Jar'],
    '🛞': [
        'Wheel',
        'Vehicles',
        'Cars'
    ],
    '🛟': [
        'Ring Buoy',
        'Life Preserver',
        'Life Ring',
        'Boats',
        'Safety',
        'Water',
        'Lifesaving'
    ],
    '🪬': [
        'Hamsa',
        'Islam',
        'Judaism',
        'Buddhism',
        'Christianity',
        'Religions',
        'Luck',
        'Protection',
        'Guidance',
        'Feminine Power',
        'Faith'
    ],
    '🪩': [
        'Mirror Ball',
        'Disco Ball'
    ],
    '🪫': [
        'Low Battery',
        'Red',
        'Energy'
    ],
    '🩻': ['X-Ray'],
    '🫧': [
        'Bubbles',
        'Soap Suds',
        'Carbonation',
        'Effervescent Personality'
    ],
    '🪪': [
        'Identification Card',
        'Identity',
        'Passport',
        'ID'
    ]
};